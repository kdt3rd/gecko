...
int safemain( int argc, char *argv[] )
{
    ...
    web::json_rpc server;
    server.method<void( const std::string &, int64_t, bool )>( "print_param" ) =
        [&]( const std::string &x, int64_t y, bool z ) -> void {
        test.message( "print_param {0} {1} {2}", x, y, z ? "true" : "false" );
    };

    server.method<base::json( void )>( "string_message" ) =
        []( void ) -> base::json { return base::json( "Hello World" ); };
