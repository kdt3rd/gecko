...
struct CustomThreadData
{
	...
	DefThreadData *operator()( void ) const
	{
		return new DefThreadData;
	}
};
...
struct ThreadData
{
	...
	explicit ThreadData( const char *ptr )
		: _ptr( ptr )
	{
		++tdCount;
	}
...
using DefThreadDataPtr = base::thread_ptr<DefThreadData, std::default_delete<DefThreadData>, CustomThreadData>;


void defthread1( const DefThreadDataPtr &td )
{
	td->dummy = 2;
}

void defthread2( const DefThreadDataPtr &td )
{
	td->dummy = 3;
}

using CtorDefThreadDataPtr = base::thread_ptr<DefThreadData, std::default_delete<DefThreadData>, CustomThreadData>;

void ctorthread1( const CtorDefThreadDataPtr &td )
{
	td->dummy = 2;
}

void ctorthread2( const CtorDefThreadDataPtr &td )
{
	td->dummy = 3;
}

using ThreadDataPtr = base::thread_ptr< ThreadData, std::default_delete<ThreadData>, base::thread_specific_null_ctor<ThreadData> >;
