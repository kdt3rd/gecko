...
int safemain( int argc, char *argv[] )
{
...
#endif
            fstest.failure(
                "registration succeeded, should have already defined automatically" );
        }
        catch ( std::exception &e )
        {
            fstest.success(
                "registration correctly failed for pre-registered fs: {0}",
                e.what() );
        }
    };
...
#ifdef _WIN32
        fstest.failure(
            "statfs not implemented for Win32 - need statvfs struct" );
#else
        struct statvfs fsinfo;
        fs->statfs( tmppath, &fsinfo );
        fstest.success(
            "file system at {0} has {1} bytes free",
            tmppath,
            ( fsinfo.f_bsize * fsinfo.f_bavail ) );
#endif
    };
    ...
    fstest["iostream"] = [&]( void ) {
        std::string msg =
            "The answer to the question of the Life, the Universe, and Everything is: 42";
    ...
    fstest["link"] = [&]( void ) {
        ...
        if ( buf.st_nlink == 2 )
            ...
            fstest.failure(
                "linked file should have two links, got {0}", buf.st_nlink );
    };
    ...
    fstest.cleanup() = [&]( void ) { fs->rmdir_all( tmppath ); };
