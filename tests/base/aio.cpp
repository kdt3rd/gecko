...
int safemain( int argc, char *argv[] )
{
	...
	test["linux_aio"] = [&]( void )
	{
        ...
        buf[13] = '\0';
        int idle_count = 0;
        ssize_t nread = aiodriver.idle_read( [&]() { ++idle_count; },
                                             buf, 13, testinput.rdbuf(), 0 );
        if ( nread == 13 && 0 == strcmp(buf, "Hello, world!") && idle_count > 0 )
            test.success( "Able to read from tmp file" );
        else
            test.failure( "aio driver idled {0}, read '{1}' instead of 'Hello, world!'", idle_count, buf );
        idle_count = 0;
        nread = aiodriver.idle_read( [&]() { ++idle_count; },
                                     buf, 1, testinput.rdbuf(), 12 );
        buf[1] = '\0';
        if ( nread == 1 && buf[0] == '!' && idle_count > 0 )
            test.success( "Able to read from tmp file at offset" );
        else
            test.failure( "aio driver idled {0}, read '{1}' instead of '!'", idle_count, buf );
	};
#endif
