...
int safemain( int argc, char *argv[] )
{
    base::cmd_line options(
        ...
        base::cmd_line::option(
            ...
            "<input_file>",
            base::cmd_line::arg<1>,
            "Input file pattern to transcode",
            true ),
        base::cmd_line::option(
            0,
            std::string(),
    ...
    if ( inP && outP )
    {
        ...
        {
            std::cout << "Converting track '" << vt->name() << "' frames "
                      << vt->begin() << " - " << vt->end() << " @ rate "
                      << vt->rate() << std::endl;
        ...
        for ( auto &vt: c.video_tracks() )
        {
            for ( int64_t f = vt->begin(); f <= vt->end(); ++f )
            {
                ...
                image_buf img = extract_frame(
                    *curFrm, std::string(), std::string(), { "R", "G", "B" } );
                int origw = img.width();
                ...
                oc.video_tracks()[ovt]->store(
                    f,
                    to_frame( img, { "R", "G", "B" }, "f16" ),
                    allocator::get() );
