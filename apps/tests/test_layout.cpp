...
namespace
{
template <typename Area> class widget : public Area
{
public:
    widget( const gl::color &c ) : _rect( c ) { this->set_minimum( 100, 25 ); }
    ...
    template <typename W> void draw_subchild( const std::shared_ptr<W> &a )
    {
        _children.push_back(
            [=]( platform::context &ctxt ) { a->draw( ctxt ); } );
    }

    template <typename W, typename... Args>
    auto add_child( const std::shared_ptr<W> &a, Args &&... args )
        -> decltype( Area::add( a, std::forward<Args>( args )... ) )
    {
        draw_subchild( a );
...
template <typename Area> class terminal_widget : public Area
{
public:
    terminal_widget( const gl::color &c ) : _rect( c )
    {
...
std::shared_ptr<widget<layout::tree>> make_tree( void )
{
    ...
    auto result =
        std::make_shared<widget<layout::tree>>( gl::grey, groove, title );
...
int safemain( int argc, char *argv[] )
{
    ...
    auto tree = make_tree();
    auto form = std::make_shared<terminal_widget<layout::form>>(
        gl::color( 0.75, 0.75, 0.75 ), tree );
    ...
    for ( size_t i = 0; i < 3; ++i )
    {
        ...
        auto f =
            std::make_shared<terminal_widget<layout::field>>( gl::grey, l, e );
    ...
    auto ftest = std::make_shared<terminal_widget<layout::field>>(
        gl::grey, label, field );
    ftest->set_spacing( 5, 5 );
    ftest->draw_subchild( label );
    ftest->draw_subchild( field );

    auto right = std::make_shared<widget<layout::box>>(
        gl::gray, base::alignment::BOTTOM );
    right->add_child( ftest );
    right->set_spacing( 5, 5 );

    auto top    = std::make_shared<widget<layout::box>>( gl::grey );
    auto center = std::make_shared<widget<layout::box>>(
        gl::black, base::alignment::BOTTOM );
    ...
    auto scroll =
        std::make_shared<terminal_widget<layout::scroll>>( gl::black );
    {
    ...
    win->set_minimum_size(
        win->query_screen()->to_native_horiz( root.minimum_width() ),
    ...
    win->exposed = [&]( void ) {
        ...
        ogl.set_projection( gl::matrix4::ortho(
            0,
