        ...
        for ( auto &v: opt.values() )
        {
            ...
            for ( auto &vt: c.video_tracks() )
            {
                ...
                for ( size_t l = 0, L = f->layer_count(); l != L; ++l )
                {
                    ...
                    std::cout << "   layer " << l << ": '" << curl.name()
                              << "' " << curl.view_count()
                              << " views (default: '"
                              << curl.default_view_name() << "')\n";
                    for ( size_t cv = 0, V = curl.view_count(); cv != V; ++cv )
                    {
                        ...
                        std::cout << "      view " << cv << ": " << curv.name()
                                  << '\n';
                        auto img =
                            static_cast<std::shared_ptr<media::image>>( curv );
                        if ( img )
                        {
                            std::cout << "        image active_area: "
                                      << img->active_area() << '\n';
                            std::cout << "                   planes: ";
                        ...
                        auto dt =
                            static_cast<std::shared_ptr<media::data>>( curv );
                        if ( dt )
                        {
                            std::cout << "        deep active_area: "
                                      << dt->active_area() << '\n';
                            std::cout << "                  planes: ";
