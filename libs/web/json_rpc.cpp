...
base::json json_rpc::local_call( const base::json &rpc )
{
    try
    {
        ...
        if ( rpc["jsonrpc"].get<base::json_string>() != "2.0" )
        {
            ...
            return error(
                -32600,
                base::json( std::string( base::format(
                    "expect jsonrpc version 2.0, got {0}",
                    rpc["jsonrpc"].get<std::string>() ) ) ) );
        }
    }
    catch ( std::exception &e )
    {
...
base::json json_rpc::error( int code, base::json &&data )
{
    ...
    switch ( code )
    {
        case -32700: error["message"] = "Parse error"; break;

        case -32600: error["message"] = "Invalid request"; break;

        case -32601: error["message"] = "Method not found"; break;

        case -32602: error["message"] = "Invalid params"; break;

        case -32603: error["message"] = "Internal error"; break;

        default: error["message"] = "Server error"; break;
    }
    if ( data.valid() )
