    ...
    : _socket( std::move( client ) )
{
    if ( req["Connection"] != "Upgrade" )
        throw_runtime(
            "Request is not an upgrade connection (got {0})",
            req["Connection"] );
    if ( req["Upgrade"] != "websocket" )
        throw_runtime(
            "Request is not for a WebSocket upgrade (got {0})",
            req["Upgrade"] );
    ...
    resp.set_header(
        "Sec-WebSocket-Accept", base::base64_encode( h.data(), h.size() ) );
    resp.send( _socket );
    _masked = true;
}
...
{
    precondition(
        server.scheme() == "ws", "not WebSocket scheme: {0}", server.pretty() );
    ...
    req.set_header(
        "Host", server.host() + ":" + base::to_string( server.port( 80 ) ) );
