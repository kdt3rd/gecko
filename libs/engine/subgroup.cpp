...
subgroup::subgroup( graph &g ) : _graph( g ) {}

////////////////////////////////////////

subgroup::subgroup( graph &g, node_id n ) : _graph( g ) { add( n ); }
    ...
    for ( size_t i = 0, nI = curN.input_size(); i != nI; ++i )
    {
        ...
        if ( m != _nodes.end() )
        {
            ...
            if ( iso != _outputs.end() )
            {
                ...
                if ( inNode.has_ref() )
                    throw_runtime(
                        "Attempt to add an input to a subgroup with a node that has a reference" );
...
bool subgroup::is_output( node_id n ) const
{
...
bool subgroup::is_member( node_id n ) const
{
...
bool subgroup::can_merge( const subgroup &o ) const
{
...
bool subgroup::internal_merge_check( const subgroup &o ) const
{
    graph &         g   = _graph;
    ...
    for ( auto &n: _nodes )
    {
        ...
        {
            for ( auto i = cur.begin_inputs(), ei = cur.end_inputs(); i != ei;
                  ++i )
            {
                if ( o.is_member( *i ) )
                {
                    std::cout << "internal_merge_check fails because input "
                              << i << " for node " << n
                              << " is a member of other" << std::endl;
                    return false;
                }
                if ( g.has_ancestor( *i, o.members() ) )
                {
                    std::cout << "internal_merge_check fails because input "
                              << i << " for node " << n
                              << " has an ancestor in other" << std::endl;
                    return false;
                }
            }
        }
...
node_id subgroup::last_input( void ) const
{
    if ( _inputs.empty() )
        return nullnode;
...
node_id subgroup::first_member( void ) const
{
...
void subgroup::swap( subgroup &o )
{
    precondition(
        &_graph != &( o._graph ),
        "attempt to swap subgroups from different graphs" );
...
void subgroup::bind_functions(
    std::vector<std::shared_ptr<subgroup_function>> &funcs )
{
...
any &subgroup::output_val( size_t i ) { return _graph[_outputs[i]].value(); }

////////////////////////////////////////

size_t subgroup::func_idx( node_id n )
{
    size_t idx = 0;
    ...
    postcondition(
        idx != N,
        "request for node {0} that is not a member of subgroup with first member {1}",
        n,
        first_member() );
