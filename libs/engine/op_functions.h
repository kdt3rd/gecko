...
public:
    ...
    virtual any
    process( graph &g, const dimensions &d, const std::vector<any> &inputs )
        const = 0;

    virtual any create_value( const dimensions &d ) const = 0;

    virtual std::shared_ptr<subgroup_function>
                 create_group_function( void ) const                      = 0;
...
public:
    ...
    typedef std::function<void( subgroup &, const dimensions & )>
        group_dispatch;
    ...
    virtual any process(
        graph &, const dimensions &, const std::vector<any> & ) const override
    {
    ...
    virtual std::shared_ptr<subgroup_function>
    create_group_function( void ) const override
    {
        return std::make_shared<GroupFunc>( _p );
    }

    virtual void
    dispatch_group( subgroup &s, const dimensions &d ) const override
    {
        _dispatch( s, d );
    }
...
template <typename Functor> class opfunc_simple : public op_function
{
public:
    typedef typename base::function_traits<Functor>::function process_func;

    opfunc_simple( Functor f ) : _p( base::to_function( f ) ) {}
    ...
    virtual any
    process( graph &, const dimensions &, const std::vector<any> &inputs )
        const override
    {
        return dispatch(
            inputs,
            base::gen_sequence<base::function_traits<Functor>::arity>{} );
    }
...
private:
    template <size_t I>
    inline
        typename base::function_traits<Functor>::template get_arg_type<I>::type
        extract( const std::vector<any> &inputs ) const
    {
        typedef typename base::function_traits<Functor>::template get_arg_type<
            I>::type ret_type;
    ...
    template <size_t... S>
    inline any dispatch(
        const std::vector<any> &inputs, const base::sequence<S...> & ) const
    {
...
{
public:
    opfunc_locked_simple( Functor f ) : opfunc_simple<Functor>( f ) {}
