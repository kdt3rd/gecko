...
node::~node( void ) { delete[] _edges; }

////////////////////////////////////////

void node::add_output( node_id o )
{
...
void node::remove_output( node_id o )
{
    if ( _output_count > 1 )
    {
        ...
        postcondition(
            outs != endOuts, "node id {0} not an output of this node", o );
    ...
    {
        precondition(
            output( 0 ) == o, "node id {0} not an output of this node", o );
        _output_count = 0;
    }
}
...
void node::update_input( node_id oldid, node_id newid )
{
...
void node::update_output( node_id oldid, node_id newid )
{
...
void node::resize_edges( uint32_t num_in, uint32_t num_out )
{
    ...
    if ( s > _storage_count )
    {
        ...
        {
            std::copy(
                _edges, _edges + std::min( num_in, _input_count ), nEdges );
            std::copy(
                _edges + _input_count,
                _edges + _input_count + std::min( _output_count, num_out ),
                nEdges + num_in );
            delete[] _edges;
        }
        _edges         = nEdges;
