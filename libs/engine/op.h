...
public:
    ...
    inline op(
        base::cstring n,
        Functor       f,
        const GroupProcessFunc &,
        const std::function<void( subgroup &, const dimensions & )> &g,
        one_to_one_parallel_t )
        : _name( n )
        , _func( new opfunc_one_to_one<Functor, GroupProcessFunc>( f, g ) )
        , _style( style::ONE_TO_ONE )
    {}
    ...
    inline op(
        ...
        n_to_one_parallel_t )
        : _name( n )
        , _func( new opfunc_one_to_one<Functor, GroupProcessFunc, 2>( f, g ) )
        , _style( style::N_TO_ONE )
    {}
    ...
    op( base::cstring n, Functor f, threaded_t )
        : _name( n )
        , _func( new opfunc_simple<Functor>( f ) )
    ...
    op( base::cstring n, Functor f, solitary_t )
        : _name( n )
        , _func( new opfunc_locked_simple<Functor>( f ) )
    ...
    op( base::cstring                                       n,
        const std::reference_wrapper<const std::type_info> &ti,
        value_t );
...
inline const std::string &op::name( void ) const { return _name; }

////////////////////////////////////////

inline op::style op::processing_style( void ) const { return _style; }
...
{
    precondition(
        _func, "Invalid operation function for operation {0}", _name );
...
{
    return (
        processing_style() == style::ONE_TO_ONE &&
