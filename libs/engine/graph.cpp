...
std::ostream &operator<<( std::ostream &os, const dimensions &d )
{
    os << "[ (" << d.x1 << ',' << d.y1 << " - " << d.x2 << ',' << d.y2 << ") "
       << ( d.x2 - d.x1 + 1 ) << 'x' << ( d.y2 - d.y1 + 1 ) << 'x' << d.planes
       << 'x' << d.images << 'x' << d.bytes_per_item << " ]";
    return os;
}

////////////////////////////////////////
graph::graph( const registry &r ) : _ops( r ), _computing( 0 ) {}
...
void graph::tag_rvalue( node_id n )
{
...
const any &graph::get_value( node_id n )
{
    precondition( n < _nodes.size(), "invalid node id {0}", n );
...
node_id graph::copy_node( const graph &o, node_id n )
{
    ...
    while ( !tocopy.empty() )
    {
        ...
        if ( doAdd )
        {
            ...
            if ( &srcReg != &_ops )
            {
                ...
                nnode = add_node(
                    opid, cur.value(), cur.dims(), inputs, cur.hash_value() );
...
node_id graph::move_node( graph &o, node_id n )
{
    precondition( n < o.size(), "invalid node id {0}", n );
    if ( &o == this )
        return n;
    precondition(
        o[n].output_size() == 0,
        "Attempt to move a node with outputs in graph" );
...
void graph::remove_node( node_id n )
{
...
void graph::clean_graph( void )
{
    ...
    {
        didAdd = false;
        for ( node_id nid = 0, N = static_cast<node_id>( _nodes.size() );
              nid != N;
              ++nid )
        {
            ...
            for ( size_t o = 0, nO = n.output_size(); o != nO; ++o )
            {
                ...
                if ( !_nodes[onid].value().has_value() &&
                     toDel.find( onid ) == toDel.end() )
                    ++validOuts;
            }
    ...
    for ( node_id nid = 0, N = static_cast<node_id>( _nodes.size() ); nid != N;
          ++nid )
    {
        if ( toDel.find( nid ) == toDel.end() )
        {
    ...
    for ( node_id nid = 0, N = static_cast<node_id>( nnodes.size() ); nid != N;
          ++nid )
    {
        node &n = nnodes[nid];
...
static void emit_node(
    ...
    const node &    curN,
    bool            incHash,
    bool            isSGoutput = false )
{
    os << std::setw( indent ) << std::setfill( ' ' ) << "" << 'N' << n
       << " [label=\"";
    if ( curN.input_size() > 0 )
    {
        ...
        for ( size_t i = 0, nI = curN.input_size(); i != nI; ++i )
        {
            ...
            if ( curN.input( i ) == nullnode )
                ...
                os << "<i" << i << ">"
                   << esc_dot(
                          base::demangle( ops[curN.op()].input_type( i ) ) );
        }
    ...
    else
    {
        ...
        if ( curN.value().has_value() )
        {
            ...
            if ( v.type() == typeid( float ) )
                ...
                os << "\\n"
                   << esc_dot( ops[curN.op()].name() ) << " ("
                   << esc_dot( base::demangle(
                          ops[curN.op()].function().result_type() ) );
        }
        else
            os << "\\n" << esc_dot( ops[curN.op()].name() );
        if ( incHash )
...
void graph::dump_dot( const std::string &fn, bool incHash ) const
{
...
void graph::dump_dot( std::ostream &os, bool incHash ) const
{
    ...
    for ( size_t si = 0; si != _subgroups.size(); ++si )
    {
        ...
        os << "\n    label=\"subgroup " << sgnum << " index " << si
           << " outputs " << s.outputs().size()
           << "\";\n    color=black\n  }\n";
        ++sgnum;
    }

    for ( node_id n = 0, N = static_cast<node_id>( _nodes.size() ); n != N;
          ++n )
    {
        const node &curN = _nodes[n];
        if ( curN.op() == nullop )
...
void graph::dump_refs( std::ostream &os ) const
{
    ...
    for ( auto &r: _ref_counts )
        os << std::setw( 8 ) << std::setfill( ' ' ) << r.first << "  "
           << std::setw( 5 ) << r.second.size() << std::endl;
}
...
static void update_nid( void *ud, node_id old, node_id nid )
{
    ...
    precondition(
        *nptr == old,
        "Out of date reference, expect {0}, got {1}, new {2}",
        *nptr,
        old,
        nid );
    *nptr = nid;
...
const any &graph::process( node_id nid )
{
    ...
    for ( node_id c: _process_list )
    {
        ...
        if ( curN.in_subgroup() )
        {
            ...
            for ( auto sginn: sg.inputs() )
            {
                ...
                {
                    std::cout << "  subgroup " << sgi << " input node " << sginn
            ...
            if ( !haveAllValues )
            {
                std::cout << "waiting to process subgroup " << sgi
                          << " until later, missing values: last input "
                          << sg.last_input() << ", current node " << c
                          << std::endl;
                continue;
            }
...
void graph::move_constants( void )
{
...
void graph::apply_peephole( void ) {}

////////////////////////////////////////

void graph::apply_grouping( void )
{
...
//						std::cout << "ADDING node " << n << " to subgroup " << maxInputGroup << std::endl;
                ...
                if ( soloIn != size_t( -1 ) )
                {
                    ...
                    TODO(
                        "Add accessor to op to check for same grouping function" );
                    TODO( "Fix the multi-input/output grouping" );
                    doCombine = ( curInN.output_size() == 1 );
                    doCombine = doCombine &&
                                ( curInN.in_subgroup() && !curInN.has_ref() );
                    doCombine = doCombine &&
                                ( curOp.function().result_type() ==
                                  _ops[curInN.op()].function().result_type() );
...
void graph::clear_grouping( void )
{
    _subgroups.clear();
...
size_t graph::find_subgroup( node_id n ) const
{
...
size_t graph::merge_subgroups( size_t a, size_t b )
{
    size_t ret  = a;
    size_t from = b;
...
void graph::split_subgroup( size_t i, node_id n )
{
...
bool graph::ok_to_merge( size_t sg, size_t check, node_id n )
{
    subgroup &sub = _subgroups[sg];
    for ( node_id outs: sub.outputs() )
    {
        node &curOut = _nodes[outs];
        for ( auto o = curOut.begin_outputs(), oe = curOut.end_outputs();
              o != oe;
              ++o )
        {
            if ( *o != n )
            {
                ...
                {
                    std::cout << "Not merging subgroup " << sg << " and "
                              << check << " because node " << outs
                              << " output is " << *o << " which is in " << check
                              << std::endl;
                    return false;
                }
            }
...
bool graph::has_ancestor( node_id n, const std::vector<node_id> &nodes ) const
{
...
bool graph::has_ancestor( node_id n, node_id a ) const
{
    precondition( n < _nodes.size(), "invalid node id" );
    const node &node = _nodes[n];
    for ( size_t i = 0, N = node.input_size(); i != N; ++i )
...
void graph::rotate_node( node_id oldpos, node_id newpos )
{
    precondition(
        oldpos < _nodes.size() && newpos < _nodes.size(),
        "invalid positions given to rotate" );
    precondition(
        _nodes[oldpos].op() != nullop, "invalid node given to rotate" );
    ...
    if ( newpos < oldpos )
    {
        for ( node_id x = 0; x < newpos; ++x )
            newnodemap[x] = x;
        for ( node_id x = oldpos + 1; x < static_cast<node_id>( _nodes.size() );
              ++x )
            newnodemap[x] = x;
    }
    else
    {
        for ( node_id x = 0; x < oldpos; ++x )
            newnodemap[x] = x;
        for ( node_id x = newpos + 1; x < static_cast<node_id>( _nodes.size() );
              ++x )
            newnodemap[x] = x;
    }
    ...
    for ( size_t i = 0, nC = tmpStore.output_size(); i != nC; ++i )
    {
        ...
        precondition(
            newpos < outN,
            "attempt to move a node from {0} to {1}, past a node {1} who has it as an input",
            oldpos,
            newpos,
            outN );
    ...
    for ( size_t i = 0, nC = tmpStore.input_size(); i != nC; ++i )
    {
        ...
        precondition(
            newpos > inN,
            "attempt to move a node from {0} to {1}, before one of it's inputs: {2}",
            oldpos,
            newpos,
            inN );
...
node_id graph::find_node( const hash::value &hv )
{
    ...
    hash &                         h )
{
    if ( _ops[op].input_size() != inputs.size() )
        throw_logic(
            "Invalid number of inputs passed to operator {0}, expect {1}, got {2}",
            _ops[op].name(),
            _ops[op].input_size(),
            inputs.size() );
    ...
    const hash::value &            hv )
{
    ...
    if ( n != nullnode )
    {
        ...
        if ( colCheck.op() != op || colCheck.input_size() != inputs.size() ||
             colCheck.dims() != d )
            throw_logic(
                "Hash collision with existing node {0} adding opname '{1}'",
                n,
                _ops[op].name() );
    ...
    const hash::value &         hv )
{
    ...
    if ( n != nullnode )
    {
        ...
        if ( colCheck.op() == nullop )
            throw_logic(
                "Graph {6}, Hash collision with existing NULL node {0}/{5} adding opname '{1}' dims {2}: {3} vs {4}",
                n,
                _ops[op].name(),
                d,
                colCheck.hash_value(),
            ...
            colCheck.op() != op || colCheck.input_size() != inputs.size() ||
            colCheck.dims() != d )
            throw_logic(
                "Hash collision with existing node {0} ({1}, {2}) adding opname '{3}' {7}/{8} dims {4}: {5} vs {6}",
                n,
                _ops[colCheck.op()].name(),
                colCheck.dims(),
                _ops[op].name(),
                d,
                colCheck.hash_value(),
...
void graph::update_hash_map( void )
{
...
void graph::update_refs( const std::map<node_id, node_id> &newnodemap )
{
...
void graph::reference( node_id n, rewrite_notify notify, void *ud )
{
...
void graph::unreference( node_id n, rewrite_notify notify, void *ud ) noexcept
{
