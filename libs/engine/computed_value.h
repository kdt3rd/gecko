...
public:
        ...
        Args &&... args )
    {
        ...
        set_id( _graph->add_node(
            opname,
            d,
            { check_or_add( *_graph, std::forward<Args>( args ) )... } ) );
    }

    template <typename... Args>
    explicit inline computed_base(
        const base::cstring &opname, const dimensions &d, Args &&... args )
    {
        _graph = find_or_create_graph(
            registry::get(), std::forward<Args>( args )... );
        set_id( _graph->add_node(
            opname,
            d,
            { check_or_add( *_graph, std::forward<Args>( args ) )... } ) );
    ...
    inline const std::shared_ptr<graph> &graph_ptr( void ) const
    {
        return _graph;
    }
...
protected:
    ...
    template <typename X, bool> struct check_delegate;

    template <typename X> struct check_delegate<X, false>
    {
        ...
        static inline std::shared_ptr<graph>
        find_graph( const registry &, X && )
        {
            return std::shared_ptr<graph>();
        }
    };
    template <typename X> struct check_delegate<X, true>
    {
        static inline node_id process( graph &g, X &&v )
        {
        ...
        static inline std::shared_ptr<graph>
        find_graph( const registry &reg, X &&v )
        {
    ...
    template <typename X> struct check_delegate<const X &, true>
    {
        ...
        static inline std::shared_ptr<graph>
        find_graph( const registry &reg, const X &v )
        {
            const std::shared_ptr<graph> &r = v.graph_ptr();
    ...
    template <typename X> inline node_id check_or_add( graph &g, X &&v )
    {
        return check_delegate<
            X,
            std::is_base_of<computed_base, typename std::decay<X>::type>::
                value>::process( g, std::forward<X>( v ) );
    }
    ...
    inline std::shared_ptr<graph> check_for_graph( const registry &reg, X &&v )
    {
        return check_delegate<
            X,
            std::is_base_of<computed_base, typename std::decay<X>::type>::
                value>::find_graph( reg, std::forward<X>( v ) );
    }
    ...
    inline std::shared_ptr<graph> find_or_create_graph( const registry &reg )
    {
...
template <typename V> class computed_value : public computed_base
{
public:
    static_assert(
        !std::is_base_of<computed_base, typename std::decay<V>::type>::value,
        "computed_value shouldn't be created for things that are already computed" );

    inline computed_value( void ) = default;

    template <typename A> explicit inline computed_value( A &&v )
    {
        _graph = std::make_shared<graph>( registry::get() );
        set_id(
            _graph->add_constant( static_cast<V>( std::forward<A>( v ) ) ) );
    }

    template <typename... Args>
    inline computed_value(
        const base::cstring &opname, const dimensions &d, Args &&... args )
        : computed_base(
              registry::get(), opname, d, std::forward<Args>( args )... )
    {}
    ...
    template <typename A> inline computed_value &operator=( A &&v )
    {
        clear_graph();
        _graph = std::make_shared<graph>( registry::get() );
        set_id(
