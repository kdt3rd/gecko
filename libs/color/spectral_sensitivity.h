...
public:
	...
	spectral_sensitivity( table xb, table yb, table zb )
		: _x_bar( std::move( xb ) ), _y_bar( std::move( yb ) ), _z_bar( std::move( zb ) )
	{}

	value_type sample_x( value_type l ) const { return _x_bar.sample( l ); }
	value_type sample_x( value_type l, value_type d ) const { return _x_bar.sample( l, d ); }
	value_type sample_y( value_type l ) const { return _y_bar.sample( l ); }
	value_type sample_y( value_type l, value_type d ) const { return _y_bar.sample( l, d ); }
