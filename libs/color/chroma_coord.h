...
public:
	...
	{
		return ( GK_LIKELY( std::abs( y ) > value_type(0) ) ) ? triplet<value_type>( Y * x / y, Y, Y * ( value_type(1) - x - y ) / y ) : triplet<value_type>( Y * x, Y, Y * ( value_type(1) - x ) );
	}

	static inline constexpr chroma_coord fromXYZ( value_type X, value_type Y, value_type Z ) noexcept
	{
		return ( GK_LIKELY( std::abs(X) > 0.F || std::abs(Y) > 0.F || std::abs(Z) > 0.F ) ) ? chroma_coord( X / (X + Y + Z), Y / (X + Y + Z) ) : chroma_coord();
	}
