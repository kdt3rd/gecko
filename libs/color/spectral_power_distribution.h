...
public:
	...
	static constexpr value_type k = T( 1.380662e-23 ); // boltzmann constant J / K
	...
	inline value_type operator()( value_type lambda ) const { return sample( lambda ); }
	inline value_type sample( value_type l ) const
	{ return c1 / ( ( l * l * l * l * l ) * ( std::exp( c2 / ( _kelvin * l ) ) - 1 ) ); }
};

// computes in relative form, such that 
template <typename T>
class illuminant_D_SPD
{
public:
	using value_type = T;
	...
	explicit constexpr illuminant_D_SPD( const chroma_coord<O> &cx ) noexcept
		: _m( value_type( O(0.0241) + O(0.2562) * cx.x - O(0.7341) * cx.y ) ),
		  _m1( value_type( O(-1.3515) - O(1.7703) * cx.x + O(5.9114) * cx.y ) / _m ),
		  _m2( value_type( O(0.0300) - O(31.4424) * cx.x + O(30.0717) * cx.y ) / _m ),
	{}
