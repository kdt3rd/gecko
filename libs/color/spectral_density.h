...
///
template <typename T> class spectral_density
{
    static_assert(
        std::is_floating_point<T>::value,
        "expect floating point value for type" );
...
public:
    ...
    spectral_density
    reinterpret( size_t nEntries, value_type start, value_type delta ) const
    {
        spectral_density r;
    ...
    {
        precondition(
            d > value_type( 0 ), "sampling invalid wavelength delta" );
        precondition( valid(), "sampling invalid spectral density table" );
        ...
        if ( nSamples % 2 == 0 )
            ++nSamples;
        value_type radius =
            static_cast<value_type>( nSamples ) / value_type( 2 );

        value_type ret = value_type( 0 );
        for ( long i = 0; i < nSamples; ++i )
        {
            value_type x =
                ( static_cast<value_type>( i ) - radius + value_type( 0.5 ) );
