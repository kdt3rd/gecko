    ...
    switch ( cr )
    {
        ...
        case cone_response::NONE:
            throw std::runtime_error(
                "Unhandled white point cone response method" );
    }
...
template <typename V>
inline triplet<V>
norm_white( cone_response cr = cone_response::DEFAULT, V Y = V( 1 ) )
{
    ...
    switch ( cr )
    {
        ...
        case cone_response::CMCCAT2000:
        case cone_response::CIECAM02:
        case cone_response::WANDELL: break;
        case cone_response::VON_KRIES: // normalized to d65...
...
///
template <typename V> class chromaticities
{
    static_assert(
        std::is_floating_point<V>::value,
        "color::chromaticities requires floating point type" );
...
public:
    ...
    inline constexpr chromaticities(
        const xy &r,
        const xy &g,
        const xy &b,
        const xy
            &w ) noexcept( std::is_nothrow_default_constructible<value_type>::
                               value )
    ...
    inline chromaticities( std::initializer_list<xy> &i )
    {
        if ( i.size() != 4 )
            throw_runtime(
                "Invalid initializer list for chromaticities, expect 4 x,y chroma pairs" );
        red   = i[0];
        green = i[1];
        blue  = i[2];
        white = i[3];
    ...
    inline mat
    XYZtoLMS( cone_response cr, value_type Y = value_type( 1 ) ) const
    {
        ...
        return ( w != nw )
                   ? mat::diag( xyz( value_type( 1 ) ) / ( m * ( w / nw ) ) ) *
                         m
                   : m;
    }

    inline mat
    LMStoXYZ( cone_response cr, value_type Y = value_type( 1 ) ) const
    {
        return XYZtoLMS( cr, Y ).invert();
    }

    /// Computes the adaptation matrix from o to this
    inline mat adaptation(
        const chromaticities &o,
        value_type            amount = value_type( 1.0 ),
        cone_response         m      = cone_response::DEFAULT ) const
    {
        amount = std::max(
            value_type( 0.0 ), std::min( value_type( 1.0 ), amount ) );

        if ( m == cone_response::NONE || white == o.white ||
             amount <= value_type( 0.0 ) )
            return mat();
        ...
        mat scale = mat::diag(
            ( rhoWdst.x / rhoWsrc.x ) * amount + value_type( 1.0 ) - amount,
    ...
    xy white = xy(
        value_type( 1 ) / value_type( 3 ), value_type( 1 ) / value_type( 3 ) );
};
