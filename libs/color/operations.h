...
tristimulus_value<
    ...
    ( bA <= bB ) ? bB : bA>
unnorm_mix(
    const tristimulus_value<Ta, bA> &a,
    const tristimulus_value<Tb, bB> &b,
    float                            m = 0.5F )
{
    precondition(
        a.current_state() == b.current_state(),
        "two colors should be in same state to mix" );
    ...
    return result_t(
        static_cast<result_comp>(
            static_cast<mix_t>( a.r() ) * mm +
            static_cast<mix_t>( b.r() ) * n ),
        static_cast<result_comp>(
            static_cast<mix_t>( a.g() ) * mm +
            static_cast<mix_t>( b.g() ) * n ),
        static_cast<result_comp>(
            static_cast<mix_t>( a.b() ) * mm +
...
tristimulus_value<
    typename std::common_type<Ta, Tb>::type,
    ( bA <= bB ) ? bB : bA>
mix( const tristimulus_value<Ta, bA> &a,
     const tristimulus_value<Tb, bB> &b,
     float                            m = 0.5F )
{
...
inline tristimulus_value<T> desaturate(
    const tristimulus_value<T, fbits> &v, T amt, bool return_to_orig = true )
{
    auto lab = convert_space( v, space::CIE_LAB_76 );
    lab.y() *= amt;
    lab.z() *= amt;
    return ( return_to_orig )
               ? convert( lab, lab.current_state(), v.current_state() )
               : lab;
}

template <typename T, int fbits>
inline T distance(
    const tristimulus_value<T, fbits> &a, const tristimulus_value<T, fbits> &b )
{
