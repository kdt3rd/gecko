...
struct transfer_curve_control
{
    ...
    constexpr transfer_curve_control(
        value_type a,
        value_type b,
        value_type c,
        value_type d,
    ...
    {}
    constexpr transfer_curve_control(
        const transfer_curve_control &s ) noexcept = default;
    constexpr transfer_curve_control( transfer_curve_control &&s ) noexcept =
        default;
    transfer_curve_control &
    operator=( const transfer_curve_control &s ) noexcept = default;
    transfer_curve_control &
    operator=( transfer_curve_control &&s ) noexcept = default;
...
inline constexpr transfer_curve_control get_curve_defaults( transfer t )
{
...
template <typename T> inline T linearize( const T v, transfer t )
{
    switch ( t )
    {
        ...
        case transfer::FOUR_PT_GAMMA_CUSTOM:
            return gamma_srgb<T>::linearize( v );
        ...
        case transfer::ARRI_LOGC_NORMSENS_SUP2:
            return gamma_srgb<T>::linearize( v );
        case transfer::ARRI_LOGC_SCENELIN_SUP2:
            return gamma_srgb<T>::linearize( v );
        case transfer::ARRI_LOGC_NORMSENS_SUP3:
            return gamma_srgb<T>::linearize( v );
        case transfer::ARRI_LOGC_SCENELIN_SUP3:
            return gamma_srgb<T>::linearize( v );
...
template <typename T> inline T encode( const T v, transfer t )
{
    switch ( t )
    {
        case transfer::LINEAR: return v;
...
inline std::ostream &
operator<<( std::ostream &os, const transfer_curve_control &tcc )
{
    os << tcc._A << ',' << tcc._B << ',' << tcc._C << ',' << tcc._D << ','
       << tcc._E;
    return os;
}
