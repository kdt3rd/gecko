...
template <typename T>
inline chroma_coord<T> illuminant_D( T ccct )
{
	double xd;
	if ( ccct < T(4000) || ccct > T(25000) )
		throw std::out_of_range( "Invalid CCCT for illuminant D" );
	if ( ccct <= T(7000) )
		xd = -4.6070e9/(ccct*ccct*ccct) + 2.9678e6 / (ccct*ccct) + 0.09911e3 / ccct + 0.244063;
	else
		xd = -2.0064e9/(ccct*ccct*ccct) + 1.9018e6 / (ccct*ccct) + 0.24748e3 / ccct + 0.237040;
	double yd = -3.0 * xd * xd + 2.870 * xd - 0.275;
	return chroma_coord<T>( T(xd), T(yd) );
}

template <typename T>
inline T CCCT( const triplet<T> &xyz )
{
	double ret = -1.0;
	if ( xyz.x > T(0) && xyz.y > T(0) && xyz.z > T(0) )
	{
		static double recipT[] = {
		...
		if ( i < nUVT )
		{
			...
			ret = 1.0 / base::lerp_fast( recipT[i - 1], recipT[i], dm / ( dm - di ) );
		}
	}
	return T(ret);
}
