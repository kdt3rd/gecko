...
template <typename V> class matrix
{
    static_assert(
        ...
        "color::matrix requires floating point type" );
...
public:
    ...
    inline constexpr matrix( value_type a, value_type b, value_type c ) noexcept
        : _m{ { a,
                value_type( 0 ),
                value_type( 0 ),
                value_type( 0 ),
    ...
    inline matrix( std::initializer_list<value_type> l )
    {
        if ( l.size() == 9 )
            ...
            throw std::logic_error(
                "Initializing 3x3 color matrix with invalid number of elements" );
    }
    inline constexpr matrix(
        ...
        value_type m20,
        value_type m21,
        value_type m22 ) noexcept
        : _m{ { m00, m01, m02, m10, m11, m12, m20, m21, m22 } }
    {}
    inline constexpr matrix(
        const triplet<value_type> &c1,
        const triplet<value_type> &c2,
        const triplet<value_type> &c3 ) noexcept
        : _m{ { c1.x, c2.x, c3.x, c1.y, c2.y, c3.y, c1.z, c2.z, c3.z } }
    {}

    template <typename U>
    inline constexpr matrix( const matrix<U> &o ) noexcept
        : _m{ { static_cast<value_type>( o[0][0] ),
                static_cast<value_type>( o[0][1] ),
                static_cast<value_type>( o[0][2] ),
                static_cast<value_type>( o[1][3] ),
                static_cast<value_type>( o[1][4] ),
                static_cast<value_type>( o[1][5] ),
                static_cast<value_type>( o[2][6] ),
                static_cast<value_type>( o[2][7] ),
    ...
    inline matrix invert( void ) const
    {
        ...
        if ( std::abs( det ) < std::numeric_limits<value_type>::epsilon() )
            throw std::runtime_error(
                "Unable to invert degenerate color matrix" );
        ...
        return matrix{ A * det, D * det, G * det, B * det, E * det,
                       H * det, C * det, F * det, I * det };
    }
...
operator*( const triplet<V> &t, const matrix<V> &a ) noexcept
{
    return triplet<V>(
...
template <typename V>
inline constexpr triplet<V>
operator*( const matrix<V> &a, const triplet<V> &t ) noexcept
{
    return triplet<V>(
