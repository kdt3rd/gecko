...
std::set<std::string> font_manager::get_families( void )
{
    ...
    if ( fs )
    {
        ...
        for ( int i = 0; i < fs->nfont; ++i )
        {
            ...
            if ( FcPatternGetString( font, FC_FAMILY, 0, &name ) ==
                 FcResultMatch )
...
std::set<std::string> font_manager::get_styles( const std::string &family )
{
    ...
    FcPattern *pat = FcPatternBuild(
        nullptr, FC_FAMILY, FcTypeString, family.c_str(), nullptr );
    FcConfigSubstitute( _impl->config, pat, FcMatchPattern );
    FcDefaultSubstitute( pat );
    ...
    if ( fs )
    {
        ...
        {
            FcPattern *font = fs->fonts[i];
            if ( FcPatternGetString( font, FC_STYLE, 0, &name ) ==
                 FcResultMatch )
                ret.emplace( reinterpret_cast<const char *>( name ) );
        }
        FcFontSetDestroy( fs );
                 ...
                 FcResultMatch )
        {
            ...
            if ( error )
                throw std::runtime_error(
                    script::freetype2::font::errorstr( error ) );

            try
            {
                ret = std::make_shared<script::freetype2::font>(
                    ftface, family, style, pts );
