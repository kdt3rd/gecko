...
void test( void )
{
    ...
    CTFontRef font =
        CTFontCreateWithName( CFSTR( "Helvetica Light" ), fontSize, nil );
    NSDictionary *      attributes = [NSDictionary
        dictionaryWithObjectsAndKeys:(id)font, kCTFontAttributeName, nil];
    ...
    CTLineRef line =
        CTLineCreateWithAttributedString( (CFAttributedStringRef)as );
    ...
    CGContextRef    ctx        = CGBitmapContextCreate(
        data, width, height, 8, width * 4, space, bitmapInfo );
    ...
    NSData *imageData =
        [imageRep representationUsingType:NSJPEGFileType properties:nil];
    NSString *fileName      = [NSString stringWithFormat:@"Kevin.jpg"];
    NSString *fileDirectory = NSHomeDirectory();
    NSString *filePath =
        [fileDirectory stringByAppendingPathComponent:fileName];
