...
public:
    ...
    {
        _out << '(' << e.expression1() << e.open() << e.expression2()
             << e.close() << ')';
    }

    void operator()( const integer_expr &e ) { _out << e.value(); }

    void operator()( const floating_expr &e ) { _out << e.value(); }

    void operator()( const identifier_expr &e ) { _out << e.value(); }
    ...
    {
        _out << "if(" << e.condition() << ") " << e.when_true() << " else "
             << e.when_false();
    }
    ...
    void operator()( const lambda_expr &e ) { _out << ( *e.get_function() ); }

    // Compile-time check for any missing operator() implementation
    template <typename T> type operator()( T /*a*/ )
    {
        static_assert(
            base::always_false<T>::value, "missing operator() for printer" );
        return type();
    }
...
public:
    ...
    std::shared_ptr<expr> operator()( const call_expr &e )
    {
        ...
        return std::make_shared<expr>(
            call_expr( e.function(), std::move( args ) ) );
    }

    std::shared_ptr<expr> operator()( const if_expr &e )
    {
        return std::make_shared<expr>( if_expr(
            e.condition()->clone(),
            e.when_true()->clone(),
            e.when_false()->clone() ) );
    }
    ...
    {
        return std::make_shared<expr>(
            for_expr( e.modifiers(), e.ranges(), e.result()->clone() ) );
    }

    std::shared_ptr<expr> operator()( const assign_expr &e )
    {
        return std::make_shared<expr>( assign_expr(
            e.variable(), e.expression()->clone(), e.next()->clone() ) );
    }

    std::shared_ptr<expr> operator()( const lambda_expr &e )
    {
        return std::make_shared<expr>(
            lambda_expr( e.get_function()->clone() ) );
    }

    // Compile-time check for any missing operator() implementation
    template <typename T> std::shared_ptr<expr> operator()( T /*a*/ )
    {
        static_assert(
            base::always_false<T>::value, "missing operator() for printer" );
