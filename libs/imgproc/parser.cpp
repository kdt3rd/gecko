...
parser::parser(
    std::map<std::u32string, std::shared_ptr<function>> &funcs,
    std::istream &                                       in,
    utf::mode                                            m )
...
bool parser::is_loop_modifier( const std::u32string &id )
{
    static std::set<std::u32string> loopmods = {
        U"sum", U"avg", U"min", U"max", U"count"
    };
...
{
    if ( _token.type() == TOK_FOR || _token.type() == TOK_IF ||
         _token.type() == TOK_BLOCK_START || _token.type() == TOK_LAMBDA )
        return primary_expr();
    ...
    else if ( _token.type() == TOK_MOD_START )
    {
        ...
        if ( _token.type() == TOK_IDENTIFIER )
        {
            ...
            if ( !expect( TOK_MOD_END ) )
                throw_runtime(
                    "expected ']' to end modifier, got '{0}'", _token.value() );
            if ( _token.type() == TOK_FOR )
            {
                ...
                throw_runtime(
                    "expected loop after modifier, got '{0}'", _token.value() );
        }
        else
            throw_runtime(
                "expected identifier for modifier, got '{0}'", _token.value() );
    }
...
std::vector<std::shared_ptr<expr>> parser::arguments( void )
{
    ...
    if ( !expect( TOK_PAREN_END ) )
        throw_runtime(
            "expected ')' to finish arguments, got '{0}'", _token.value() );
...
std::shared_ptr<expr> parser::expr_block( void )
{
    ...
    if ( last->is<assign_expr>() )
    {
        ...
        throw_runtime(
            "last expression in block cannot be an assignment (starting at {0})",
            loc.first );
    }
    list.pop_back();
...
std::unique_ptr<function> parser::get_function( void )
{
    ...
    if ( _token.type() == TOK_FUNCTION )
    {
        ...
        if ( _token.type() != TOK_IDENTIFIER )
            throw_runtime(
                "expected function name, got '{0}'", _token.value() );
    }
    else if ( _token.type() != TOK_LAMBDA )
    ...
    if ( expect( TOK_PAREN_START ) )
        arg_list( [&f]( const std::u32string &a, const std::u32string &mod ) {
            f->add_arg( a, mod );
        } );
    else
    ...
    if ( !expect( TOK_PAREN_END ) )
        throw_runtime(
            "expected ')' to end function arguments, got '{0}'",
            _token.value() );
...
void parser::arg_list(
    const std::function<void( std::u32string &, std::u32string & )> &cb )
{
    if ( arg( cb ) )
    {
        while ( _token.type() == TOK_COMMA )
        {
            ...
            if ( !arg( cb ) )
                throw_runtime(
                    "expected identifier for argument name, got '{0}'",
                    _token.value() );
        }
    }
}
    ...
    const std::function<void( std::u32string &, std::u32string & )> &cb )
{
    ...
    else if ( expect( TOK_MOD_START ) )
    {
        ...
        if ( _token.type() == TOK_IDENTIFIER )
        {
            ...
            throw_runtime(
                "expected argument name, got '{0}'", _token.value() );
        return true;
    }
...
std::shared_ptr<expr> parser::if_expr( void )
{
    ...
    if ( !expect( TOK_PAREN_START ) )
        throw_runtime(
            "expected '(' to start 'if' condition, got '{0}'", _token.value() );
    auto cond = expression();
    if ( !expect( TOK_PAREN_END ) )
        throw_runtime(
            "expected ')' to end 'if' condition, got '{0}'", _token.value() );
