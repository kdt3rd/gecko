...
public:
    assign_type(
        unifier &u, std::map<const void *, std::shared_ptr<expr>> &exprs )
    ...
    void operator()( const prefix_expr &e ) { visit( e.expression() ); }

    void operator()( const postfix_expr &e ) { visit( e.expression() ); }
    ...
    void operator()( const circumfix_expr &e ) { visit( e.expression() ); }
    ...
    void operator()( const for_expr &e ) { visit( e.result() ); }
...
environment::environment(
    std::map<std::u32string, std::shared_ptr<function>> &f )
    : _funcs( f )
{}
    ...
    if ( ty.valid() )
    {
        if ( ty.is<type_callable>() )
        {
            ...
            if ( top.size() != args.size() )
                throw_runtime(
                    "different number of arguments ({0} and {1})",
                    top.size(),
                    args.size() );
...
type environment::join( const type_primary &t1, const type_primary &t2 )
{
    ...
    switch ( t2.get_type() )
    {
        ...
        case pod_type::BOOLEAN: return t2;

        case pod_type::UNKNOWN: break;
    }
...
std::shared_ptr<expr>
environment::infer( const function &f, std::vector<type> &arg_types )
{
    precondition(
        arg_types.size() == f.args().size(),
        "mismatch for argument types {0} vs {1} for function {2}",
        arg_types.size(),
        f.args().size(),
        f.name() );
...
std::shared_ptr<expr>
environment::unify( const function &f, std::vector<type> &arg_types )
{
    precondition(
        arg_types.size() == f.args().size(),
        "mismatch for argument types {0} vs {1} for function {2}",
...
std::shared_ptr<expr>
environment::unify( const function &f, type_callable &call )
{
    precondition(
        call.size() == f.args().size(),
        "mismatch for argument types {0} vs {1} for function {2}",
        call.size(),
        f.args().size(),
        f.name() );
