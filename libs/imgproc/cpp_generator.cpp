...
cpp_generator::cpp_generator( std::ostream &cpp ) : _cpp( cpp )
{
...
std::string cpp_generator::operator()( const range_expr &e )
{
    ...
    else
    {
        if ( e.by() )
            _cpp << base::format(
                "\tfor ( auto {0} = imgproc::get_start( {1}, {3} ); {0} < imgproc::get_end( {1}, {3} ); {0} += {2} )\n",
                result,
                visit( e.start() ),
                visit( e.by() ),
                e.index() );
            ...
            _cpp << base::format(
                "\tfor ( auto {0} = imgproc::get_start( {1}, {2} ); {0} < imgproc::get_end( {1}, {2} ); {0} += 1 )\n",
                result,
                visit( e.start() ),
                e.index() );
    }
