...
{
public:
    function( std::u32string n ) : _name( std::move( n ) ) {}

    template <typename... Args>
    function( std::u32string n, const Args &... args ) : _name( std::move( n ) )
    {
        std::array<std::u32string, sizeof...( args )> list{ { args... } };
    ...
    const std::vector<std::u32string> &args( void ) const { return _args; }
