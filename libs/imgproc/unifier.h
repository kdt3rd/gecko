...
public:
    ...
    {
        static_assert(
            base::always_false<T1, T2>::value,
            "missing operator() for unifier types" );
    }
    ...
    template <typename Range> void add_constraint( const Range &range )
    {
...
private:
    ...
    void
         replace( type &x, const type_variable &target, const type &replacement );
