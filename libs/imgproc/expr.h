...
{
public:
    integer_expr( std::u32string t, uint64_t v ) : _value( t ), _i( v ) {}

    const std::u32string &value( void ) const { return _value; }

    uint64_t integer( void ) const { return _i; }
...
{
public:
    floating_expr( std::u32string t, double v ) : _value( t ), _f( v ) {}

    const std::u32string &value( void ) const { return _value; }

    double floating( void ) const { return _f; }
...
{
public:
    identifier_expr( std::u32string t ) : _value( t ) {}
...
public:
    circumfix_expr(
        const std::u32string &       op,
        const std::u32string &       cl,
        const std::shared_ptr<expr> &x )
        : _open( op ), _close( cl ), _x( x )
    {}

    const std::u32string &open( void ) const { return _open; }

    const std::u32string &close( void ) const { return _close; }
...
public:
    call_expr(
        const std::u32string &func, std::vector<std::shared_ptr<expr>> &&args )
    ...
    void set_instance( const std::shared_ptr<expr> &i ) { _instance = i; }
...
public:
    if_expr(
        const std::shared_ptr<expr> &c,
        const std::shared_ptr<expr> &t,
        const std::shared_ptr<expr> &f )
        : _condition( c ), _true( t ), _false( f )
    {}

    const std::shared_ptr<expr> &condition( void ) const { return _condition; }

    const std::shared_ptr<expr> &when_true( void ) const { return _true; }

    const std::shared_ptr<expr> &when_false( void ) const { return _false; }
...
{
public:
    range_expr( std::shared_ptr<expr> &r ) : _start( r ) {}
    ...
    void set_variable( std::u32string v ) { _variable = std::move( v ); }
...
public:
    ...
    for_expr(
        const std::vector<std::u32string> &m,
        const std::vector<range_expr> &    ranges,
        std::shared_ptr<expr>              result )
        : _mods( m ), _ranges( ranges ), _result( result )
    {}

    void add_modifier( const std::u32string &mod ) { _mods.push_back( mod ); }
    ...
    void set_result( const std::shared_ptr<expr> &r ) { _result = r; }

    const std::vector<std::u32string> &modifiers( void ) const { return _mods; }

    const std::vector<range_expr> &ranges( void ) const { return _ranges; }
    ...
    {
        precondition(
            vars.size() == _ranges.size(), "invalid number of variable names" );
...
{
public:
    lambda_expr( const std::shared_ptr<function> &f ) : _func( f ) {}
...
public:
    ...
    const type &get_type( void ) const { return _type; }
