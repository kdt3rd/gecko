...
statement::statement( statement &&s ) : _db( s._db ), _stmt( s._stmt ) {}
...
bool statement::step( void )
{
    ...
    switch ( ret )
    {
        ...
        default: throw_runtime( sqlite3_errmsg( _db ) );
    }
}

////////////////////////////////////////

size_t statement::size( void ) const { throw_not_yet(); }
...
{
    int ret = sqlite3_bind_text64(
        _stmt, i, x.c_str(), x.size(), SQLITE_TRANSIENT, SQLITE_UTF8 );
    if ( ret != SQLITE_OK )
        throw_runtime( "{0} (field {1})", sqlite3_errmsg( _db ), i );
}
...
{
    int ret = sqlite3_bind_text64(
        _stmt, i, x, std::strlen( x ), SQLITE_TRANSIENT, SQLITE_UTF8 );
    if ( ret != SQLITE_OK )
        throw_runtime( "{0} (field {1})", sqlite3_errmsg( _db ), i );
}
...
{
    int ret =
        sqlite3_bind_blob64( _stmt, i, b.data(), b.size(), SQLITE_TRANSIENT );
    if ( ret != SQLITE_OK )
        throw_runtime( "{0} (field {1})", sqlite3_errmsg( _db ), i );
}
