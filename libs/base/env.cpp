...
std::string env::get( cstring var ) const
{
...
bool env::is_set( cstring var ) const
{
    std::unique_lock<std::mutex> lk( _mutex );
    auto                         mi = _env.find( var );
    return ( mi != _env.end() );
...
void env::set( cstring var, cstring val )
{
    std::unique_lock<std::mutex> lk( _mutex );
    _env[var] = val;
    clear_cache();
...
void env::clear_except( cstring var )
{
    std::unique_lock<std::mutex>       lk( _mutex );
...
void env::clear_except( std::initializer_list<cstring> vars )
{
    std::unique_lock<std::mutex>       lk( _mutex );
    std::map<std::string, std::string> n;
    for ( auto &i: _env )
...
char **env::launch_vars( void ) const
{
    std::unique_lock<std::mutex> lk( _mutex );
