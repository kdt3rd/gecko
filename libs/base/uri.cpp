...
std::string uri::full_path( void ) const
{
    if ( _path.empty() )
        return std::string( 1, '/' );
...
uri::parse_query( char kv_sep, char arg_sep ) const
{
    ...
    for ( auto &a: args )
    {
        ...
        if ( kvPos == std::string::npos )
            throw_runtime(
                "invalid key/value argument '{0}' - no separator '{2}'",
                a,
                kv_sep );
        ...
        if ( i < str.size() )
        {
            ...
            if ( i + 1 < str.size() )
            {
                if ( std::isxdigit( str[i] ) && std::isxdigit( str[i + 1] ) )
                    ...
                    throw_runtime(
                        "invalid percent encoding at end: '{0}'", str );
            }
            else
...
void uri::parse( cstring str )
{
    ...
    else
    {
        ...
        if ( colon < question )
            path =
                str.substr( colon + 1, std::min( hash, question ) - colon - 1 );
        if ( question < hash )
        {
        ...
        if ( begins_with( path, "//" ) )
        {
            ...
            throw_runtime(
                "expected uri path to start with slash: '{0}'", str );
    }
