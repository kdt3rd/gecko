...
void variable_set::set( const std::string &v, std::string val )
{
    unset_array( v );
    unset_hash( v );
...
void variable_set::set( const std::string &v, std::vector<std::string> val )
{
    unset_simple( v );
    unset_hash( v );
...
void variable_set::set(
    const std::string &v, std::map<std::string, std::string> val )
{
    unset_simple( v );
    unset_array( v );
...
void variable_set::set( const std::string &v, size_t i, const std::string &val )
{
...
void variable_set::set(
    const std::string &v, const std::string &key, const std::string &val )
{
    auto s = _simple_vars.find( v );
    if ( s != _simple_vars.end() )
...
void variable_set::append( const std::string &v, const std::string &val )
{
    auto s = _simple_vars.find( v );
    if ( s != _simple_vars.end() )
    {
...
std::string variable_set::get( const std::string &v ) const
{
    auto s = _simple_vars.find( v );
    if ( s != _simple_vars.end() )
        return s->second;
...
std::string variable_set::get( const std::string &v, size_t i ) const
{
    auto s = _simple_vars.find( v );
    if ( s != _simple_vars.end() )
    {
...
variable_set::get( const std::string &v, const std::string &hkey ) const
{
    ...
    if ( s != _simple_vars.end() )
    {
        ...
        if ( pos != hkey.size() )
            throw_runtime(
                "Variable {0} is a simple variable, not a hash variable, expect integer index, got {1}",
                v,
                hkey );
        if ( idx == 0 )
            throw_runtime(
                "Invalid array index 0 dereferencing variable {0}", v );
        --idx;
    ...
    if ( a != _array_vars.end() )
    {
        ...
        if ( pos != hkey.size() )
            throw_runtime(
                "Variable {0} is an array, not a hash variable, expect integer index, got {1}",
                v,
                hkey );
        if ( idx == 0 )
            throw_runtime(
...
std::string variable_set::lookup( const std::string &v )
{
    if ( v.empty() )
        return std::string();
    ...
    if ( subvar != std::string::npos )
        return lookup( replace_unix_vars(
            v,
    ...
    precondition(
        apos == std::string::npos || apos < cpos,
        "Invalid assumption about array references in variable {0}",
        v );

    if ( v[0] == '#' )
    {
        precondition(
            apos == std::string::npos && cpos == std::string::npos,
            "Unable to get the length of variable {0} and retrieve and index or modify the variable at the same time",
            v );
    ...
    if ( cpos != std::string::npos )
    {
        ...
        if ( v[cpos] == ':' )
        {
            if ( cpos == ep )
                throw_runtime(
                    "Invalid variable modifier on '{0}', need argument to :",
                    v );
            ...
            else
            {
                ...
                if ( pos < offpair.size() )
                {
                    ...
                    if ( pos == 0 )
                        throw_runtime(
                            "Invalid lenght argument in variable dereference '{0}'",
                            v );
                }
                if ( vals.size() == 1 )
                {
        ...
        else if ( v[cpos] == '/' )
        {
            ...
            if ( replP == std::string::npos )
                throw_runtime(
                    "Invalid variable modifier on '{0}', need replacement to /pattern/replacement",
                    v );
            ...
            throw_logic(
                "Unhandled modification character '{0}' in '{1}'", v[cpos], v );
    }
...
void variable_set::unset( const std::string &v )
{
    unset_simple( v );
    unset_array( v );
    unset_hash( v );
...
size_t variable_set::length( const std::string &v ) const
{
    auto s = _simple_vars.find( v );
    if ( s != _simple_vars.end() )
...
bool variable_set::defined( const std::string &v ) const
{
...
std::string variable_set::substitute( const std::string &v )
{
    return replace_unix_vars(
        v, std::bind( &variable_set::lookup, this, std::placeholders::_1 ) );
}

////////////////////////////////////////

void variable_set::unset_simple( const std::string &v )
{
...
void variable_set::unset_array( const std::string &v )
{
...
void variable_set::unset_hash( const std::string &v )
{
