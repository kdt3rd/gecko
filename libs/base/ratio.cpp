...
ratio::ratio( int64_t n, int64_t d, bool s ) : _num( n ), _den( d )
{
    if ( s )
        simplify();
}
...
ratio ratio::common( const ratio &r ) const
{
    precondition(
        valid() && r.valid(),
        "unable to find common denominator for invalid ratios" );
...
std::pair<ratio, ratio> rebase( const ratio &a, const ratio &b )
{
    ...
    if ( a.denominator() == 1 )
        return std::make_pair(
            ratio( a.numerator() * b.denominator(), b.denominator(), false ),
            ratio( b.numerator(), b.denominator(), false ) );

    if ( b.denominator() == 1 )
        return std::make_pair(
            ratio( a.numerator(), a.denominator(), false ),
    ...
    // numbers growing
    return std::make_pair(
        ratio( a.numerator() * ( d / a.denominator() ), d, false ),
