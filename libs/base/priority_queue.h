...
template <typename Key> class priority_queue
{
public:
    priority_queue(
        size_t                                 size,
        const std::function<bool( Key, Key )> &comp = std::greater<Key>() )
        : _comp( comp )
    {
        if ( size > 0 )
            _heap.reserve( size );
    ...
    void set_default( Key k ) { _default = k; }
    ...
    size_t size( void ) const { return _heap.size(); }

    void reserve( size_t s ) { _heap.reserve( s ); }

    void clear( void ) { _heap.clear(); }
    ...
    void push( Key k )
    {
        if ( _initialized )
            _heap.insert(
                std::upper_bound( _heap.begin(), _heap.end(), k, _comp ), k );
        else
            _heap.push_back( k );
    }
    ...
    Key operator[]( size_t i ) const { return _heap.at( i ); }
