    ...
    , show_plus( false )
{
    ...
    precondition(
        *fmt == '{', "format specifier expected '{' (got '{0}')", *fmt );
    ...
    while ( *fmt == ',' )
    {
        ...
        switch ( *fmt )
        {
            case 'w': width = parse_number( fmt, end ); break;
            ...
            case 'p': precision = parse_number( fmt, end ); break;
            ...
            case 'a':
                ...
                switch ( *fmt )
                {
                    ...
                    default:
                        throw_runtime(
                            "expected alignment 'l' or 'r' (got '{0}')", *fmt );
                }
                ++fmt;
                break;
...
void detail::format_specifier::apply( std::ostream &out )
{
    if ( width >= 0 )
        out.width( width );
...
int detail::format_specifier::parse_number( const char *&fmt, const char *end )
{
    char start  = *fmt++;
    int  n      = 0;
    int  digits = 0;
