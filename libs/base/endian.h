...
#include <cstdint>
    ...
    ( defined( _MSC_VER ) && _MSC_VER >= 1400 ) /* Visual Studio */
#    include <cstdlib>
...
template <typename T> inline T bswap_16( T x )
{
...
template <typename T> inline T bswap_32( T x )
{
...
template <typename T> inline T bswap_64( T x )
{
...
template <typename T, size_t> struct bswap_helper
{};

template <typename T> struct bswap_helper<T, 1ul>
{
...
template <typename T> struct bswap_helper<T, 2ul>
{
    static inline T swap( T v ) { return bswap_16( v ); }
...
template <typename T> struct bswap_helper<T, 4ul>
{
    static inline constexpr T swap( T v ) { return bswap_32( v ); }
    static inline T           hosttobe( T v )
    {
        if ( is_little_endian() )
            return swap( v );
...
template <typename T> struct bswap_helper<T, 8ul>
{
    static inline T swap( T v ) { return bswap_64( v ); }
    static inline T hosttobe( T v )
    {
        if ( is_little_endian() )
            return swap( v );
...
template <typename T> inline T host2be( T c )
{
...
template <typename T> inline T host2le( T c )
{
