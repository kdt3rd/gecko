...
static_assert(
    kReadWriterBits + kReaderBits + kWriterBits <= 64,
    "Invalid number of bits for atomic store" );
...
static const uint64_t kMaxReadWriterCount =
    ( uint64_t( 1 ) << kReadWriterBits ) - 1;
...
inline uint64_t numReaders( uint64_t v )
{
    return ( v >> kReaderOffset ) & kMaxReaderCount;
}
inline uint64_t numWriters( uint64_t v )
{
    return ( v >> kWriterOffset ) & kMaxWriterCount;
}
inline uint64_t numReadWriters( uint64_t v )
{
    return ( v >> kReadWriterOffset ) & kMaxReadWriterCount;
}

inline uint64_t incReader( uint64_t v )
{
    precondition(
        numReaders( v + kOneReader ) < kMaxReaderCount,
        "Too many readers for shared_mutex" );
    return v + kOneReader;
}
...
{
    return (
        ( ( ( v >> kReadWriterOffset ) & kMaxReadWriterCount )
          << kReaderOffset ) |
        ( v & kWriterMask ) );
}
...
inline uint64_t decWriter( uint64_t v )
{
    precondition(
        numWriters( v ) > 0,
        "Attempt to decrement empty reader pool in shared_mutex" );
    return v - kOneWriter;
}
...
{
    precondition(
        numReadWriters( v + kOneReadWriter ) <= kMaxReadWriterCount,
        "Too many readers for shared_mutex" );
...
shared_mutex::shared_mutex( void ) : _state( 0UL ) {}
...
void shared_mutex::lock( void )
{
    uint64_t curState =
        _state.fetch_add( kOneWriter, std::memory_order_acquire );
...
bool shared_mutex::try_lock( void )
{
    uint64_t c = _state.load( std::memory_order_relaxed );
    return (
        c == 0UL && _state.compare_exchange_strong(
                        c, c + kOneWriter, std::memory_order_acquire ) );
}

////////////////////////////////////////

void shared_mutex::unlock( void )
{
    ...
    {
        precondition(
            numReaders( curState ) == 0,
            "Expecting 0 readers during unique write lock" );
...
void shared_mutex::lock_shared( void )
{
...
bool shared_mutex::try_lock_shared( void )
{
    uint64_t c = _state.load( std::memory_order_relaxed );
    return (
        numWriters( c ) == 0UL &&
        _state.compare_exchange_strong(
            c, c + kOneReader, std::memory_order_acquire ) );
}

////////////////////////////////////////

void shared_mutex::unlock_shared( void )
{
    uint64_t prevState =
        _state.fetch_sub( kOneReader, std::memory_order_release );
    precondition(
        numReaders( prevState ) > 0,
        "Attempt to unlock_shared a shared_mutex with no read locks" );
