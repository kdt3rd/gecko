...
template <typename ret_type> class collector_last
{
public:
    typedef ret_type collector_result;

    explicit collector_last( void ) : _last() {}
    ...
    collector_result result( void ) { return _last; }
...
template <> class collector_default<void>
{
public:
...
public:
    inline bool invoke(
        collector &c, const std::function<result( args... )> &cbf, args... a )
    {
        return c( cbf( a... ) );
    }
};
...
public:
    inline bool
    invoke( collector &c, const std::function<void( args... )> &cbf, args... a )
    {
        cbf( a... );
        return c();
    }
...
class proto_signal<ret_type( args... ), collector>
    : private collector_invocation<collector, ret_type( args... )>
{
protected:
...
private:
    ...
    public:
        ...
        ~signal_link( void ) { assert( ref_count == 0 ); }
    ...
    void ensure_ring( void )
    {
        if ( !_callback_ring )
        {
            ...
            _callback_ring
                ->incref(); // ref_count = 2, head of ring, can be deactivated but not removed
...
public:
    ...
    proto_signal( const callback &method ) : _callback_ring( nullptr )
    {
        if ( method != nullptr )
        {
            ensure_ring();
        ...
        do
        {
            if ( link->function != nullptr )
            {
                const bool continue_emission =
                    this->invoke( col, link->function, a... );
...
template <
    typename signature,
    class collector = detail::collector_default<
...
public:
    ...
    signal( const callback &method = callback() ) : proto_signal( method ) {}
...
template <
    ...
    typename ret_type,
    typename... args>
std::function<ret_type( args... )>
slot( instance &o, ret_type ( object_type::*m )( args... ) )
{
...
std::function<ret_type( args... )>
slot( object_type *o, ret_type ( object_type::*m )( args... ) )
{
    return [o, m]( args... a ) { return ( o->*m )( a... ); };
}

/// @brief Collect while callbacks return true (!0).
template <typename ret_type> class collector_while
{
public:
    typedef ret_type collector_result;
    ...
    const collector_result &result( void ) { return _result; }
...
template <typename ret_type> class collector_until
{
public:
    typedef ret_type collector_result;
...
template <typename ret_type> class collector_vector
{
public:
