...
public:
    ...
    /// @sa base::format
    template <typename... Args> void test( bool v, Args... args )
    {
        if ( v )
            success( args... );
        else
    ...
    template <typename... Args> void negative_success( Args... args )
    {
        negative_success(
            std::string( format( std::forward<Args>( args )... ) ) );
    }
    ...
    size_t success_count( void ) const { return _success.size(); }

    size_t failure_count( void ) const { return _failure.size(); }

    size_t run_count( void ) const { return _success.size() + _failure.size(); }
...
private:
    void
    run( std::map<std::string, std::function<void( void )>>::iterator &it );
