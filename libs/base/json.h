...
public:
    ...
    json( void ) { set<json_null>( json_null( nullptr ) ); }

    /// @brief JSON string constructor
    json( const json_string &x ) : _storage( x ) {}

    /// @brief JSON string constructor
    json( json_string &&x ) : _storage( std::move( x ) ) {}

    /// @brief JSON object constructor
    json( const json_object &x ) : _storage( x ) {}

    /// @brief JSON array constructor
    json( const json_array &x ) : _storage( x ) {}

    /// @brief JSON object move constructor
    json( json_object &&x ) : _storage( std::move( x ) ) {}

    /// @brief JSON array move constructor
    json( json_array &&x ) : _storage( std::move( x ) ) {}

    /// @brief JSON number constructor
    json( const json_number &x ) { set<json_number>( x ); }

    /// @brief JSON number constructor
    json( json_number &&x ) { set<json_number>( std::move( x ) ); }
    ...
    {
        set<json_number>(
            std::make_pair( static_cast<double>( x ), to_string( x ) ) );
    }
    ...
    json( json_bool x ) { set<json_bool>( x ); }

    /// @brief JSON null constructor
    json( json_null x ) { set<json_null>( x ); }

    /// @brief pass through check to storage
    template <typename X> inline bool is( void ) const
    {
        return _storage.is<X>();
    }

    inline bool valid( void ) const { return _storage.valid(); }
    ...
    template <typename X, typename... Args> void set( Args &&... args )
    {
        _storage.set<X>( std::forward<Args>( args )... );
    }

    template <typename X> X &get( void )
    {
        try
        {
        ...
        {
            throw_runtime(
                "unable to extract type '{0}' from json entry, actual type is '{1}'",
                typeid( X ).name(),
                _storage.type_name() );
        }
        catch ( ... )
    ...
    template <typename X> const X &get( void ) const
    {
        try
        {
            return _storage.get<X>();
    ...
    void clear( void ) { return _storage.clear(); }

    const char *type_name( void ) const { return _storage.type_name(); }
...
inline void operator>>( const std::string &in, json &j ) { j.parse( in ); }
