...
template <typename T>
class thread_specific_default_ctor
{
public:
	static_assert( std::is_default_constructible<T>::value, "Thread-local value must be default constructible, switch to thread_specific_null_ctor or provide your own" );
...
template <typename T>
class thread_specific_null_ctor
{
public:
...
///
template <typename T, typename D = std::default_delete<T>, typename C = thread_specific_default_ctor<T> >
class thread_ptr
{
	...
	struct thread_ptr_holder
	{
		using pointer = T *;
		using deleter = D;
		thread_ptr_holder( const deleter &d ) noexcept : _ptr( nullptr ), _del( d ) {}
		thread_ptr_holder( pointer p, const deleter &d ) noexcept : _ptr( p ), _del( d ) {}
		~thread_ptr_holder( void )
		{
			_del( _ptr );
		}
...
public:
	...
	/// NB: NOT noexcept here since we may not be able to create the TLS chunk
	inline thread_ptr( pointer p, const deleter_type &d )
		: _del( d )
	{ init( p ); }
	/// NB: NOT noexcept here since we may not be able to create the TLS chunk
	inline thread_ptr( pointer p, deleter_type &&d )
		: _del( std::move( d ) )
	{ init( p ); }
	/// NB: NOT noexcept here since we may not be able to create the TLS chunk
	inline thread_ptr( pointer p, const ctor_type &c )
		: _cons( c )
	{ init( p ); }
	/// NB: NOT noexcept here since we may not be able to create the TLS chunk
	inline thread_ptr( pointer p, ctor_type &&c )
		: _cons( std::move( c ) )
	{ init( p ); }
	/// NB: NOT noexcept here since we may not be able to create the TLS chunk
	...
	typename std::add_lvalue_reference<element_type>::type inline operator*( void ) const
	{
		return *get();
	}
	...
	explicit inline operator bool( void ) const noexcept { return get() == pointer() ? false : true; }

	inline pointer release( void ) const noexcept { return thread_holder().release(); }
	inline void reset( pointer p = pointer() ) const noexcept { thread_holder().reset( p ); }
...
private:
	void init( pointer p )
	{
		...
		if ( _tls_key == TLS_OUT_OF_INDEXES )
			throw_lasterror( "Unable to create thread local storage - out of indices" );
		...
		if ( r != 0 )
			throw_location( std::system_error( r, std::generic_category(), "Unable to create thread local storage" ) );
#endif
...
#else
	static const pthread_key_t kInvalidKey = -1;
