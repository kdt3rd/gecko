...
template <
    typename SV,
    bool = std::enable_if<
        std::is_copy_constructible<typename SV::value_type>::value ||
...
template <typename SV> struct shrink_helper<SV, true>
{
...
private:
    ...
    struct sv_impl : public alloc_type
    {
        ...
        sv_impl( void ) noexcept(
            std::is_nothrow_default_constructible<alloc_type>::value ) =
            default;
        sv_impl( apointer ptr, size_t n ) noexcept(
            std::is_nothrow_default_constructible<alloc_type>::value )
        ...
        void reset_to_small(
            apointer ptr, size_t n, size_t ls, bool dodealloc = true )
        {
            if ( dodealloc )
                this->deallocate(
                    _start, static_cast<size_type>( _storage_end - _start ) );
            _start       = ptr;
            _end         = ptr + n;
            _storage_end = ptr + ls;
        }
    ...
    using storage                       = typename std::
        aligned_storage<store_count * value_size, alignof( T )>::type;
    storage _store;

    inline apointer small_store( void )
    {
        return reinterpret_cast<apointer>( &_store );
    }
    inline capointer small_store( void ) const
    {
        return reinterpret_cast<capointer>( &_store );
    }
...
public:
    ...
    small_vector( void ) noexcept(
        ...
        : _impl( small_store(), store_count )
    {}
    small_vector( const allocator_type &a )
    ...
    small_vector( const small_vector &o )
        : _impl(
              ...
              small_store(),
              store_count,
              allocator_traits::select_on_container_copy_construction( _impl ) )
    {
    ...
    small_vector( small_vector &&o ) noexcept
    {
        if ( o.is_small() )
        {
            _impl._end = detail::uninit_copy_a(
    ...
    small_vector( small_vector &&o, const allocator_type &a ) noexcept(
        small_vector::allocator_is_always_equal() )
        : _impl( small_store(), store_count, a )
    {
        if ( o.is_small() )
        {
    ...
    small_vector(
        InputIter b, InputIter e, const allocator_type &a = allocator_type() )
        : _impl( small_store(), store_count, a )
    {
        using iter_cat =
            typename std::iterator_traits<InputIter>::iterator_category;
    ...
    small_vector &operator=( const small_vector &o )
    {
        if ( &o != this )
        {
            ...
            assign_helper(
                o.begin(), o.end(), std::random_access_iterator_tag() );
        }
        return *this;
    }
        ...
        allocator_traits::propagate_on_container_move_assignment::value )
    {
        if ( &o != this )
        {
            constexpr bool has_move_storage =
                 ...
                 !is_small() && !o.is_small() )
            {
                ...
                _impl.swap( o._impl );
                if ( allocator_traits::propagate_on_container_move_assignment::
                         value )
                    get_allocator_ref() = std::move( o.get_allocator_ref() );
                o._impl.reset_to_small(
                    o.small_store(), 0, o.store_count, false );
            }
            else
            {
    ...
    small_vector &operator=( std::initializer_list<value_type> l )
    {
        assign_helper( l.begin(), l.end(), std::random_access_iterator_tag() );
        return *this;
    }
        ...
        else if ( n > size() )
        {
            ...
            for ( size_t x = n - size(); x > 0; --x )
            {
                allocator_traits::construct(
                    _impl, std::addressof( _impl._end ), v );
                ++_impl._end;
            }
        }
        else
    ...
    {
        return size_type( _impl._end - _impl._start );
    }
    inline size_type max_size( void ) const noexcept
    {
        return allocator_traits::max_size( _impl );
    }
    ...
    {
        return size_type( _impl._storage_end - _impl._start );
    }

    inline bool empty( void ) const noexcept
    {
        return _impl._start == _impl._end;
    }
    ...
    {
        if ( n > max_size() )
            throw std::length_error( format(
                "small_vector reserve {0} beyond max_size {1}",
                n,
                max_size() ) );
        if ( capacity() < n )
        {
            const size_type old  = size();
            pointer         news = allocate_and_copy(
                n,
            ...
            if ( !is_small() )
                _impl.deallocate(
                    _impl._start,
                    static_cast<size_t>( _impl._storage_end - _impl._start ) );
            _impl._start       = news;
            _impl._end         = news + old;
    ...
    void range_check( size_type i ) const
    {
        if ( i > size() )
            throw std::out_of_range( format(
                "small_vector::range_check index {0} >= this->size() {1}",
                i,
                size() ) );
    }
#ifndef NDEBUG
    void require_nonempty( void ) const
    {
        if ( empty() )
            throw std::out_of_range(
                "small_vector::require_nonempty this is empty" );
    }
#else
...
public:
    ...
    {
        return empty() ? nullptr : _impl._start;
    }
    ...
    emplace_back( Args &&... args )
    {
        ...
        {
            allocator_traits::construct(
                _impl, _impl._end, std::forward<Args>( args )... );
            ++_impl._end;
        }
        else
    ...
    {
        return do_erase( begin() + ( p - cbegin() ) );
    }
...
private:
    ...
    size_type check_len( size_type n ) const
    {
        if ( ( max_size() - size() ) < n )
            throw std::length_error(
                "small_vector request to allocate past max_size" );
    ...
    void realloc_insert( iterator p, Args &&... args )
    {
        ...
        {
            allocator_traits::construct(
                _impl, start + nbefore, std::forward<Args>( args )... );
            e = pointer();
            e = detail::uninit_copy_a(
                std::make_move_iterator( _impl._start ),
        ...
        if ( !is_small() )
            _impl.deallocate(
                _impl._start,
                static_cast<size_type>( _impl._storage_end - _impl._start ) );
        _impl._start       = start;
        _impl._end         = e;
    ...
    struct TempValue
    {
        ...
        {
            allocator_traits::construct(
                v->_impl, ptr(), std::forward<Args>( args )... );
        }

        ~TempValue( void ) { allocator_traits::destroy( _vec->_impl, ptr() ); }
    ...
    private:
        pointer ptr( void )
        {
            return std::pointer_traits<pointer>::pointer_to( val() );
        }

        small_vector *_vec;
        using buf_type = typename std::
            aligned_storage<sizeof( value_type ), alignof( value_type )>::type;
        buf_type _buf;
    };

    template <typename Arg> void insert_helper( iterator p, Arg &&v )
    {
        allocator_traits::construct(
            _impl, _impl._end, std::move( *( _impl._end - 1 ) ) );
        ++_impl._end;
    ...
    void range_init( InpIter f, InpIter l, std::input_iterator_tag )
    {
    ...
    void range_init( FwdIter f, FwdIter l, std::forward_iterator_tag )
    {
    ...
    void assign_helper( FwdIter f, FwdIter l, std::forward_iterator_tag )
    {
        ...
        if ( n > capacity() )
        {
            ...
            if ( !is_small() )
                _impl.deallocate(
                    _impl._start, _impl._storage_end - _impl._start );
            _impl._start       = tmp;
            _impl._end         = tmp + n;
            _impl._storage_end = _impl._end;
        }
    ...
    template <typename InpIter>
    void
    range_insert( iterator p, InpIter f, InpIter l, std::input_iterator_tag )
    {
        for ( ; f != l; ++f )
        {
            p = insert( p, *f );
    ...
    template <typename FwdIter>
    void
    range_insert( iterator p, FwdIter f, FwdIter l, std::forward_iterator_tag )
    {
        if ( f == l )
            return;
        ...
        if ( nleft >= n )
        {
            ...
            if ( nafter > n )
            {
                detail::uninit_copy_a(
                    std::make_move_iterator( _impl._storage_end - n ),
                    std::make_move_iterator( _impl._end ),
                    _impl._end,
                    _impl );
                _impl._end += n;
...
inline bool operator==(
    const small_vector<T, N1, Alloc> &a, const small_vector<T, N2, Alloc> &b )
{
...
inline void
swap( base::small_vector<T, N, Alloc> &a, base::small_vector<T, N, Alloc> &b )
{
    a.swap( b );
}
