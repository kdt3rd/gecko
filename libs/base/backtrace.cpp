...
void backtrace_symbols_fd( void * const *stack, int n, int fd )
{
	...
	std::unique_ptr<uint8_t[]> buf( new uint8_t[sizeof( SYMBOL_INFO ) + 256 * sizeof( char )] );
	...
	for( int i = 0; i < n; ++i )
	{
		...
		msg << (n - i - 1) << ": " << symbol->Name << " (0x" << std::hex << std::setw(8) << std::setfill( '0' ) << symbol->Address << std::dec << '\n';
...
char **
backtrace_symbols( void * const *stack, int n )
{
	HANDLE process = GetCurrentProcess();
	SymInitialize( process, NULL, TRUE );
	std::unique_ptr<uint8_t[]> buf( new uint8_t[sizeof( SYMBOL_INFO ) + 256 * sizeof( char )] );
	...
	char *memchunk = reinterpret_cast<char *>( malloc( dataOff + sbuf.size() + 1 ) );
	if ( ! memchunk )
