...
public:
#ifdef __APPLE__
    posix_watcher( void ) : _notify_fd( kqueue() )
    {
        if ( _notify_fd == -1 )
    ...
    void move( base::fs_watch &w, base::fs_watch &neww ) override
    {
        ...
        precondition(
            found, "Unregistered watch class received in unregister" );
    }
    ...
    void unregisterWatch( base::fs_watch &w ) override
    {
        ...
        for ( auto i = _watches.begin(); i != _watches.end(); ++i )
        {
            if ( std::get<2>( i->second ) == &w )
            {
                ...
                if ( r != 0 )
                    throw_errno(
                        "Unable to remove file system watch on {0}",
                        std::get<0>( i->second ) );
                _watches.erase( i );
                found = true;
                break;
...
private:
    void watchThread( void )
    {
        ...
        while ( true )
        {
            ...
            while ( cur < nRead )
            {
                ...
                cur += inevt->len;
                if ( static_cast<size_t>( nRead - cur ) <
    ...
    std::map<int, std::tuple<base::uri, base::fs_event, base::fs_watch *>>
           _watches;
#endif
    int _notify_fd;
};

std::shared_ptr<base::fs_watcher> getWatcher( void )
{
    std::unique_lock<std::mutex> lk( theWatcherMutex );
...
uri posix_file_system::current_path( void ) const
{
...
void posix_file_system::lstat( const uri &path, struct stat *buf )
{
    precondition(
        path.scheme() == "file", "posix_file_system expected \"file\" uri" );
...
void posix_file_system::statfs( const uri &path, struct statvfs *s )
{
    precondition(
        path.scheme() == "file", "posix_file_system expected \"file\" uri" );
    std::string fpath = path.full_path();
...
uri posix_file_system::readlink( const uri &path, size_t sz )
{
    precondition(
        path.scheme() == "file", "posix_file_system expected \"file\" uri" );
    std::string fpath = path.full_path();
...
directory_iterator posix_file_system::readdir( const uri &path )
{
    ...
    std::shared_ptr<DIR> dir(
        ::opendir( fpath.c_str() ), []( DIR *d ) { ::closedir( d ); } );
    if ( !dir )
    ...
    size_t n =
        static_cast<size_t>( std::max(
            fpathconf( dirfd( dir.get() ), _PC_NAME_MAX ), long( 255 ) ) ) +
        1;
    n += offsetof( struct dirent, d_name );
    std::shared_ptr<struct dirent> dir_ent(
        reinterpret_cast<dirent *>( new char[n] ),
        []( struct dirent *d ) { delete[] reinterpret_cast<char *>( d ); } );
...
void posix_file_system::mkdir( const uri &path, mode_t mode )
{
    precondition(
        path.scheme() == "file", "posix_file_system expected \"file\" uri" );
    std::string fpath = path.full_path();
...
void posix_file_system::mkdir_all( const uri &path, mode_t mode )
{
    precondition(
        path.scheme() == "file",
        "posix_file_system expected \"file\" uri, got {0}",
        path.scheme() );
...
void posix_file_system::rmdir( const uri &path )
{
    precondition(
        path.scheme() == "file", "posix_file_system expected \"file\" uri" );
    std::string fpath = path.full_path();
...
void posix_file_system::unlink( const uri &path )
{
    precondition(
        path.scheme() == "file", "posix_file_system expected \"file\" uri" );
    std::string fpath = path.full_path();
...
void posix_file_system::symlink( const uri &curpath, const uri &newpath )
{
    precondition(
        newpath.scheme() == "file", "posix_file_system expected \"file\" uri" );
...
void posix_file_system::link( const uri &curpath, const uri &newpath )
{
    precondition(
        curpath.scheme() == "file",
        "posix_file_system expected \"file\" uri, got {0}",
...
void posix_file_system::rename( const uri &oldpath, const uri &newpath )
{
    precondition(
        oldpath.scheme() == "file", "posix_file_system expected \"file\" uri" );
    precondition(
        newpath.scheme() == "file", "posix_file_system expected \"file\" uri" );
...
posix_file_system::open_read( const base::uri &path, std::ios_base::openmode m )
{
    precondition(
        path.scheme() == "file", "posix_file_system expected \"file\" uri" );
    std::unique_ptr<unix_streambuf> sb(
        new unix_streambuf( m, path, page_size ) );
    return istream( std::move( sb ) );
}

////////////////////////////////////////

ostream posix_file_system::open_write(
...
fs_watch posix_file_system::watch(
    const base::uri &              path,
