...
#define Skein_Put64_LSB_First(dst08,src64,blocksize) memcpy(dst08,src64,blocksize)
...
/*****************************  Skein_512 ******************************/
void skein512::process( const uint8_t *blkPtr, size_t blkCnt, size_t byteCntAdd )
{
...
//        /* precompute the key schedule for this block */
        ...
        ks[8] = ks[0] ^ ks[1] ^ ks[2] ^ ks[3] ^
                ks[4] ^ ks[5] ^ ks[6] ^ ks[7] ^ 0x5555555555555555;

        ts[2] = ts[0] ^ ts[1];

        Skein_Get64_LSB_First(w,blkPtr,8); /* get input block in little-endian format */
