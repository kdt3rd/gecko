...
public:
		  ...
		  _pop_pos( 0 )
	{
		size_t N = 1<<log2size;
		if ( N < 4 )
			throw_runtime( "lock-free bounded queue of size {0} (log2 size {1}) is too small to be effective", N, log2size );
	...
	bool try_push( const value_type &data )
	{
		...
		for ( ;; )
		{
			...
			intptr_t tagdelta = static_cast<intptr_t>( curtag ) - static_cast<intptr_t>( loc );
			if ( tagdelta == 0 )
			{
				if ( _push_pos.compare_exchange_weak( loc, loc + 1, std::memory_order_relaxed ) )
					break;
			}
			else if ( tagdelta < 0 )
				return false;
	...
	bool try_pop( value_type &v )
	{
		...
		for ( ;; )
		{
			...
			intptr_t tagdelta = static_cast<intptr_t>( curtag ) - static_cast<intptr_t>( loc + 1 );
			if ( tagdelta == 0 )
			{
				// try to update to pull from the next queue entry in the ring
				if ( _pop_pos.compare_exchange_weak( loc, loc + 1, std::memory_order_relaxed ) )
					break;
			}
			else if ( tagdelta < 0 )
				return false;
		...
		to_pull->tag.store( loc + _buffer_index_mask + 1, std::memory_order_release );
...
private:
	...
	inline constexpr size_t alignment_offset = std::hardware_destructive_interference_size;
#else
