...
void process::set_pipe( bool in, bool out, bool err )
{
    ...
    if ( in )
    {
        ...
        auto bin = std::unique_ptr<base::ostream::streambuf_type>(
            new base::unix_streambuf(
                std::ios_base::out | std::ios_base::binary,
                sin.steal_write(),
                false,
                "stdin" ) );
        _stdin = std::unique_ptr<base::ostream>(
            new base::ostream( std::move( bin ) ) );
    }
    ...
    if ( out )
    {
        ...
        auto bout = std::unique_ptr<base::istream::streambuf_type>(
            new base::unix_streambuf(
                std::ios_base::in | std::ios_base::binary,
                sout.steal_read(),
                false,
                "stdout" ) );
        _stdout = std::unique_ptr<base::istream>(
            new base::istream( std::move( bout ) ) );
    }
    ...
    if ( err )
    {
        ...
        auto berr = std::unique_ptr<base::istream::streambuf_type>(
            new base::unix_streambuf(
                std::ios_base::in | std::ios_base::binary,
                serr.steal_read(),
                false,
                "stderr" ) );
        _stderr = std::unique_ptr<base::istream>(
            new base::istream( std::move( berr ) ) );
    }
#endif
}
...
void process::wait( void )
{
    std::unique_lock<std::mutex> lock( the_mutex );
    the_condition.wait(
        lock, [=]( void ) { return !this->signaled() && !this->exited(); } );
}
