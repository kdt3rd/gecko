...
template <
    typename A,
    typename B,
    class Allocator = std::allocator<std::pair<A, B>>>
...
public:
    ...
    inline const_reference_A operator[]( const value_type_B &b ) const
    {
        ...
        if ( i == end() )
            throw std::out_of_range(
                "bidirectional_map operator[] item not found" );
        return i->first;
    }
    ...
    inline iterator find( const value_type_A &a )
    {
        ...
        return std::lower_bound(
            begin(), end(), a, []( value_type &x, const value_type_A &a ) {
                return x.first == a;
            } );
    }
    inline const_iterator find( const value_type_A &a ) const
    {
        ...
        return std::lower_bound(
            ...
            a,
            []( const value_type &x, const value_type_A &a ) {
                return x.first == a;
            } );
    }
    inline iterator find( const value_type_B &b )
    {
        ...
        return std::find_if(
            begin(), end(), [&]( value_type &x ) { return x.second == b; } );
    }
    inline const_iterator find( const value_type_B &b ) const
    {
        // we aren't sorted by b so can't do log(N) search
        return std::find_if( begin(), end(), [&]( const value_type &x ) {
            return x.second == b;
        } );
    }
    ...
    inline std::pair<iterator, bool> emplace( Args &&... args )
    {
        ...
        postcondition(
            i != end() && !( v < *i ), "expect to insert and find item" );
        return std::make_pair( i, true );
    }

    inline void erase( iterator pos ) { _m.erase( pos ); }
...
inline void swap(
    base::bidirectional_map<A, B, Allocator> &a,
