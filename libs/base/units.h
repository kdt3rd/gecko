...
namespace units
{
template <typename T, typename ratio_to_meters = std::ratio<1>> class length;
...
struct convert_impl
{
    ...
    static constexpr inline out apply( const length<T, RtoM> &l )
    {
        ...
        return out( static_cast<value_type>(
            static_cast<comt>( l.count() ) * static_cast<comt>( convr::num ) /
            static_cast<comt>( convr::den ) ) );
    }
};
...
template <typename T, typename RtoM> class length
{
public:
    static_assert(
        std::is_arithmetic<T>::value,
        "length should be composed of an arithmetic value type" );
    using value_type      = T;
    ...
    template <
        ...
        typename = std::enable_if_t<
            std::is_convertible<typename std::decay<OT>::type, value_type>::
                value &&
    ...
    template <
        typename OT,
        typename other_ratio,
        typename = std::enable_if_t<
            std::is_floating_point<value_type>::value ||
            ( std::integral_constant<
                  bool,
                  1 == std::ratio_divide<other_ratio, ratio_to_meters>::den>::
                  value &&
              ( !std::is_floating_point<OT>::value ) )>>
    constexpr inline length( const length<OT, other_ratio> &o )
        : _val( convert<length>( o ).count() )
    {}
    ~length( void )          = default;
        ...
        is_length<OT>::value &&
            ( std::is_floating_point<value_type>::value ||
              ( std::integral_constant<
                    bool,
                    1 == std::ratio_divide<
                             typename OT::ratio_to_meters,
                             ratio_to_meters>::den>::value &&
                ( !std::is_floating_point<typename OT::value_type>::value ) ) ),
        OT>
    as( void ) const
    {
    ...
    explicit inline operator length<tO, rO>( void ) const
    {
        return length<tO, rO>( *this );
    }
    ...
    inline length &operator=( value_type v )
    {
        _val = v;
        return *this;
    }

    inline length &operator++( void )
    {
        ++_val;
        return *this;
    }
    inline length operator++( int )
    {
        length tmp( *this );
        ++_val;
        return tmp;
    ...
    inline length &operator+=( const length &l )
    {
        _val += l.count();
        return *this;
    }
    ...
    inline length operator+( const length &l ) const
    {
        return length( count() + l.count() );
    }
    inline length operator+( value_type v ) const
    {
...
/// per iau 2012
template <typename T>
using astronomical_units = length<T, std::ratio<149597870700, 1>>;
...
template <typename T>
using miles = length<T, std::ratio<254 * 12 * 5280, 10000>>;
template <typename T>
using leagues = length<T, std::ratio<254 * 12 * 5280 * 3, 10000>>;
...
template <typename T>
using fathoms = length<T, std::ratio<254 * 12 * 3 * 2, 10000>>;
