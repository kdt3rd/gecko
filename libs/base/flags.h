...
public:
    ...
    flags( void ) : flags( store_type( 0 ) ) {}

    flags( const std::initializer_list<enum_type> &initList )
    {
        _flags = std::accumulate(
            initList.begin(),
            initList.end(),
            store_type( 0 ),
            []( enum_type x, enum_type y ) { return x | y; } )
    }

    // Value constructor
    explicit flags( store_type value ) : _flags( value ) {}

    operator store_type() const { return _flags; }
    ...
    constexpr size_t size() const { return sizeof( enum_type ) * 8; }

    bool test( enum_type flag ) const { return ( _flags & flag ) > 0; }

    bool any( void ) const { return _flags > 0; }

    bool none( void ) const { return flags == 0; }
...
{
    return flags<Enum>(
        flags<Enum>::store_type( lhs ) & flags<Enum>::store_type( rhs ) );
}
...
operator<<( std::basic_ostream<charT, traits> &os, const flags<Enum> &flagSet )
{
