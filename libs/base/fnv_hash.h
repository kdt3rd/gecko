...
struct fnv1a_impl<uint32_t>
{
	...
	static inline constexpr uint32_t add_n( uint32_t h, const uint8_t *v ) noexcept
	{
		return N == 0 ? h : add( add_n<(N > 0 ? N - 1 : N)>( h, v ), v[N - 1] );
	}
...
struct fnv1a_impl<uint64_t>
{
	...
	template <std::size_t N>
	static inline constexpr uint64_t add_n( uint64_t h, const uint8_t *v ) noexcept
	{
		return N == 0 ? h : add( add_n<(N > 0 ? N - 1 : N)>( h, v ), v[N - 1] );
	}
...
template <typename T, typename = typename std::enable_if<std::is_integral<T>::value &&
														 ...
														 (sizeof(T) == 4 || sizeof(T) == 8), T>::type>
class fnv1a
{
	using hash_type = typename std::conditional<sizeof(T) == 4, uint32_t,
...
public:
	...
	template <size_t N>
	inline void add( const uint8_t *v )
	{
	...
	constexpr value_type operator()( void ) const { return static_cast<value_type>( _hash ); }
...
fnv1a<T> &operator <<( fnv1a<T> &h, V &v )
{
	
	h.template add<sizeof(V)>( reinterpret_cast<const uint8_t *>( std::addressof( v ) ) );
	return h;
}

template < typename T, typename charT, typename traitsT = std::char_traits<charT>, typename allocT = std::allocator<charT> >
fnv1a<T> &operator <<( fnv1a<T> &h, const std::basic_string<charT, traitsT, allocT> &v )
{
...
template < typename T, typename charT, typename traitsT = std::char_traits<charT> >
...
template <typename T>
inline fnv1a<T> &operator <<( fnv1a<T> &h, const char *s )
{
