...
{
    static_assert(
        sizeof...( Extractors ) == sizeof...( Args ),
        "missing argument binders" );
...
callable( Functor f, Extractors &&... e )
{
    return callable(
        f,
        make_int_sequence<sizeof...( Extractors )>{},
        std::forward<Extractors>( e )... );
}
...
inline R dispatch_help(
    const std::function<R( FuncArgs... )> &f,
...
template <typename R, typename... FuncArgs, typename TupleType, size_t... S>
inline R dispatch_help(
    R ( *f )( FuncArgs... ), const TupleType &args, const sequence<S...> & )
{
    return f( std::get<S>( args )... );
}

template <typename... FuncArgs, typename TupleType, size_t... S>
inline void dispatch_help(
    const std::function<void( FuncArgs... )> &f,
    const TupleType &                         args,
    const sequence<S...> & )
{
    f( std::get<S>( args )... );
...
template <typename... FuncArgs, typename TupleType, size_t... S>
inline void dispatch_help(
    void ( *f )( FuncArgs... ), const TupleType &args, const sequence<S...> & )
{
    f( std::get<S>( args )... );
}
...
inline R call_func_from_tuple(
    const std::function<R( FuncArgs... )> &f, const TupleType &args )
{
    static_assert(
        sizeof...( FuncArgs ) == std::tuple_size<TupleType>::value,
        "Mis-match of function argument count to arguments provided" );
    return priv::dispatch_help(
        f, args, gen_sequence<sizeof...( FuncArgs )>{} );
}

template <typename... FuncArgs, typename TupleType>
inline void call_func_from_tuple(
    const std::function<void( FuncArgs... )> &f, const TupleType &args )
{
    static_assert(
        sizeof...( FuncArgs ) == std::tuple_size<TupleType>::value,
        "Mis-match of function argument count to arguments provided" );
...
template <typename R, typename... FuncArgs, typename TupleType>
inline R call_func_from_tuple( R ( *f )( FuncArgs... ), const TupleType &args )
{
    static_assert(
        sizeof...( FuncArgs ) == std::tuple_size<TupleType>::value,
        "Mis-match of function argument count to arguments provided" );
