...
public:
    ...
    ~base_streambuf( void ) override { this->close(); }
...
#endif
        _open_mode(
            base::exchange( b._open_mode, std::ios_base::openmode( 0 ) ) )
...
protected:
    ...
    virtual off_type bytes_avail( void ) { return off_type( -1 ); }
...
protected:
    ...
    virtual int_type underflow( void ) override
    {
        ...
        if ( _open_mode & std::ios_base::in )
        {
            ...
            if ( check_facet( _codecvt_cache ).always_noconv() )
            {
                ilen = read(
                    this->eback(),
                    static_cast<size_t>( buflen ) * sizeof( char_type ) );
                gotEOF = ( ilen == 0 );
            }
            else
            {
                ...
                if ( _conv_buf_sz < blen )
                {
                    std::unique_ptr<char[]> buf(
                        new char[static_cast<size_t>( blen )] );
                    if ( rem )
                        std::memcpy(
                            buf.get(), _conv_next, static_cast<size_t>( rem ) );
                    ...
                    std::memmove(
                        _conv_buf, _conv_next, static_cast<size_t>( rem ) );
                ...
                do
                {
                    ...
                    {
                        precondition(
                            ( _conv_end - _conv_buf + rlen ) <= _conv_buf_sz,
                            "codecvt::max_length() is not valid" );
                        std::streamsize elen =
                            read( _conv_end, static_cast<size_t>( rlen ) );
                        if ( elen == 0 )
                            gotEOF = true;
                        else if ( elen == -1 )
                            break;
            ...
            else if ( gotEOF )
            {
                ...
                if ( r == std::codecvt_base::partial )
                    throw std::runtime_error(
                        "Incomplete character code in stream" );
            }
    ...
    virtual std::streamsize
    xsgetn( char_type *s, std::streamsize count ) override
    {
        std::streamsize ret = 0;
        if ( _pback_live )
        {
             ...
             ( _open_mode & std::ios_base::in ) )
        {
            ...
            {
                traits_type::copy(
                    s, this->gptr(), static_cast<size_t>( avail ) );
                s += avail;
                this->setg(
                    this->eback(), this->gptr() + avail, this->egptr() );
                ret += avail;
                count -= avail;
            }
            ...
            {
                nread = this->read(
                    s, static_cast<size_t>( count ) * sizeof( char_type ) );
                if ( nread == -1 )
    ...
    xsputn( const char_type *s, std::streamsize count ) override
    {
             ...
             !_reading )
        {
            ...
            if ( count >= lim )
            {
                ...
                const std::streamsize bufCount =
                    ( this->pptr() - this->pbase() ) * charSize;
                const std::streamsize countOut = count * charSize;
                ret                            = this->writev(
                    this->pbase(),
                    static_cast<size_t>( bufCount ),
                    s,
                    static_cast<size_t>( countOut ) );
    ...
    virtual int_type overflow( int_type ch = traits_type::eof() ) override
    {
             ...
             _open_mode & std::ios_base::app )
        {
            if ( _reading )
            {
                ...
                if ( seek_priv( gptr_off, std::ios_base::cur, _state_last ) ==
                     pos_type( off_type( -1 ) ) )
                    return ret;
            }
            ...
            if ( this->pbase() < this->pptr() )
            {
                ...
                if ( convertExternal(
                         this->pbase(), this->pptr() - this->pbase() ) )
                {
                    update_base_buffer_pointers( 0 );
    ...
    virtual int_type pbackfail( int_type c = traits_type::eof() ) override
    {
        ...
        if ( _open_mode & std::ios_base::in )
        {
            ...
            if ( this->eback() < this->gptr() )
            {
                ...
                this->seekoff( -1, std::ios_base::cur ) !=
                pos_type( off_type( -1 ) ) )
            {
                tmpC = this->underflow();
...
protected:
    ...
    inline bool convertExternal( char_type *ibuf, std::streamsize ilen )
    {
        ...
        {
            elen = write(
                ibuf, static_cast<size_t>( ilen ) * sizeof( char_type ) );
            if ( elen >= 0 )
        ...
        else
        {
            ...
            auto             r = _codecvt_cache->out(
                _state_cur, ibuf, ibuf + ilen, iend, buf, buf + blen, bend );
    ...
    pos_type seek_priv(
        off_type off, std::ios_base::seekdir dir, codecvt_state_type state )
    {
        pos_type ret = pos_type( off_type( -1 ) );
        if ( terminate_output() )
        {
    ...
    bool terminate_output( void )
    {
        ...
        if ( _writing && !check_facet( _codecvt_cache ).always_noconv() &&
             isvalid )
        {
            ...
            {
                char *next;
                r = _codecvt_cache->unshift(
                    _state_cur, buf, buf + blen, next );
                if ( r == std::codecvt_base::error )
                    isvalid = false;
                else if (
                    ...
                    r == std::codecvt_base::partial )
                {
                    ...
                    if ( ilen > 0 )
                    {
                        const std::streamsize elen =
                            write( buf, static_cast<size_t>( ilen ) );
                        if ( elen != ilen )
                            isvalid = false;
                    }
                }
