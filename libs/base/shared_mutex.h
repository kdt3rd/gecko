...
public:
	using mutex_type = M;
	explicit shared_lock_guard( mutex_type &m ) : _m( m )
	{
		_m.lock_shared();
	}
	shared_lock_guard( mutex_type &m, std::adopt_lock_t ) : _m( m )
	{}
	~shared_lock_guard( void )
	{
		_m.unlock_shared();
	}
...
public:
	...
	inline shared_lock( shared_lock &&o ) noexcept
		: shared_lock()
	{
		swap( o );
	}
	...
	shared_lock( mutex_type &m, std::defer_lock_t ) noexcept : _ptr( &m ), _owns( false ) {}
	shared_lock( mutex_type &m, std::try_to_lock_t ) : _ptr( &m ), _owns( m.try_lock_shared() ) {}
