...
auto expand_call( Function &&f, Tuple &&params, sequence<S...> )
    -> decltype( f( std::get<S>( std::forward<Tuple>( params ) )... ) )
{
    return std::forward<Function>( f )(
        std::get<S>( std::forward<Tuple>( params ) )... );
}
...
auto apply( Function &f, const std::tuple<Args...> &params )
    -> decltype( detail::expand_call(
        f, params, typename base::gen_sequence<sizeof...( Args )>::type() ) )
{
    return detail::expand_call(
        f, params, typename base::gen_sequence<sizeof...( Args )>::type() );
}
...
std::tuple<typename std::tuple_element<N, T>::type...>
select_tuple( const T &t, sequence<N...> )
{
...
template <std::size_t I = 0, typename Func, typename... Tp>
    ...
    I<sizeof...( Tp ), void>::type for_each( std::tuple<Tp...> &t, Func &&f )
{
