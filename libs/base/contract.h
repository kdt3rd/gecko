...
public:
    ...
    location_exception &
    operator=( location_exception &&e ) noexcept( true ) = default;

    /// @brief Get the source file location.
    const char *file( void ) const { return _file; }
...
void print_exception(
    std::ostream &out, const std::exception &e, int level = 0 );
...
public:
    ...
    precondition_error &
    operator=( precondition_error &&e ) noexcept( true ) = default;
...
public:
    ...
    postcondition_error &
    operator=( postcondition_error &&e ) noexcept( true ) = default;
    using std::logic_error::logic_error;
};
...
throw_add_location_impl( const char *f, int l, Args &&... a )
{
    try
    {
        std::throw_with_nested( location_exception( f, l ) );
    }
    catch ( ... )
    {
        std::throw_with_nested(
            std::runtime_error( format( std::forward<Args>( a )... ) ) );
    }
}
...
#define throw_location( exc )                                                  \
    ::base::detail::throw_location_impl( exc, __FILE__, __LINE__ )
...
#define throw_add( ... )                                                       \
    std::throw_with_nested(                                                    \
        std::runtime_error( ::base::format( __VA_ARGS__ ) ) );
...
#    define throw_lasterror( ... )                                             \
        throw_location( std::system_error(                                     \
            ...
            ::base::format( __VA_ARGS__ ) ) )
/// @brief Throw a errno error using format
/// @sa base::format
/// windows puts errno errors in the generic category
#    define throw_errno( ... )                                                 \
        throw_location( std::system_error(                                     \
            errno, std::generic_category(), ::base::format( __VA_ARGS__ ) ) )
#else
/// @brief Throw a errno error using format
/// @sa base::format
#    define throw_errno( ... )                                                 \
        throw_location( std::system_error(                                     \
            errno, std::system_category(), ::base::format( __VA_ARGS__ ) ) )
#endif

/// @brief If check is not true, throw a precondition_error
#define precondition( check, ... )                                             \
    ( GK_LIKELY( static_cast<bool>( check ) )                                  \
          ? void( 0 )                                                          \
          : throw_location( ::base::precondition_error(                        \
                ::base::format( __VA_ARGS__ ) ) ) )
...
#define postcondition( check, ... )                                            \
    ( GK_LIKELY( static_cast<bool>( check ) )                                  \
          ? void( 0 )                                                          \
          : throw_location( ::base::postcondition_error(                       \
                ::base::format( __VA_ARGS__ ) ) ) )
...
#define logic_check( check, ... )                                              \
    ( GK_LIKELY( static_cast<bool>( check ) )                                  \
          ? void( 0 )                                                          \
          : throw_location(                                                    \
                std::logic_error( ::base::format( __VA_ARGS__ ) ) ) )
