...
pack::pack( int w, int h, bool allowFlipped ) { reset( w, h, allowFlipped ); }
...
void pack::reset( int w, int h, bool allowFlipped )
{
    _width        = w;
    _height       = h;
...
pack::area pack::insert( int aw, int ah )
{
...
bool pack::fits( size_t idx, int w, int h, int &y, int &wastedArea ) const
{
...
pack::find_min_waste( int aw, int ah, int &bestH, int &bestWA, size_t &idx )
{
    ...
    for ( size_t i = 0; i < _skyLine.size(); ++i )
    {
        ...
        {
            if ( wastedArea < bestWA ||
                 ( wastedArea == bestWA && ( y + ah ) < bestH ) )
            {
                bestH  = y + ah;
                bestWA = wastedArea;
                idx    = i;
...
void pack::add( size_t idx, const area &a )
{
    add_waste( idx, a.width, a.height, a.y );

    _skyLine.emplace(
        _skyLine.begin() + long( idx ), a.x, a.y + a.height, a.width );
...
pack::area pack::waste_insert( int aw, int ah )
{
    size_t idx = size_t( -1 );
    area   retval;
...
void pack::add_waste( size_t idx, int w, int /*h*/, int y )
{
    const int rectLeft  = _skyLine[idx].x;
    const int rectRight = rectLeft + w;
    ...
    for ( ; idx < _skyLine.size() && _skyLine[idx].x < rectRight; ++idx )
    {
        ...
        _freeRects.emplace_back(
            leftSide, cur.y, rightSide - leftSide, y - cur.y );
    }
}
