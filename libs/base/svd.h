...
template <typename T> struct svd
{
    static_assert(
        ...
        "svd requires arithmetic type for float_type" );
    ...
    /// values
    inline bool
    init( const grid_type &a, float_type stable_eps = float_type( 1.0e-8 ) )
    {
        _u = a;
    ...
    inline bool
    init( grid_type &&a, float_type stable_eps = float_type( 1.0e-8 ) )
    {
        _u = std::move( a );
        return stable_decomp( stable_eps );
    }
...
private:
    ...
    static inline size_t cols( const grid_type &a )
    {
        return a.empty() ? size_t( 0 ) : a.front().size();
    }
...
template <typename T> void svd<T>::decomp( void )
{
    size_t n = cols( _u );
    size_t m = rows( _u );
    ...
    for ( size_t i = 0; i < n; ++i )
    {
        ...
        anorm = std::max( anorm, ( std::abs( _w[i] ) + std::abs( rv1[i] ) ) );
    }
    ...
    for ( size_t k = n - 1; k < n; --k )
    {
        for ( size_t its = 0; its < 30; ++its )
        {
            ...
            for ( l = k; l <= k; --l )
            {
                ...
                if ( l == 0 ||
                     std::abs( rv1[l] ) <=
                         std::numeric_limits<float_type>::epsilon() * anorm )
                {
                    flag = false;
                    break;
                }
            ...
            {
                throw_runtime(
                    "No convergence after 30 SVD decomp iterations" );
                break;
            }
...
template <typename T> bool svd<T>::stable_decomp( float_type eps )
{
    size_t     N      = cols( _u );
...
template <typename T> void svd<T>::solve( const col_type &b, col_type &x ) const
{
    size_t n = cols( _u );
    size_t m = rows( _u );
    if ( n == 0 || m == 0 )
