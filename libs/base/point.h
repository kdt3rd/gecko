...
public:
    static_assert(
        has_arithmetic_ops<T>::value,
        "point should be composed of an arithmetic value type" );
    ...
    explicit inline point( Args &&... p ) : _v{ std::forward<Args>( p )... }
    {
        static_assert(
            sizeof...( Args ) == N,
            "expect to initialize point with correct number of coords" );
    }

    /// @brief construct a point with a differently typed point.
    ///
    /// Requires explicit construction to avoid blind conversion
    template <typename U> explicit inline point( const point<U, N> &o )
    {
        std::copy( o.begin(), o.end(), std::begin( _v ) );
    }

    template <typename U, size_t M> inline point( const point<U, M> &o )
    {
        static_assert(
            M <= N,
            "expect a compatible point that will result in no data loss" );
        std::copy( o.begin(), o.end(), std::begin( _v ) );
    ...
    template <typename U> explicit inline operator point<U, N>() const
    {
        return point<U, N>( *this );
    }

    coord_type &                       operator[]( size_t i ) { return _v[i]; }
    constexpr inline const coord_type &operator[]( size_t i ) const
    {
        return _v[i];
    }

    coord_type &at( size_t i )
    {
        precondition( i < N, "invalid offset {0}", i );
        return _v[i];
    }
    const coord_type &at( size_t i ) const
    {
        precondition( i < N, "invalid offset {0}", i );
        return _v[i];
    }
    ...
    template <typename... Args> void set( Args &&... v )
    {
        static_assert(
            sizeof...( Args ) == N,
            "expect to initialize point with correct number of coords" );
    ...
    template <typename... Args> void move_by( Args &&... v )
    {
        static_assert(
            sizeof...( Args ) == N,
            "expect to initialize point with correct number of coords" );
    ...
    template <typename U> inline point &operator*=( U &&v )
    {
    ...
    /// @brief Scale point by v.
    template <typename U> point operator*( U &&v ) const
    {
        point r = *this;
        r *= std::forward<U>( v );
        return r;
...
private:
    template <typename A> inline void setN( size_t i, A &&v )
    {
...
template <typename T, size_t N> inline point<T, N> abs( const point<T, N> &a )
{
    point<T, N> r;
    for ( size_t i = 0; i != N; ++i )
        r[i] = std::abs( a[i] );
...
template <typename T, size_t N> inline point<T, N> ceil( const point<T, N> &a )
{
    point<T, N> r;
    for ( size_t i = 0; i != N; ++i )
        r[i] = std::ceil( a[i] );
...
template <typename T, size_t N> inline point<T, N> floor( const point<T, N> &a )
{
    point<T, N> r;
    for ( size_t i = 0; i != N; ++i )
        r[i] = std::floor( a[i] );
...
template <typename T, size_t N>
inline T signed_area(
    const point<T, 2> &p1, const point<T, 2> &p2, const point<T, 2> &p3 )
{
    static_assert(
        std::is_signed<T>::value,
        "point coordinate type should be a signed value" );
    T a = p1[0] * ( p2[1] - p3[1] );
    T b = p2[0] * ( p3[1] - p1[1] );
    T c = p3[0] * ( p1[1] - p2[1] );
    return a + b + c;
...
template <typename T, size_t N>
inline T
area( const point<T, 2> &p1, const point<T, 2> &p2, const point<T, 2> &p3 )
{
...
template <typename T, size_t N>
inline point_orientation
turn( const point<T, 2> &p1, const point<T, 2> &p2, const point<T, 2> &p3 )
{
    T a = signed_area( p1, p2, p3 );
    return (
        ( a > T( 0 ) ) ? point_orientation::COUNTERCLOCKWISE
                       : ( ( a < T( 0 ) ) ? point_orientation::CLOCKWISE
...
inline point<T, 2> polar( F1 r, F2 a )
{
    static_assert(
        ...
        "polar requires floating point type" );
    return point<T, 2>(
        static_cast<T>( r * static_cast<F1>( std::cos( a ) ) ),
