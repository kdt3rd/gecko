...
std::string cmd_line::option::name( void ) const
{
    if ( _long.empty() )
    {
        if ( _short != 0 )
...
bool cmd_line::option::match( const std::string &o )
{
    if ( o.empty() )
        return false;
...
bool cmd_line::option::is_non_option( void )
{
...
{
    auto helpfunc =
        [=]( option &, size_t &, const std::vector<char *> & ) -> bool {
...
{
    auto verfunc =
        [name, ver]( option &, size_t &, const std::vector<char *> & ) -> bool {
...
const cmd_line::option &cmd_line::operator[]( const char *n ) const
{
...
cmd_line::option &cmd_line::operator[]( const char *n )
{
...
void cmd_line::parse( int argc, char *argv[] )
{
...
void cmd_line::parse( const std::vector<char *> &args )
{
    size_t current = 0;
...
std::string cmd_line::simple_usage( void ) const
{
...
bool cmd_line::args( option &opt, size_t &idx, const std::vector<char *> &args )
{
    if ( !opt.is_non_option() )
        ++idx;
...
bool cmd_line::multi(
    option &opt, size_t &idx, const std::vector<char *> &args )
{
    if ( !opt.is_non_option() )
        ++idx;
...
bool cmd_line::counted( option &opt, size_t &idx, const std::vector<char *> & )
{
    precondition(
        !opt.is_non_option(), "unnamed option not allowed as counted flag" );
...
std::ostream &operator<<( std::ostream &out, const cmd_line &cmdline )
{
