...
template <typename Number> class intervals
{
private:
...
public:
    ...
    size_t size( void ) const { return _ranges.size(); }
    ...
    auto end( void ) -> decltype( _ranges.end() ) { return _ranges.end(); }

    template <typename... Types> void insert( Number x, Types... rest )
    {
