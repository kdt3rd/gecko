...
{
public:
    iterator_range( It b, It e ) : _begin( b ), _end( e ) {}

    It begin( void ) const { return _begin; }

    It end( void ) const { return _end; }

    size_t size( void ) const { return _end - _begin; }

    typename It::value_type &operator[]( int i ) { return ( *( _begin + i ) ); }
...
template <typename It> iterator_range<It> range( It b, It e )
{
...
public:
    ...
    iterator_type base( void ) const { return _iterator; }

    size_type offset( void ) const { return _n; }

    reference operator*(void)const { return ( *_iterator )[_n]; }
...
inline bool operator==(
    const column_iterator<I, T, S> &a, const column_iterator<I, T, S> &b )
{
...
column( C &c, typename C::size_type n )
{
    typedef column_iterator<
        typename C::iterator,
        typename C::value_type::value_type,
        typename C::size_type>
        iter_type;
    return iterator_range<iter_type>(
        iter_type( c.begin(), n ), iter_type( c.end(), n ) );
}

////////////////////////////////////////

/// @brief Construct a row iterator
template <typename C> iterator_range<typename C::iterator> rows( C &c )
{
...
template <typename C>
std::vector<iterator_range<column_iterator<
    typename C::iterator,
    typename C::value_type::value_type,
    typename C::size_type>>>
columns( C &c )
{
    ...
    if ( !c.empty() )
    {
        ...
        {
            ranges.emplace_back(
                iter_type( c.begin(), i ), iter_type( c.end(), i ) );
        }
    }
