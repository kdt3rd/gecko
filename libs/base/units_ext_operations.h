...
#    error                                                                     \
        "Do not include units_ext_operations.h by itself, meant as a helper for units.h"
#endif
...
namespace detail
{
template <typename a_length, typename b_length> struct unit_math
{
    ...
    using convr   = typename std::ratio_divide<
        typename a_length::ratio_to_meters,
    ...
    static constexpr inline result_type
    apply( const a_length &a, const b_length &b, Operator f )
    {
        return result_type( std::forward<Operator>( f )(
            convert<result_type>( a ).count(),
...
{
    static_assert(
        !std::is_unsigned<tA>::value,
        "unary negate should receive a signed value type" );
...
struct compare_impl
{
    ...
    static constexpr inline bool apply(
        const length<aT, aRtoM> &a, const length<bT, bRtoM> &b, CompareFunc f )
    {
        return std::forward<CompareFunc>( f )(
            static_cast<comt>( a.count() ),
            static_cast<comt>( b.count() ) * static_cast<comt>( convr::num ) /
...
struct compare_impl<convr, comt, CompareFunc, true, true>
{
    ...
    {
        return std::forward<CompareFunc>( f )(
            static_cast<comt>( a.count() ), static_cast<comt>( b.count() ) );
    }
};
...
struct compare_impl<convr, comt, CompareFunc, true, false>
{
        ...
        const length<aT, aRtoM> &a, const length<bT, bRtoM> &b, CompareFunc f )
    {
        return std::forward<CompareFunc>( f )(
            static_cast<comt>( a.count() ),
            static_cast<comt>( b.count() ) / static_cast<comt>( convr::den ) );
    }
};
...
compare( const a_length &a, const b_length &b, CompareFunc f )
{
    using a_value = typename a_length::value_type;
    using b_value = typename b_length::value_type;
    using convr   = typename std::ratio_divide<
...
inline std::enable_if_t<
    !std::is_floating_point<tA>::value && !std::is_integral<tA>::value,
    length<tA, rA>>
round( const length<tA, rA> &a )
{
    return length<tA, rA>( std::round( a.count() ) );
...
template <typename tA, typename rA>
inline length<tA, rA>
distance_squared( const length<tA, rA> &a, const length<tA, rA> &b )
{
    static_assert(
        std::is_signed<tA>::value,
        "expect signed type value for length computing distance" );
...
template <typename tA, typename rA>
inline length<tA, rA>
distance( const length<tA, rA> &a, const length<tA, rA> &b )
{
    static_assert(
        std::is_floating_point<tA>::value,
        "expect floating point value for computing distance" );
    tA d = a.count() - b.count();
...
template <typename T, typename R>
inline base::units::length<long long, R>
llround( const base::units::length<T, R> &a )
{
