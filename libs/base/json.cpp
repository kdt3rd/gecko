...
namespace
{
const base::json &error_obj( void )
{
...
void json::dump( void ) const { std::cout << *this << std::endl; }

////////////////////////////////////////

void json::parse_value(
    ...
    int &                        line )
{
    ...
    switch ( *it )
    {
        case '[': parse_array( it, end, line ); break;
    ...
    int &                        line )
{
    ...
    if ( *it != '[' )
        throw_runtime(
            "invalid json value, expecting array open, at line {0}", line );
    ++it;
    ...
    while ( it != end )
    {
        ...
        if ( *it != ',' )
            throw_runtime(
                "expected ',' or ']' in json array at line {0}", line );
        ++it;
    }
    ...
    int &                        line )
{
    ...
    if ( *it != '{' )
        throw_runtime(
            "invalid json value, expecting object open, at line {0}", line );
    ++it;
    ...
    if ( *it != '}' )
    {
        while ( it != end )
        {
            ...
            if ( *it != ':' )
                throw_runtime(
                    "expected ':' in json object at line {0} (got {1})",
                    line,
                    *it );
            ++it;
            skip_whitespace( it, end, line );
            ...
            if ( *it != ',' )
                throw_runtime(
                    "expected ',' or '}' in json object at line {0} (got {1})",
                    line,
                    *it );
            ++it;
            skip_whitespace( it, end, line );
    ...
    int &                        line )
{
    ...
    if ( *it != '"' )
        throw_runtime(
            "invalid json value, expecting string open but got {1}, at line {0}",
            line,
            *it );
    ++it;
            ...
            switch ( c )
            {
                ...
                case 'u':
                {
                    ...
                    for ( size_t i = 0; i < 4; ++i )
                    {
                        ...
                        if ( it == end )
                            throw_runtime(
                                "unterminated unicode at end of string" );
    ...
    int &                        line )
{
    ...
    while ( it != end && *next != '\0' )
    {
        if ( *it != *next )
            throw_runtime(
                "invalid json value validating boolean true at line {0}",
                line );
        ++next;
        ++it;
    }
    ...
    if ( *next != '\0' )
        throw_runtime(
            "invalid json, extra chars validating boolean true value, at line {0}",
            line );
    ...
    int &                        line )
{
    ...
    while ( it != end && *next != '\0' )
    {
        if ( *it != *next )
            throw_runtime(
                "invalid json value validating boolean false at line {0}",
                line );
        ++next;
        ++it;
    }
    ...
    if ( *next != '\0' )
        throw_runtime(
            "invalid json value, extra values validating boolean false, at line {0}",
            line );
    ...
    int &                        line )
{
    ...
    while ( it != end && *next != '\0' )
    {
        if ( *it != *next )
            throw_runtime(
                "invalid json value validating null at line {0}", line );
        ++next;
        ++it;
    }
    ...
    if ( *next != '\0' )
        throw_runtime(
            "invalid json value, extra values validating null, at line {0}",
            line );
    ...
    int &                        line )
{
    ...
    if ( n.empty() )
        throw_runtime(
            "invalid json value parsing number at line {0} (character {1})",
            line,
            *it );
        ...
        for ( auto c: j.get<json_string>() )
        {
            switch ( c )
            {
                ...
                default:
                    if ( std::iscntrl( c ) )
                        out << format(
                            "\\u{0,w4,b16,f0,ar}", static_cast<int>( c ) );
                    else
                        out << c;
                    break;
            }
