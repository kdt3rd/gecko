...
template <typename... Ts> class variant_helper;

template <typename F, typename... Ts> class variant_helper<F, Ts...>
{
public:
    ...
    template <typename T> static inline constexpr bool is_valid( void ) noexcept
    {
        return std::is_base_of<F, T>::value || std::is_same<F, T>::value ||
               variant_helper<Ts...>::template is_valid<T>();
    }
...
template <typename... Ts> class visitor_helper;
...
public:
    ...
    static inline Result visit( Visitor &v, const Variant &x )
    {
        if ( x.template is<T1>() )
            ...
            return visitor_helper<T2, Ts...>::
                template visit<Result, Visitor, Variant>( v, x );
    }
...
template <typename T> class visitor_helper<T>
{
public:
    template <typename Result, typename Visitor, typename Variant>
    static inline Result visit( Visitor &v, const Variant &x )
...
public:
    visitor_multi(
        Visitor &v, Tuple &&t, const V2 &v2, const Variants &... variants )
        : _realv( v ), _values( std::move( t ) ), _rest( v2, variants... )
    {}

    template <typename Value> Result operator()( const Value &v )
    {
        auto t = std::tuple_cat( _values, std::tie( v ) );
        visitor_multi<Result, Visitor, decltype( t ), V2, Variants...> newv(
            _realv, t, tuple_tail( _rest ) );
...
public:
    ...
    variant( const variant<Ts...> &old ) : _type_id( old._type_id )
    {
        Helper::copy( old._type_id, &old._data, &_data );
    }

    /// @brief Move constructor
    variant( variant<Ts...> &&old ) : _type_id( old._type_id )
    {
    ...
    template <
        typename T,
        typename base::enable_if_any<
            int,
            std::is_base_of<Ts, typename std::remove_const<T>::type>::
                value...>::type * = nullptr>
    variant( T &&t )
    {
        set<T>( std::move( t ) );
    ...
    bool valid( void ) const { return _type_id != invalid_type(); }
    ...
    template <typename T, typename... Args> void set( Args &&... args )
    {
        ...
        static_assert(
            Helper::template is_valid<TT>(), "invalid variant type" );
...
{
    return detail::visitor_helper<
        Ts...>::template visit<Result, Visitor, base::variant<Ts...>>( v, x );
}
...
template <
    ...
    typename... Variants>
inline Result visit(
    Visitor &v, const Variant1 &x, const Variant2 &y, const Variants &... vs )
{
    detail::visitor_multi<
        Result,
        Visitor,
        std::tuple<>,
        Variant1,
