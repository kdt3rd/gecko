...
{
template <size_t a> struct aligner
{
    template <typename T> static inline T *calc( void *ptr )
    {
...
public:
    ~concurrent_mem_pool( void ) { clear(); }
    ...
    template <typename T> T *get( size_t n )
    {
        ...
        if ( block::too_large( n ) )
        {
            std::unique_ptr<uint8_t[]> data{
                new uint8_t[n * sz + sizeof( big_block )]
            };
...
private:
    ...
    struct block : lock_free_list_node<block>
    {
        static constexpr size_t kDataSize =
            chunk_size - ( sizeof( lock_free_list_node<block> ) +
                           sizeof( std::atomic<size_t> ) );

        static constexpr bool too_large( size_t n )
        {
            return n * sz >= kDataSize;
        }
    ...
    static_assert(
        sizeof( block ) == chunk_size,
        "Invalid data size / alignment computation" );
    ...
    template <typename T> inline T *finish( block *b, bool full, void *ptr )
    {
        return_block( b, full );
        return reinterpret_cast<T *>( aligner::calc( ptr ) );
    }
