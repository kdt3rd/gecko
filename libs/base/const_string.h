...
template <class charT, typename traitsT> class const_string
{
    template <typename S, bool = false> struct ctor_helper
    {
        template <typename V>
    ...
    template <typename S> struct ctor_helper<S, true>
    {
    ...
    template <typename S> static constexpr std::size_t ctor_len( S &&s )
    {
        using Y = typename std::remove_reference<S>::type;
        using help =
            ctor_helper<Y, std::is_array<Y>::value && std::is_const<Y>::value>;
...
public:
    ...
    constexpr std::basic_string<charT, traitsT> to_string( void ) const
    {
        return empty() ? std::basic_string<charT, traitsT>()
                       : std::basic_string<charT, traitsT>( data(), size() );
    }

#ifdef ENABLE_STRING_VIEW_COMPAT
    constexpr const_string(
        const STRING_VIEW_COMPAT_NS::basic_string_view<charT, traitsT> &sv )
        : _str( sv.data() ), _sz( sv.size() )
    {}
    /// @brief cast operator to std::basic_string_view
    constexpr operator STRING_VIEW_COMPAT_NS::basic_string_view<charT, traitsT>(
        void ) const
    {
        return STRING_VIEW_COMPAT_NS::basic_string_view<charT, traitsT>(
            data(), size() );
    }
#endif
    ...
    constexpr const value_type at( size_type i ) const
    {
        return i <= _sz ? _str[i]
                        : throw std::out_of_range(
                              "const_string access out of range" );
    }
    ...
    constexpr const value_type &back( void ) const
    {
        return *( _str + ( size() - 1 ) );
    }
    ...
    constexpr int compare( const const_string &s ) const
    {
        return (
            size() <= s.size()
                ? compare_priv(
                      begin(), s.begin(), size(), size() == s.size() ? 0 : -1 )
                : compare_priv( begin(), s.begin(), s.size(), 1 ) );
    }

    constexpr int
    compare( size_type pos, size_type n, const const_string &s ) const
    {
        return substr( pos, n ).compare( s );
    }

    constexpr int
    compare( size_type pos, size_type n, const value_type *s ) const
    {
    ...
    {
        return (
            pos < size() ? ( traits_type::eq( _str[pos], c )
                                 ? pos
                                 : find_first_of( c, pos + 1 ) )
                         : npos );
    }
    ...
    {
        return (
            pos < size() ? ( traits_type::find( s, n, _str[pos] ) != nullptr
                                 ? pos
                                 : find_first_of( s, pos + 1, n ) )
                         : npos );
    }
    ...
    constexpr size_type find_last_of( value_type c, size_type pos = npos ) const
    {
        return (
            pos >= size()
                ? find_last_of( c, size() - 1 )
                : ( traits_type::eq( _str[pos], c )
                        ? pos
...
inline constexpr bool operator==(
    const const_string<charT, traitsT> &lhs,
    const const_string<charT, traitsT> &rhs )
{
...
{
    return lhs.compare(
               const_string<charT, traitsT>( std::forward<S>( rhs ) ) ) == 0;
}
...
{
    return rhs.compare(
               const_string<charT, traitsT>( std::forward<S>( lhs ) ) ) == 0;
}
...
inline typename std::basic_string<charT, traitsT, allocT> operator+(
    const std::basic_string<charT, traitsT, allocT> &s,
    const_string<charT, traitsT>                     cs )
{
...
template <typename charT, typename traitsT>
inline typename std::basic_string<charT, traitsT>
operator+( const_string<charT, traitsT> a, const_string<charT, traitsT> b )
{
...
inline Ret stoa(
    RetT ( *convFunc )( const CharT *, CharT **, Base... ),
    const char * fname,
    const CharT *s,
    std::size_t *pos,
    Base... base )
...
inline int
stoi( cstring s, std::size_t *pos = nullptr, int base = 10 ) // NOLINT
{
    return detail::stoa<long, int>(
        &std::strtol,
        "base::const_string::stoi",
        s.data(),
        pos,
        base ); // NOLINT
}
...
inline long
stol( cstring s, std::size_t *pos = nullptr, int base = 10 ) // NOLINT
{
    return detail::stoa(
        &std::strtol, "base::const_string::stol", s.data(), pos, base );
}

inline long long
stoll( cstring s, std::size_t *pos = nullptr, int base = 10 ) // NOLINT
{
    return detail::stoa(
        &std::strtoll, "base::const_string::stoll", s.data(), pos, base );
}

inline unsigned long
stoul( cstring s, std::size_t *pos = nullptr, int base = 10 ) // NOLINT
{
    return detail::stoa(
        &std::strtoul, "base::const_string::stoul", s.data(), pos, base );
}

inline unsigned long long
stoull( cstring s, std::size_t *pos = nullptr, int base = 10 ) // NOLINT
{
    return detail::stoa(
        &std::strtoull, "base::const_string::stoull", s.data(), pos, base );
}

inline float stof( cstring s, std::size_t *pos = nullptr )
{
    return detail::stoa(
        &std::strtof, "base::const_string::stof", s.data(), pos );
}
inline double stod( cstring s, std::size_t *pos = nullptr )
{
    return detail::stoa(
        &std::strtod, "base::const_string::stod", s.data(), pos );
}
inline long double stold( cstring s, std::size_t *pos = nullptr )
{
    return detail::stoa(
        &std::strtold, "base::const_string::stold", s.data(), pos );
}
