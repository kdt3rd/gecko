...
public:
    ...
    public:
        ...
        typedef std::function<bool(
            option &, size_t &, const std::vector<char *> & )>
            parser;
        ...
        const std::string &args_help( void ) const { return _args; }
        ...
        explicit operator bool( void ) const { return _has_value; }
        ...
        const std::vector<char *> &values( void ) const { return _values; }

        /// @brief Get the ith value.
        /// @param i The index of the value to get.
        /// @returns The ith value for this option.
        char *operator[]( size_t i ) { return _values.at( i ); }
    ...
    /// @param o Option to add to the command line parser.
    /// @param opts Extra options to also add.
    template <typename... Opts> void add( option o, Opts &&... opts )
    {
    ...
    /// @brief Add an option.
    /// @param o Option to add to the command line parser.
    void add( option o ) { _options.emplace_back( std::move( o ) ); }
    ...
    const option &operator[]( size_t n ) const { return _options.at( n ); }

    /// @brief Get nth option.
    /// @param n Index of option to get.
    /// @returns The nth option.
    option &operator[]( size_t n ) { return _options.at( n ); }
    ...
    const std::vector<option> &options( void ) const { return _options; }
        ...
        for ( size_t i = 0; i < b; ++i )
        {
            if ( idx < args.size() )
            {
                if ( args[idx][0] == '-' )
                {
                    if ( i < a )
                        throw_runtime(
                            "option '{0}' needs at least {1} values (got {2})",
                            opt.name(),
                            a,
                            i );
                    break;
    ...
    /// @returns True if parsing was successful.
    static bool
    flag( option &opt, size_t &idx, const std::vector<char *> &args );
    ...
    /// @returns True if parsing was successful.
    static bool
    args( option &opt, size_t &idx, const std::vector<char *> &args );
    ...
    /// @returns True if parsing was successful.
    static bool
    multi( option &opt, size_t &idx, const std::vector<char *> &args );
    ...
    /// @returns True if parsing was successful.
    static bool
    counted( option &opt, size_t &idx, const std::vector<char *> &args );
