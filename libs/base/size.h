...
public:
    static_assert(
        ...
        "size should be composed of an arithmetic value type" );
    using coord_type = T;
    ...
    constexpr size( coord_type ww, coord_type hh ) : _w( ww ), _h( hh ) {}
    ...
    template <typename U> explicit inline operator size<U>() const
    {
        return size<U>( *this );
    }
    ...
    void shrink( coord_type dw, coord_type dh )
    {
        _w -= dw;
        _h -= dh;
    }
    ...
    void grow( coord_type dw, coord_type dh )
    {
        _w += dw;
        _h += dh;
    }
    ...
    size operator+( const size &s ) const { return { _w + s._w, _h + s._h }; }

    /// @brief less than operator
    inline bool operator<( const size &o ) const
    {
        return _w < o._w ||
               ( std::equal_to<coord_type>()( _w, o._w ) && _h < o._h );
    }
...
{
    return std::equal_to<T>()( a.w(), b.w() ) &&
           std::equal_to<T>()( a.h(), b.h() );
}
...
template <typename T> inline size<T> abs( const size<T> &s )
{
    using namespace std;
...
/// @brief Round size up following rounding rules
template <typename T> inline size<T> round( const size<T> &s )
{
    using namespace std;
...
template <typename T> inline size<long> lround( const size<T> &s )
{
    using namespace std;
...
/// @brief Round size up to nearest integer based on half-way
template <typename T> inline size<long long> llround( const size<T> &s )
{
    using namespace std;
...
template <typename T> inline size<T> ceil( const size<T> &s )
{
    using namespace std;
    return size<T>(
        static_cast<T>( ceil( s.w() ) ), static_cast<T>( ceil( s.h() ) ) );
}

/// @brief Round size up to nearest integer
template <typename T> inline size<T> floor( const size<T> &s )
{
    using namespace std;
    return size<T>(
        static_cast<T>( floor( s.w() ) ), static_cast<T>( floor( s.h() ) ) );
}

/// @brief Round size up to nearest integer
template <typename T> inline size<T> max( const size<T> &a, const size<T> &b )
{
    using namespace std;
...
/// @brief Round size up to nearest integer
template <typename T> inline size<T> min( const size<T> &a, const size<T> &b )
{
    using namespace std;
...
namespace std
{
template <typename T> inline base::size<T> abs( const base::size<T> &a )
{
    return base::abs( a );
}

template <typename T> inline base::size<T> round( const base::size<T> &a )
{
    return base::round( a );
}

template <typename T> inline base::size<long> lround( const base::size<T> &a )
{
...
template <typename T> inline base::size<T> ceil( const base::size<T> &a )
{
    return base::ceil( a );
}

template <typename T> inline base::size<T> floor( const base::size<T> &a )
{
    return base::floor( a );
}
