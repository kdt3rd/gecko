...
#define throw_bad_any_cast() throw_location( base::bad_any_cast() )

template <typename T> struct in_place_type
{};
...
private:
    ...
    template <
        typename T,
        typename safeT = std::is_nothrow_move_constructible<T>,
        bool fits      = ( sizeof( T ) <= sizeof( val_storage ) ) &&
                    ( alignof( T ) <= alignof( val_storage ) )>
    ...
    using proc_func =
        void *(*)( val_op, const any *, any *, const std::type_info *tinfo );
    ...
    template <typename T> struct local_store
    {
        ...
        static inline void *
        apply( val_op o, const any *v, any *a, const std::type_info *tinfo )
        {
            switch ( o )
            {
                case val_op::tinfo:
                ...
                case val_op::destroy: destroy( *const_cast<any *>( v ) ); break;
    ...
    inline void *call_proc(
        val_op                op,
        any *                 o     = nullptr,
        const std::type_info *tinfo = nullptr ) const
    {
...
public:
    ...
    template <typename T>
    using processor = typename std::conditional<
        internal_storage<T>::value,
        local_store<T>,
        heap_store<T>>::type;
    ...
    inline any( const any &o ) : _processor( nullptr )
    {
        if ( o.has_value() )
            o.call_proc( val_op::copy, this, nullptr );
    }
    inline any( any &&o ) noexcept : _processor( nullptr )
    {
        if ( o.has_value() )
    ...
    template <
        ...
        typename    = typename std::enable_if<
            ...
            !std::is_same<Tp, any>::value>::type>
    explicit inline any(
        in_place_type<T>, std::initializer_list<U> il, Args &&... args )
    {
    ...
    template <
        ...
        typename    = typename std::enable_if<
            ...
            std::is_copy_constructible<Tp>::value>::type>
    ...
    inline const std::type_info &type() const noexcept
    {
        if ( has_value() )
            return *static_cast<const std::type_info *>(
                this->_processor( val_op::tinfo, nullptr, nullptr, nullptr ) );
        return typeid( void );
    }
...
protected:
    ...
    template <typename T> const T *as( void ) const noexcept
    {
        return static_cast<const T *>(
            this->call_proc( val_op::get, nullptr, &typeid( T ) ) );
    }

    template <typename T> T *as( void ) noexcept
    {
...
template <typename T, typename... Args> inline any make_any( Args &&... args )
{
...
template <typename T>
inline T
any_cast_move_true( typename std::remove_reference<T>::type *p, std::true_type )
{
    return std::move( *p );
}

template <typename T>
inline T any_cast_move_true(
    typename std::remove_reference<T>::type *p, std::false_type )
{
    return *p;
}
...
template <class T> inline const T *any_cast( const any *a ) noexcept
{
    if ( a == nullptr )
        return nullptr;
    return a->as<T>();
...
template <class T> inline T *any_cast( any *a ) noexcept
{
    if ( a == nullptr )
        return nullptr;
    return a->as<T>();
...
namespace std
{
inline void swap( base::any &a, base::any &b ) { a.swap( b ); }
