...
inline const std::type_info &extract_arg_type( size_t i, sequence<S...> )
{
    struct tref
    {
        constexpr inline tref( const std::type_info &r ) : _r( r ) {}
        constexpr inline operator const std::type_info &(void)const
        {
            return _r;
        }
    ...
    static constexpr tref type_table[] = { typeid(
        typename base::nth_variadic<S, Args...>::type )... };

    return type_table[i];
}

template <typename R, typename... Args> struct base_traits
{
    static constexpr size_t arity = sizeof...( Args );
    typedef R               result_type;
    typedef R ( *pointer )( Args... );

    template <size_t N> using get_arg_type = base::nth_variadic<N, Args...>;

    typedef std::function<R( Args... )> function;

    static inline const std::type_info &arg_type( size_t i )
    {
        return extract_arg_type<Args...>(
            i, gen_sequence<sizeof...( Args )>{} );
    }
};

template <typename... Args> struct base_traits<void, Args...>
{
    static constexpr size_t arity = sizeof...( Args );
...
inline typename function_traits<Functor>::function to_function( Functor f )
{
...
to_function( R ( C::*memF )( Args... ), C &o )
{
    return priv::binder_help(
        memF, &o, make_int_sequence<sizeof...( Args )>{} );
}
