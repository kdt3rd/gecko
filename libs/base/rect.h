...
public:
    ...
    static constexpr coord_type kEndOff =
        std::is_integral<T>::value ? coord_type( 1 ) : coord_type( 0 );
    ...
    rect( coord_type w, coord_type h ) : _extent( w, h ) { fix_extent(); }
    ...
    rect( const size_type &s ) : _extent( s ) { fix_extent(); }

    /// @brief allow explicit construction from other rectangle type
    template <typename U>
    explicit constexpr rect( const rect<U> &o )
        : _position( point_type( o.position() ) )
        , _extent( size_type( o.extent() ) )
    {}
    ...
    constexpr coord_type
    x( coord_type percent, coord_type radius = coord_type( 0 ) ) const
    {
        static_assert(
            !std::is_integral<coord_type>::value,
            "expect a floating point type for percent accessor" );
        return _position[0] + radius +
               ( _extent.w() - radius * coord_type( 2 ) ) * percent;
    }

    /// @brief Y coordinate in the rectangle
    constexpr coord_type
    y( coord_type percent, coord_type radius = coord_type( 0 ) ) const
    {
        static_assert(
            !std::is_integral<coord_type>::value,
            "expect a floating point type for percent accessor" );
        return _position[1] + radius +
               ( _extent.h() - radius * coord_type( 2 ) ) * percent;
    }
    ...
    /// @brief X coordinate of right side
    constexpr coord_type x2( void ) const
    {
        return _position[0] + _extent.w() - kEndOff;
    }

    /// @brief Y coordinate of bottom side
    constexpr coord_type y2( void ) const
    {
        return _position[1] + _extent.h() - kEndOff;
    }
    ...
    constexpr point_type top_right( void ) const
    {
        return point_type( x2(), y1() );
    }

    /// @brief Top center point
    constexpr point_type top_center( void ) const
    {
        return point_type( ( x1() + x2() ) / coord_type( 2 ), y1() );
    }

    /// @brief Bottom left corner
    constexpr point_type bottom_left( void ) const
    {
        return point_type( x1(), y2() );
    }

    /// @brief Bottom right corner
    constexpr point_type bottom_right( void ) const
    {
    ...
    constexpr point_type center( void ) const
    {
        return point_type(
            ( x1() + x2() ) / coord_type( 2 ),
            ( y1() + y2() ) / coord_type( 2 ) );
    ...
    coord_type radius( void ) const
    {
        return std::min( width(), height() ) / coord_type( 2 );
    }
    ...
    /// @brief Set height
    void set_height( coord_type h )
    {
        _extent.set_height( h );
        fix_extent();
    }

    /// @brief Set size
    void set_size( coord_type w, coord_type h )
    {
        _extent.set_width( w );
        _extent.set_height( h );
        fix_extent();
    ...
    /// @brief Set size
    void set_extent( const size_type &s )
    {
        _extent = s;
        fix_extent();
    }
    ...
    void trim( coord_type l, coord_type r, coord_type t, coord_type b )
    {
        _extent.shrink( l + r, t + b );
        _position.move_by( l, t );
    }
    ...
    void shrink(
        coord_type left, coord_type right, coord_type top, coord_type bottom )
    {
    ...
    void
    grow( coord_type left, coord_type right, coord_type top, coord_type bottom )
    {
        _position.move_by( -left, -top );
    ...
    bool empty( void ) const { return _extent.empty(); }
    ...
    static rect from_points(
        coord_type xx1, coord_type yy1, coord_type xx2, coord_type yy2 )
    {
        if ( xx2 < xx1 || yy2 < yy1 )
        {
            std::swap( xx1, xx2 );
...
private:
    ...
    {
        _position.move_by(
            std::min( _extent.w(), coord_type( 0 ) ),
            std::min( _extent.h(), coord_type( 0 ) ) );
        _extent.set(
            coord_type( std::abs( _extent.w() ) ),
