...
inline void mix( const uint64_t *data, std::array<uint64_t, 12> &s )
{
    s[0] += data[0];
    s[2] ^= s[10];
    s[11] ^= s[0];
...
short_msg( const void *message, size_t length, base::spooky_hash::value &hash )
{
    ...
    switch ( remainder )
    {
        case 15: d += static_cast<uint64_t>( p8[14] ) << 48; GK_FALLTHROUGH;
        case 14: d += static_cast<uint64_t>( p8[13] ) << 40; GK_FALLTHROUGH;
        case 13: d += static_cast<uint64_t>( p8[12] ) << 32; GK_FALLTHROUGH;
        case 12:
            d += static_cast<uint64_t>( p32[2] );
            c += p64[0];
            break;
        case 11: d += static_cast<uint64_t>( p8[10] ) << 16; GK_FALLTHROUGH;
        case 10: d += static_cast<uint64_t>( p8[9] ) << 8; GK_FALLTHROUGH;
        case 9: d += static_cast<uint64_t>( p8[8] ); GK_FALLTHROUGH;
        case 8: c += p64[0]; break;

        case 7: c += static_cast<uint64_t>( p8[6] ) << 48; GK_FALLTHROUGH;
        case 6: c += static_cast<uint64_t>( p8[5] ) << 40; GK_FALLTHROUGH;
        case 5: c += static_cast<uint64_t>( p8[4] ) << 32; GK_FALLTHROUGH;
        case 4: c += p32[0]; break;

        case 3: c += static_cast<uint64_t>( p8[2] ) << 16; GK_FALLTHROUGH;
        case 2: c += static_cast<uint64_t>( p8[1] ) << 8; GK_FALLTHROUGH;
        case 1: c += static_cast<uint64_t>( p8[0] ); break;
...
void spooky_hash::reset( const value &seed )
{
    _length    = 0;
    _remainder = 0;
    _state[0]  = seed[0];
...
void spooky_hash::add( const void *msg, size_t len )
{
    ...
    if ( _remainder )
    {
        ...
        p64 = reinterpret_cast<const uint64_t *>(
            ( reinterpret_cast<const uint8_t *>( msg ) + prefix ) );
        len -= prefix;
    }
    ...
    _remainder =
        ( len - static_cast<size_t>(
                    ...
                    reinterpret_cast<const uint8_t *>( p64 ) ) );
    while ( p64 < end )
    {
        mix( p64, h );
        p64 += stateSize;
...
spooky_hash::value spooky_hash::finish( void ) const
{
    value ret;
    if ( _length < bufSize )
    {
...
{
    os << std::hex << std::setw( 16 ) << std::setfill( '0' ) << v[0]
       << std::setw( 16 ) << std::setfill( '0' ) << v[1] << std::dec;
    return os;
}
