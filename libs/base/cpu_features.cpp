...
namespace
{
#if defined( __x86_64__ ) || defined( _M_X64 ) || defined( __X86__ ) ||        \
    defined( __i386__ ) || defined( i386 ) || defined( _M_IX86 ) ||            \
    defined( __386__ )
#    define IS_INTEL_ISH 1
#    ifdef _WIN32
inline unsigned int
pullMaxID( unsigned int idx, std::array<unsigned int, 4> &regs ) noexcept
{
    __cpuid( (int *)regs.data(), 0 );
    return regs[0];
}
inline void
pullID( unsigned int idx, std::array<unsigned int, 4> &regs ) noexcept
{
...
inline void pullID(
    unsigned int                 idx,
    unsigned int                 count,
    std::array<unsigned int, 4> &regs ) noexcept
{
...
struct CPUFeatureStore
{
    CPUFeatureStore( void ) noexcept
    {
        ...
        if ( maxFeat >= 1 )
        {
            ...
            _cache_line_bits =
                static_cast<size_t>( ( regs[1] >> 8 ) & 0xFF ) * 8;
        }
        ...
        if ( _amd )
        {
            ...
            if ( maxExtFeat >= 6 )
            {
                ...
                switch ( _l2_assoc )
                {
                    ...
                    default: _l2_assoc = size_t( -1 ); break;
                }
                _l3_data_line_size = regs[3] & 0xFF;
                _l3_data_cache =
                    static_cast<size_t>( regs[3] >> 18 ) * 512 * 1024;
                ...
                switch ( _l3_assoc )
                {
                    ...
                    default: _l3_assoc = size_t( -1 ); break;
                }
            }
        }
    ...
    void decodeCacheTLB( uint8_t v, bool &check4 )
    {
        switch ( v )
        {
            ...
            case 0xEC: // L3 cache: 24MB, 24-way, 64
                ...
                _l3_assoc          = 24;
                break;
            case 0xF0: _prefetch_size = 64; break;
            case 0xF1: _prefetch_size = 128; break;
            case 0xFF:
...
const CPUFeatureStore &theFeatures( void )
{
...
namespace base
{
const char *cpu::vendor( void ) { return theFeatures()._vendor; }

const char *cpu::brand( void ) { return theFeatures()._brand; }

void cpu::output( std::ostream &os )
{
    ...
    if ( has_RDTSCP() )
        os << " rdtscp";
    os << " ]\n  cache_line_bits " << cache_line_bits() << " prefetch "
       << prefetch() << " bytes"
       << "\n  L1 instr: [ " << l1_instruction_cache() << " / "
       << l1_instruction_line_size() << " byte lines / "
       << l1_instruction_associativity() << "-way assoc ]"
       << "\n  L1 data: [ " << l1_data_cache() << " / " << l1_data_line_size()
       << " byte lines / " << l1_associativity() << "-way assoc ]"
       << "\n  L2 data: [ " << l2_data_cache() << " / " << l2_data_line_size()
       << " byte lines / " << l2_associativity() << "-way assoc ]"
       << "\n  L3 data: [ " << l3_data_cache() << " / " << l3_data_line_size()
       << " byte lines / " << l3_associativity() << "-way assoc ]";
#endif
}
...
size_t cpu::l1_instruction_cache( void )
{
    return theFeatures()._l1_instr_cache;
}
size_t cpu::l1_instruction_line_size( void )
{
    return theFeatures()._l1_instr_line_size;
}
size_t cpu::l1_instruction_associativity( void )
{
    return theFeatures()._l1_instr_assoc;
}

size_t cpu::l1_data_cache( void ) { return theFeatures()._l1_data_cache; }
size_t cpu::l1_data_line_size( void )
{
    return theFeatures()._l1_data_line_size;
}
...
size_t cpu::l2_data_line_size( void )
{
    return theFeatures()._l2_data_line_size;
}
...
size_t cpu::l3_data_line_size( void )
{
    return theFeatures()._l3_data_line_size;
}
...
bool cpu::fast_unaligned_load( void )
{
    return has_misaligned_sse() || ( theFeatures()._intel && has_POPCNT() );
}
bool cpu::has_misaligned_sse( void ) { return theFeatures()._feat81_ECX[7]; }
bool cpu::has_MMX( void ) { return theFeatures()._feat1_EDX[23]; }
bool cpu::has_MMX_EXT( void )
{
    return theFeatures()._amd && theFeatures()._feat81_EDX[22];
}
...
bool cpu::has_SSE4a( void )
{
    return theFeatures()._amd && theFeatures()._feat81_ECX[6];
}
bool cpu::has_3DNOW( void )
{
    return theFeatures()._amd && theFeatures()._feat81_EDX[31];
}
bool cpu::has_3DNOW_EXT( void )
{
    return theFeatures()._amd && theFeatures()._feat81_EDX[30];
}
...
bool cpu::has_HLE( void )
{
    return theFeatures()._intel && theFeatures()._feat7_EBX[4];
}
...
bool cpu::has_RTM( void )
{
    return theFeatures()._intel && theFeatures()._feat7_EBX[11];
}
...
bool cpu::has_LZCNT( void )
{
    return theFeatures()._intel && theFeatures()._feat81_ECX[5];
}
bool cpu::has_ABM( void )
{
    return theFeatures()._amd && theFeatures()._feat81_ECX[5];
}
bool cpu::has_XOP( void )
{
    return theFeatures()._amd && theFeatures()._feat81_ECX[11];
}
bool cpu::has_TBM( void )
{
    return theFeatures()._amd && theFeatures()._feat81_ECX[21];
}
