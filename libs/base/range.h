...
template <typename Number> class range
{
public:
    static_assert(
        std::is_integral<Number>::value, "Number must be integeral type" );

    constexpr range( Number f, Number l )
        : _first( std::min( f, l ) ), _last( std::max( f, l ) )
    {}

    constexpr Number first( void ) const { return _first; }

    constexpr Number last( void ) const { return _last; }
    ...
    constexpr bool operator<( const range &that ) const
    {
        return ( _first < that._first )
                   ? true
                   : ( ( _first == that._first ) ? _last < that._last : false );
    }

    void clear( void ) { _last = _first; }
