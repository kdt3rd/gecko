        ...
        if ( str[cur] == '$' )
        {
            ...
            if ( std::isalpha( str[cur] ) || str[cur] == '_' )
            {
                ...
                if ( cur != last && str[cur] == '[' )
                {
                    ...
                    if ( count == 0 )
                    {
                        ...
                        throw_runtime(
                            "Unable to find end of array reference with var looking like '{0}' in string '{1}'",
                            var,
                            str );
                }
                else
            ...
            else if ( str[cur] == '{' )
            {
                ...
                if ( count == 0 )
                {
                    ...
                    throw_runtime(
                        "Unable to find end of variable reference with var looking like '{0}' in string '{1}'",
                        var,
                        str );
            }
            else
                throw_runtime(
                    "Invalid variable character at position {0} in string '{1}'",
                    cur,
                    str );
        }
        else
    ...
    const std::function<std::string( const std::string & )> &varLookup )
{
    ...
    {
        postcondition(
            !vars.empty() && ( pieces.size() == vars.size() ||
                               pieces.size() == ( vars.size() + 1 ) ),
            "bug in find_unix_vars with string '{0}', expect N or N+1 pieces for N vars",
            str );
    ...
    postcondition(
        pieces.size() == 1 && vars.empty(),
        "bug in find_unix_vars with string '{0}'",
        str );
    ...
    const std::string &       str )
{
    ...
    while ( i != std::string::npos )
    {
        ...
        if ( e == std::string::npos )
            throw_runtime(
                "Unable to find end of variable marker in {0}", str );
    ...
    std::function<std::string( const std::string & )> &varLookup )
{
    ...
    while ( cur != last )
    {
        if ( str[cur] == '%' )
        {
            ...
            if ( e == std::string::npos )
                throw_runtime(
                    "Unable to find end of variable in {0} starting at position {1}",
                    str,
                    cur );
            if ( e == ( cur + 1 ) )
                retval.push_back( '%' );
