...
viewer_w::viewer_w( void ) : _rect( color{ 0.21F, 0.21F, 0.21F, 1.F } ) {}
...
void viewer_w::remove_video_track(
    const std::shared_ptr<media::video_track> &t )
{
...
void viewer_w::update_images( bool force_reload )
{
    ...
    for ( auto &i: _images )
    {
             ...
             ( force_reload || curs._last_loaded != _current_sample ) )
        {
            ...
            {
                std::cerr << "Unable to read sample " << _current_sample
                          << " for image ID " << i.first << ": " << e.what()
                          << std::endl;
                curs._image->clear();
