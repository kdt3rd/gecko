...
inline char32_t byte2of2( int b ) { return follow( b ); }

inline char32_t byte1of3( int b ) { return uint32_t( b & ~0xE0 ) << 12; }

inline char32_t byte2of3( int b ) { return follow( b ) << 6; }

inline char32_t byte3of3( int b ) { return follow( b ); }

inline char32_t byte1of4( int b ) { return uint32_t( b & ~0xF0 ) << 18; }

inline char32_t byte2of4( int b ) { return follow( b ) << 12; }

inline char32_t byte3of4( int b ) { return follow( b ) << 6; }

inline char32_t byte4of4( int b ) { return follow( b ); }
} // namespace
...
char32_t read_16be( std::istream &in )
{
    ...
    result =
        char32_t( 0x10000 + ( ( result & 0x3FF ) << 10 ) + ( w2 & 0x3FF ) );
...
iterator::iterator( std::istream &str, mode m ) : _mode( m ), _stream( str ) {}
...
iterator &iterator::operator++()
{
    ...
    switch ( _mode )
    {
        case UTF8: _value = read_8( _stream ); break;

        case UTF16BE: _value = read_16be( _stream ); break;

        case UTF16LE: _value = read_16le( _stream ); break;

        case UTF32BE: _value = read_32be( _stream ); break;

        case UTF32LE: _value = read_32le( _stream ); break;
    }
...
void write( std::ostream &out, char32_t cp )
{
    ...
    if ( cp <= 0x7F )
        ...
        out << static_cast<char_type>( 0xC0 | ( cp >> 6 ) )
            << static_cast<char_type>( 0x80 | ( cp & 0x3F ) );
    else if ( cp <= 0xFFFF )
        out << static_cast<char_type>( 0xE0 | ( cp >> 12 ) )
            << static_cast<char_type>( 0x80 | ( ( cp >> 6 ) & 0x3F ) )
            << static_cast<char_type>( 0x80 | ( cp & 0x3F ) );
    else if ( cp <= 0x10FFFF )
        out << static_cast<char_type>( 0xF0 | ( cp >> 18 ) )
            << static_cast<char_type>( 0x80 | ( ( cp >> 12 ) & 0x3F ) )
            << static_cast<char_type>( 0x80 | ( ( cp >> 6 ) & 0x3F ) )
            << static_cast<char_type>( 0x80 | ( cp & 0x3F ) );
    else
    ...
    for ( size_t b = 0; b < str.size(); ++b )
    {
        ...
        if ( base < 0xFFFFFFFF )
        {
            for ( size_t c = b + 1; c < str.size(); ++c )
            {
                ...
                if ( b + 1 != c && canonical_combining_class( str[c - 1] ) >=
                                       canonical_combining_class( str[c] ) )
                    comb = 0xFFFFFFFF;
                if ( comb < 0xFFFFFFFF )
                {
...
std::ostream &operator<<( std::ostream &out, const std::u32string &str )
{
    ...
    for ( size_t i = 0; i < str.size(); ++i )
    {
        if ( str[i] <= 0x7F )
            ...
            out << static_cast<char_type>( 0xC0 | ( str[i] >> 6 ) )
                << static_cast<char_type>( 0x80 | ( str[i] & 0x3F ) );
        else if ( str[i] <= 0xFFFF )
            out << static_cast<char_type>( 0xE0 | ( str[i] >> 12 ) )
                << static_cast<char_type>( 0x80 | ( ( str[i] >> 6 ) & 0x3F ) )
                << static_cast<char_type>( 0x80 | ( str[i] & 0x3F ) );
        else if ( str[i] <= 0x10FFFF )
            out << static_cast<char_type>( 0xF0 | ( str[i] >> 18 ) )
                << static_cast<char_type>( 0x80 | ( ( str[i] >> 12 ) & 0x3F ) )
                << static_cast<char_type>( 0x80 | ( ( str[i] >> 6 ) & 0x3F ) )
                << static_cast<char_type>( 0x80 | ( str[i] & 0x3F ) );
        else
