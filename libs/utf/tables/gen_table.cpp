...
static std::vector<double>
                             nvalue( maxcp, std::numeric_limits<double>::quiet_NaN() );
...
namespace
{
template <typename T> T from_string( const std::string &str, bool hex = false )
{
    T                 t;
...
static std::map<std::string, uint32_t> propNames{
    { "XIDS", 1 << 0 },    { "XIDC", 1 << 1 },    { "Pat_WS", 1 << 2 },
    { "Pat_Syn", 1 << 3 }, { "Math", 1 << 4 },    { "WSpace", 1 << 5 },
    { "Hex", 1 << 6 },     { "Dash", 1 << 8 },    { "Hyphen", 1 << 9 },
    { "QMark", 1 << 10 },  { "Term", 1 << 11 },   { "Upper", 1 << 12 },
    { "Lower", 1 << 13 },  { "Alpha", 1 << 14 },  { "OUpper", 1 << 15 },
    { "OLower", 1 << 16 }, { "OAlpha", 1 << 17 }, { "OMath", 1 << 18 },
    { "NChar", 1 << 19 },  { "Nu", 1 << 20 },     { "De", 1 << 21 },
    { "Di", 1 << 22 },
};
...
static std::map<std::string, uint8_t> catNames{
    { "Cc", 1 },  { "Cf", 2 },  { "Cn", 3 },  { "Co", 4 },  { "Cs", 5 },
    { "Ll", 6 },  { "Lm", 7 },  { "Lo", 8 },  { "Lt", 9 },  { "Lu", 10 },
    { "Mc", 11 }, { "Me", 12 }, { "Mn", 13 }, { "Nd", 14 }, { "Nl", 15 },
    { "No", 16 }, { "Pc", 17 }, { "Pd", 18 }, { "Pe", 19 }, { "Pf", 20 },
    { "Pi", 21 }, { "Po", 22 }, { "Ps", 23 }, { "Sc", 24 }, { "Sk", 25 },
    { "Sm", 26 }, { "So", 27 }, { "Zl", 28 }, { "Zp", 29 }, { "Zs", 30 }
};

////////////////////////////////////////

static std::map<std::string, double> numberValues{
    { "-1/2", -1.0 / 2.0 }, { "1/10", 1.0 / 10.0 }, { "1/16", 1.0 / 16.0 },
    { "1/2", 1.0 / 2.0 },   { "1/3", 1.0 / 3.0 },   { "1/4", 1.0 / 4.0 },
    { "1/5", 1.0 / 5.0 },   { "1/6", 1.0 / 6.0 },   { "1/7", 1.0 / 7.0 },
    { "1/8", 1.0 / 8.0 },   { "1/9", 1.0 / 9.0 },   { "11/2", 11.0 / 2.0 },
    { "13/2", 13.0 / 2.0 }, { "15/2", 15.0 / 2.0 }, { "17/2", 17.0 / 2.0 },
    { "2/3", 2.0 / 3.0 },   { "2/5", 2.0 / 5.0 },   { "3/16", 3.0 / 16.0 },
    { "3/2", 3.0 / 2.0 },   { "3/4", 3.0 / 4.0 },   { "3/5", 3.0 / 5.0 },
    { "3/8", 3.0 / 8.0 },   { "4/5", 4.0 / 5.0 },   { "5/2", 5.0 / 2.0 },
    { "5/6", 5.0 / 6.0 },   { "5/8", 5.0 / 8.0 },   { "7/2", 7.0 / 2.0 },
    { "7/8", 7.0 / 8.0 },   { "9/2", 9.0 / 2.0 }
};

////////////////////////////////////////

static std::map<std::string, uint8_t> decompNames{
    { "none", 0 },  { "can", 1 },  { "com", 2 },  { "sup", 3 },  { "fra", 4 },
    { "nb", 5 },    { "sub", 6 },  { "font", 7 }, { "enc", 8 },  { "wide", 9 },
    { "vert", 10 }, { "sqr", 11 }, { "iso", 12 }, { "fin", 13 }, { "init", 14 },
    { "med", 15 },  { "sml", 16 }, { "nar", 17 }
};
...
public:
    ...
    {
        assert( !mode.empty() );
        if ( mode.back() == CHARACTER || mode.back() == SURROGATE ||
             mode.back() == RESERVED || mode.back() == NONCHAR )
        {
            if ( name == "cp" )
            {
                    ...
                    if ( value != "#" )
                    {
                        ...
                        if ( decomp.empty() )
                            throw std::runtime_error(
                                "Didn't read any decomposition characters" );
                        decomp.push_back( 0 );
...
static std::map<std::string, std::string> catTypes{
    { "Cc", "other_control" },         { "Cf", "other_format" },
    { "Cn", "other_not_assigned" },    { "Co", "other_private" },
    { "Cs", "other_surrogate" },       { "Ll", "letter_lowercase" },
    { "Lm", "letter_modifier" },       { "Lo", "letter_other" },
    { "Lt", "letter_titlecase" },      { "Lu", "letter_uppercase" },
    { "Mc", "mark_combining" },        { "Me", "mark_enclosing" },
    { "Mn", "mark_nonspacing" },       { "Nd", "number_decimal" },
    { "Nl", "number_letter" },         { "No", "number_other" },
    { "Pc", "punctuation_connector" }, { "Pd", "punctuation_dash" },
    { "Pe", "punctuation_close" },     { "Pf", "punctuation_final" },
    { "Pi", "punctuation_initial" },   { "Po", "punctuation_other" },
    { "Ps", "punctuation_open" },      { "Sc", "symbol_currency" },
    { "Sk", "symbol_modifier" },       { "Sm", "symbol_math" },
    { "So", "symbol_other" },          { "Zl", "separator_line" },
    { "Zp", "separator_paragraph" },   { "Zs", "separator_space" }
};

static std::map<std::string, std::string> catMainTypes{
    { "C", "other" },     { "L", "letter" },      { "M", "mark" },
    { "N", "number" },    { "P", "punctuation" }, { "S", "symbol" },
    { "Z", "separator" },
};
...
static std::map<std::string, uint8_t> bidiNames = {
    { "L", 1 },   { "LRE", 2 }, { "LRO", 3 }, { "R", 4 },    { "AL", 5 },
    { "RLE", 6 }, { "RLO", 7 }, { "PDF", 8 }, { "EN", 9 },   { "ES", 10 },
    { "ET", 11 }, { "AN", 12 }, { "CS", 13 }, { "NSM", 14 }, { "BN", 15 },
    { "B", 16 },  { "S", 17 },  { "WS", 18 }, { "ON", 19 },
};
...
int safemain( int argc, char *argv[] )
{
...

    {
        ...
        {
            std::ifstream in(
                fn.c_str(), std::ios_base::in | std::ios_base::binary );
            ...
            if ( zErr != Z_OK )
            {
                std::cerr << "Unable to init compression library: (" << zErr
                          << ") " << ( strm.msg ? strm.msg : zError( zErr ) )
                          << std::endl;
                return -1;
            }
            ...
            do
            {
                strm.avail_in = static_cast<uInt>(
                    in.read( reinterpret_cast<char *>( tmpBufIn ), kChunk )
                        .gcount() );
                if ( strm.avail_in == 0 )
                    break;
                ...
                do
                {
                    ...
                    switch ( zErr )
                    {
                        ...
                        case Z_MEM_ERROR:
                            std::cerr
                                << "Error reading compressed file: (" << zErr
                                << ") "
                                << ( strm.msg ? strm.msg : zError( zErr ) )
                                << std::endl;
                    ...
                    decomp.append(
                        reinterpret_cast<char *>( tmpBufOut ), have );
    ...
    for ( char32_t cp = 0; cp < maxcp; ++cp )
    {
        if ( compexcl[cp] == 0 && decomptype[cp] == 1 )
        {
            ...
            if ( decompose[off + 1] == 0 )
                throw std::runtime_error(
                    "Singleton decomposition should not be composed" );
            char32_t comb = decompose[off + 1];
            if ( decompose[off + 2] != 0 )
                throw std::runtime_error(
                    "More than 2 character in composition" );
            ...
            comb_chart.resize(
                std::max(
                    uint32_t( comboff + 1 ), uint32_t( comb_chart.size() ) ),
                0 );
    ...
    for ( size_t i = 0; i < maxcp; ++i )
    {
        if ( !std::isnan( nvalue[i] ) )
        {
            ...
            if ( f == doubleVals.size() )
            {
                ...
                if ( v >= std::numeric_limits<uint64_t>::lowest() &&
...

    {
        ...
        out << "// Automatically generated file\n"
               "\n"
               "#include <utfcat.h>\n"
               "#include <cstdint>\n"
               "\n"
        ...
        for ( auto i = catTypes.begin(); i != catTypes.end(); ++i )
        {
            ...
            out << "{\n";
            out << "\treturn get_category( cp ) == "
                << static_cast<uint32_t>( catNames[i->first] ) << ";\n";
            out << "}\n";
        }
        for ( auto i = catMainTypes.begin(); i != catMainTypes.end(); ++i )
        {
            ...
            out << "\treturn cat > " << static_cast<uint32_t>( tmin )
                << " && cat < " << static_cast<uint32_t>( tmax ) << ";\n";
            out << "}\n";
        }
...

    {
        ...
        for ( auto i = propTypes.begin(); i != propTypes.end(); ++i )
        {
            out << "bool is_" << i->second << "( char32_t cp )\n";
            out << "{\n";
            out << "\treturn ( get_property( cp ) & "
                << static_cast<uint32_t>( propNames[i->first] ) << " ) != 0;\n";
            out << "}\n";
        }
