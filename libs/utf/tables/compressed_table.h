...
{
    static bool test( const T &a, const T &b ) { return a == b; }
};
...
template <typename T> class compressed_table
{
public:
    ...
    size_t size( void ) { return blocks.size() * block_size; }

    size_t data_size( void ) { return data.size(); }

    size_t num_blocks( void ) { return blocks.size(); }

    void
    save( std::ostream &out, const std::string &dtype, const std::string &name )
    {
