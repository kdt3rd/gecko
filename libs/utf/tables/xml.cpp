...
// Eat whitespace
void skip_space(
    std::istreambuf_iterator<char> &i, std::istreambuf_iterator<char> &eos )
{
    ...
    std::istreambuf_iterator<char> &i, std::istreambuf_iterator<char> &eos )
{
    ...
    if ( i != eos )
    {
        if ( *i != '<' )
        {
            std::cerr << "GOT char '" << *i << "' (" << static_cast<int>( *i )
                      << ")" << std::endl;
    ...
    while ( *i != '<' )
    {
        if ( i == eos )
            throw std::runtime_error(
                "XML parse error (end of stream) reading content" );
