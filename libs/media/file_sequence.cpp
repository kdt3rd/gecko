...
file_sequence::file_sequence( base::uri path, marker_style ms )
    : _uri( std::move( path ) ), _marker_style( ms ), _pattern_size( 0 )
{
    ...
    switch ( ms )
    {
        case MARKER_POUND: extract_pound_pattern(); break;
        case MARKER_DOLLARF: extract_dollarf_pattern(); break;
        case MARKER_PERCENTD: extract_percentd_pattern(); break;
        case MARKER_AUTO:
...
base::uri file_sequence::uri( void ) const
{
...
base::uri file_sequence::get_frame( int64_t f ) const
{
    ...
    {
        precondition(
            _pattern_suffix.empty(),
            "No pattern size, but pattern suffix is not empty" );
...
bool file_sequence::extract_frame( const base::uri &u, int64_t &f ) const
{
    ...
    {
        if ( fn.size() <
             ( _pattern_prefix.size() + _pattern_suffix.size() + 1 ) )
            return false;
        ...
        size_t                 digCount = 0;
        while ( digCount < _pattern_size && fpos < fn.size() &&
                std::isdigit( fn[fpos] ) )
        {
...
file_sequence file_sequence::guess( base::uri u, marker_style ms )
{
    ...
    if ( count > 0 )
    {
        ...
        else if ( ms == MARKER_DOLLARF )
        {
            if ( fn[longestStart] == '0' )
                fn = seqPref + std::string( "$F" ) + std::to_string( count ) +
                     seqSuf;
            else
        ...
        {
            if ( fn[longestStart] == '0' )
                fn = seqPref + std::string( "%0" ) + std::to_string( count ) +
                     std::string( 1, 'd' ) + seqSuf;
            else
...
bool file_sequence::extract_pound_pattern( void )
{
...
bool file_sequence::extract_dollarf_pattern( void )
{
    ...
    if ( base::find_unix_vars( pieces, vars, _pattern_prefix ) )
    {
        ...
        if ( frameVar != size_t( -1 ) )
        {
            ...
            if ( vpos == std::string::npos )
            {
                ...
                if ( vpos == std::string::npos )
                    throw_runtime(
                        "Invalid assumption about variable names looking for $F variable in {0} (var name '{1}')",
                        _pattern_prefix,
                        vars[frameVar] );
            }
...
bool file_sequence::extract_percentd_pattern( void )
{
    ...
    if ( start != std::string::npos )
    {
        ...
        // but who wants that?
        int count = 0;
        while ( tmpPos < _pattern_prefix.size() &&
                std::isdigit( _pattern_prefix[tmpPos] ) )
        {
        ...
        if ( _pattern_prefix[tmpPos] != 'd' )
            throw_runtime(
                "Expect a sequence marker of the form '%07d' or '%d' in {0}",
                _pattern_prefix );
        if ( count == 0 )
            count = 1;
...
void file_sequence::split_pattern( size_t start, size_t sz )
{
