...
public:
    ...
    image_buffer(
        ...
        int64_t                   ystride,
        int64_t                   offset = 0,
        base::endianness          e      = base::endianness::NATIVE )
        : _data( data )
        , _offset( offset )
        , _region( r )
        , _xstride( xstride )
    ...
    image_buffer( const image_buffer &interleaved, int64_t offset )
        ...
        , _xstride( interleaved._xstride )
        , _ystride( interleaved._ystride )
        ...
        , _unsigned( isunsign )
    {
        precondition( offset % 8 == 0, "offset should be multiple of 8 bits" );
        precondition(
            ( isfloat && !isunsign ) || ( !isfloat ), "float must be signed" );
    }
    ...
    void reset_position( int64_t x1, int64_t y1 )
    {
        _region.set_x( x1 );
        _region.set_y( y1 );
    }
    ...
    constexpr inline int64_t xstride_bytes( void ) const
    {
        return _xstride / 8;
    }
    constexpr inline int64_t ystride_bytes( void ) const
    {
        return _ystride / 8;
    }
    ...
    void *row( int64_t y )
    {
        ...
        data +=
            ( offset() + ( ( y - y1() ) >> _ysubsample_shift ) * _ystride ) / 8;
        return data;
    }
    ...
    template <typename T>
    static image_buffer
    simple_buffer( int64_t x1, int64_t y1, int64_t x2, int64_t y2 )
    {
        ...
        return image_buffer(
            ...
            ystride * 8,
            0,
            base::endianness::NATIVE,
            isf,
            isu );
    }

    template <typename T>
    static image_buffer simple_interleaved(
        int64_t          w,
        int64_t          h,
        int              chans,
        base::endianness endi = base::endianness::NATIVE )
    {
        rect barea{ 0, 0, w, h };
        // TBD: make each line sse (avx2) 256-bit (32-byte) aligned? we would have to
        // adjust opengl texturing and a few other places if we do that, and wouldn't be useable
        ...
        std::shared_ptr<void> vdata = std::shared_ptr<uint8_t>(
            ...
            base::array_deleter<uint8_t>() );
        return image_buffer(
            vdata, bits, barea, xstride * 8, ystride * 8, 0, endi, isf, isu );
    }

    static image_buffer full_plane(
        int64_t x1,
        int64_t y1,
        int64_t x2,
        int64_t y2,
        int8_t  bits,
        int8_t  xsubsamp_shift,
        int8_t  ysubsamp_shift,
        bool    isfloat,
        bool    isunsigned )
    {
        rect    barea = rect::from_points( x1, y1, x2, y2 );
        ...
        int64_t ystride =
            ( barea.width() * bytes ) >> int64_t( xsubsamp_shift );
        size_t sz = static_cast<size_t>(
            ystride * ( barea.height() >> int64_t( ysubsamp_shift ) ) );
