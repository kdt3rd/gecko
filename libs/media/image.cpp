...
image::image( const area_rect &area ) : _full_area( area ), _active_area( area )
{}
...
void image::color_state( const color::state &s ) { _color_state = s; }
    ...
    bool                       preferred_chunk ) const
{
    ...
    if ( interleaved() )
    {
        ...
        {
            precondition(
                pl._bits == _planes[p]._layout._bits,
                "unhandled mismatch of bits for interleaved buffer" );
            // TODO: fix this for 4:2:2 buffers!
            precondition(
                pl._ysubsample == _planes[p]._layout._ysubsample,
                "unhandled mismatch of y subsample for interleaved buffer" );
            precondition(
                pl._xsubsample == _planes[p]._layout._xsubsample,
                "unhandled mismatch of x subsample for interleaved buffer" );
...
void image::extract_image( std::vector<image_buffer> &planes )
{
    precondition(
        size() == planes.size(),
        "expect image buffer list same size as plane count" );
    ...
    for ( size_t p = 0; p != planes.size(); ++p )
    {
        ...
        if ( !ib.raw() )
            throw_runtime(
                "Invalid un-initialized image buffer for plane {0} ('{1}')",
                p,
                plane_name( p ) );

        if ( pl._bits != ib.bits() )
            throw_runtime(
                "Mis-match of bits for plane {0} ('{1}'): {2} vs {3}",
                p,
                plane_name( p ),
                pl._bits,
                ib.bits() );
