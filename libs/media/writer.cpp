...
void           initWriters( void ) { media::register_exr_writer(); }

static std::vector<std::shared_ptr<media::writer>> theWriters;
static std::map<std::string, std::shared_ptr<media::writer>>
    theWritersByExtension;
...
writer::writer( base::cstring n ) : _name( n ) {}
...
parameter_set
writer::parameters_by_ext( const base::uri &u, const std::string &opts )
{
    ...
    const std::string &                   forcewriter )
{
    ...
    if ( forcewriter.empty() )
    {
        ...
        if ( !writer )
            throw_runtime(
                "No writer found for output path {0}, please specify writer manually",
                u );
    }
    else
    {
        ...
        if ( !writer )
            throw_runtime(
                "Writer override set to {0}, but specified writer not found",
                forcewriter );
    }
...
void writer::register_writer( const std::shared_ptr<writer> &w )
{
    std::unique_lock<std::mutex> lk( theWriterMutex );
    ...
    {
        precondition(
            theWritersByExtension.find( e ) == theWritersByExtension.end(),
            "Extension {0} already handled by {1}",
            e,
            theWritersByExtension[e]->name() );
