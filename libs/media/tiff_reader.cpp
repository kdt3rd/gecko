...
static void tiff_warning_handler_ext(
    thandle_t sp, const char *p, const char *fmt, va_list args )
{}

static void tiff_error_handler_ext(
    thandle_t sp, const char *p, const char *fmt, va_list args )
{}
...
public:
    ...
    virtual void
    doWrite( int64_t, const std::vector<std::shared_ptr<image_frame>> & )
    {
        throw_logic( "reader asked to write a frame" );
    }
...
image_frame *tiff_read_track::doRead( int64_t f )
{
    ...
    if ( t )
    {
            ...
            switch ( bps )
            {
                ...
                case 8:
                    if ( sf == SAMPLEFORMAT_INT )
                        imgbuf = image_buffer::simple_buffer<int8_t>(
                            x1, y1, x2, y2 );
                    else
                        imgbuf = image_buffer::simple_buffer<uint8_t>(
                            x1, y1, x2, y2 );
                    break;
                case 16:
                    if ( sf == SAMPLEFORMAT_INT )
                        imgbuf = image_buffer::simple_buffer<int16_t>(
                            x1, y1, x2, y2 );
                    else
                        imgbuf = image_buffer::simple_buffer<uint16_t>(
                            x1, y1, x2, y2 );
                    break;
                case 32:
                    if ( sf == SAMPLEFORMAT_UINT )
                        imgbuf = image_buffer::simple_buffer<uint32_t>(
                            x1, y1, x2, y2 );
                    else if ( sf == SAMPLEFORMAT_INT )
                        imgbuf = image_buffer::simple_buffer<int32_t>(
                            x1, y1, x2, y2 );
                    else // void or ieeefp
                        imgbuf = image_buffer::simple_buffer<float>(
                            x1, y1, x2, y2 );
                    break;
                case 64:
                    if ( sf == SAMPLEFORMAT_UINT )
                        imgbuf = image_buffer::simple_buffer<uint64_t>(
                            x1, y1, x2, y2 );
                    else if ( sf == SAMPLEFORMAT_INT )
                        imgbuf = image_buffer::simple_buffer<int64_t>(
                            x1, y1, x2, y2 );
                    else // void or ieeefp
                        imgbuf = image_buffer::simple_buffer<double>(
                            x1, y1, x2, y2 );
                    break;
                default: break;
            }
        ...
        else
        {
            ...
            if ( PLANARCONFIG_SEPARATE == isP )
            {
                ...
                {
                    uint8_t *dptr =
                        reinterpret_cast<uint8_t *>( ret->at( s ).data() );
                    precondition(
                        ret->at( s ).ystride_bytes() == ssize,
                        "invalid line stride assumption about scanline size" );
            ...
            {
                std::unique_ptr<uint8_t[]> tmpbuf = std::unique_ptr<uint8_t[]>(
                    new uint8_t[TIFFScanlineSize( t )] );
                for ( uint32_t row = 0; row < sizeY; ++row )
                {
                    ...
                    for ( uint16_t s = 0; s < samps; ++s )
                    {
                        ...
                        uint8_t *     dptr =
                            reinterpret_cast<uint8_t *>( ib.data() );
                        ...
                        switch ( ib.bits() )
                        {
                            ...
                            {
                                uint16_t *dst16 =
                                    reinterpret_cast<uint16_t *>( dptr );
                                const uint16_t *src16 =
                                    reinterpret_cast<const uint16_t *>( sptr );
                                for ( uint32_t x = 0; x < sizeX; ++x )
                            ...
                            {
                                uint32_t *dst32 =
                                    reinterpret_cast<uint32_t *>( dptr );
                                const uint32_t *src32 =
                                    reinterpret_cast<const uint32_t *>( sptr );
                                for ( uint32_t x = 0; x < sizeX; ++x )
                            ...
                            case 64:
                            {
                                ...
                                const uint64_t *src64 =
                                    reinterpret_cast<const uint64_t *>( sptr );
                                for ( uint32_t x = 0; x < sizeX; ++x )
                                    dst64[x] = src64[x * samps + s];
                                break;
                            }
                            default: throw_not_yet();
                        }
                    }
                }
            }
...
{
public:
    TIFFReader( void ) : file_per_sample_reader( "TIFF" )
    {
...
container TIFFReader::create( const base::uri &u, const metadata & )
{
    container result;
    ...
    result.add_track(
        std::make_shared<tiff_read_track>( start, last, std::move( fseq ) ) );
