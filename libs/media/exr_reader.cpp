...
public:
    ...
    bool read( char c[], int n ) override
    {
        ...
        catch ( ... )
        {
            std::throw_with_nested( std::runtime_error(
                "Error reading from OpenEXR stream: " + _stream.uri() ) );
        }
    }
    ...
    EXR::Int64 tellg( void ) override
    {
        ...
        {
            std::throw_with_nested( std::runtime_error(
                "Error telling position on OpenEXR stream: " +
                _stream.uri() ) );
        }
    }
    ...
    void seekg( EXR::Int64 pos ) override
    {
        ...
        {
            std::throw_with_nested( std::runtime_error(
                "Error seeking to position on OpenEXR stream: " +
                _stream.uri() ) );
        }
    }
    ...
    void clear( void ) override
    {
        ...
        {
            std::throw_with_nested( std::runtime_error(
                "Error clearing on OpenEXR stream: " + _stream.uri() ) );
        }
    }
...
plane_layout to_layout( const EXR::Channel &c )
{
    ...
    switch ( c.type )
    {
        ...
        default: throw_runtime( "Unknown pixel type in EXR file" );
    }

    return pl;
}

template <typename M, typename V> inline metadata_value create_mv( const V &x )
{
    return M::make( x );
}

template <typename M> inline metadata_value create_mv( const EXR::LineOrder &l )
{
    switch ( l )
    {
    ...
    throw_logic(
        "Unknown EXR line order specification: {0}", static_cast<int>( l ) );
}
...
inline metadata_value create_mv( const EXR::Compression &c )
{
    ...
    throw_logic(
        "Unknown EXR compression specification: {0}", static_cast<int>( c ) );
}

template <typename M> inline metadata_value create_mv( const EXR::Envmap &e )
{
    switch ( e )
    {
    ...
    throw_logic(
        "Unknown EXR environment mapping specification: {0}",
        static_cast<int>( e ) );
}
...
inline metadata_value create_array_val( const V *p, size_t N )
{
    ...
    precondition(
        N == arr.size(),
        "unexpected mismatch in array size and argument size" );
...
template <typename M, typename V>
inline metadata_value
create_mv( const IMATH_NAMESPACE::Box<IMATH_NAMESPACE::Vec2<V>> &b )
{
...
template <typename M> inline metadata_value create_mv( const EXR::Rational &r )
{
    return M::make( std::make_pair( r.n, r.d ) );
}

template <typename M> inline metadata_value create_mv( const EXR::TimeCode &tc )
{
...
template <typename M> inline metadata_value create_mv( const EXR::KeyCode &kc )
{
    int32_t vals[7] = { kc.filmMfcCode(),  kc.filmType(),   kc.prefix(),
                        kc.count(),        kc.perfOffset(), kc.perfsPerFrame(),
                        kc.perfsPerCount() };
...
inline void cast_and_set( M &md, const char *name, const EXR::Attribute &attr )
{
    ...
    if ( eAttr )
        ...
        throw_runtime(
            "Unable to convert EXR attribute '{0}' to correct type (says '{1}')",
            name,
            attr.typeName() );
}
...
{
    return aname == "dataWindow" || aname == "displayWindow" ||
           aname == "envmap" || aname == "tiles" || aname == "type";
}
...
static void fill_metadata( M &md, color::state &cs, const EXR::Header &header )
{
    for ( auto attri = header.begin(); attri != header.end(); ++attri )
    {
        ...
        if ( type == EXR::CompressionAttribute::staticTypeName() )
            cast_and_set<EXR::CompressionAttribute, meta_string_t>(
                md, attrname, attrib );
        else if ( type == EXR::LineOrderAttribute::staticTypeName() )
            cast_and_set<EXR::LineOrderAttribute, meta_int32_t>(
                md, attrname, attrib );
        else if ( type == EXR::StringAttribute::staticTypeName() )
            cast_and_set<EXR::StringAttribute, meta_string_t>(
                md, attrname, attrib );
        else if ( type == EXR::FloatAttribute::staticTypeName() )
        {
            if ( aname == "whiteLuminance" )
            {
                const EXR::FloatAttribute *eAttr =
                    dynamic_cast<const EXR::FloatAttribute *>( &attrib );
                if ( eAttr )
                {
            ...
            else if ( aname == "pixelAspectRatio" )
            {
                ...
                cast_and_set<EXR::FloatAttribute, meta_float32_t>(
                    md, attrname, attrib );
        }
        else if ( type == EXR::M33fAttribute::staticTypeName() )
            cast_and_set<EXR::M33fAttribute, meta_m33f_t>(
                md, attrname, attrib );
        else if ( type == EXR::M44fAttribute::staticTypeName() )
            cast_and_set<EXR::M44fAttribute, meta_m44f_t>(
                md, attrname, attrib );
        else if ( type == EXR::V3fAttribute::staticTypeName() )
            cast_and_set<EXR::V3fAttribute, meta_vec3f_t>(
                md, attrname, attrib );
        else if ( type == EXR::V3iAttribute::staticTypeName() )
            cast_and_set<EXR::V3iAttribute, meta_vec3i_t>(
                md, attrname, attrib );
        else if ( type == EXR::V2fAttribute::staticTypeName() )
            cast_and_set<EXR::V2fAttribute, meta_vec2f_t>(
                md, attrname, attrib );
        else if ( type == EXR::V2iAttribute::staticTypeName() )
            cast_and_set<EXR::V2iAttribute, meta_vec2i_t>(
                md, attrname, attrib );
        else if ( type == EXR::IntAttribute::staticTypeName() )
            cast_and_set<EXR::IntAttribute, meta_int32_t>(
                md, attrname, attrib );
        else if ( type == EXR::StringVectorAttribute::staticTypeName() )
            cast_and_set<EXR::StringVectorAttribute, meta_stringlist_t>(
                md, attrname, attrib );
        else if ( type == EXR::DoubleAttribute::staticTypeName() )
            cast_and_set<EXR::DoubleAttribute, meta_float64_t>(
                md, attrname, attrib );
        else if ( type == EXR::V2dAttribute::staticTypeName() )
            cast_and_set<EXR::V2dAttribute, meta_vec2d_t>(
                md, attrname, attrib );
        else if ( type == EXR::V3dAttribute::staticTypeName() )
            cast_and_set<EXR::V3dAttribute, meta_vec3d_t>(
                md, attrname, attrib );
        else if ( type == EXR::M33dAttribute::staticTypeName() )
            cast_and_set<EXR::M33dAttribute, meta_m33d_t>(
                md, attrname, attrib );
        else if ( type == EXR::M44dAttribute::staticTypeName() )
            cast_and_set<EXR::M44dAttribute, meta_m44d_t>(
                md, attrname, attrib );
        else if ( type == EXR::Box2iAttribute::staticTypeName() )
            cast_and_set<EXR::Box2iAttribute, meta_recti_t>(
                md, attrname, attrib );
        else if ( type == EXR::Box2fAttribute::staticTypeName() )
            cast_and_set<EXR::Box2fAttribute, meta_rectf_t>(
                md, attrname, attrib );
        ...
        //				cast_and_set<EXR::FloatVectorAttribute, meta_float_list_t>( md, attrname, attrib );
            ...
            cast_and_set<EXR::TimeCodeAttribute, meta_smpte_timecode_t>(
                md, attrname, attrib );
        else if ( type == EXR::KeyCodeAttribute::staticTypeName() )
            cast_and_set<EXR::KeyCodeAttribute, meta_keycode_t>(
                md, attrname, attrib );
        else if ( type == EXR::ChromaticitiesAttribute::staticTypeName() )
        {
            if ( aname == "chromaticities" )
            {
                const EXR::ChromaticitiesAttribute *eAttr =
                    dynamic_cast<const EXR::ChromaticitiesAttribute *>(
                        &attrib );
                if ( eAttr )
                {
                    auto &cx = eAttr->value();
                ...
                cast_and_set<
                    ...
                    meta_chromaticities_t>( md, attrname, attrib );
        }
        else if ( type == EXR::RationalAttribute::staticTypeName() )
            cast_and_set<EXR::RationalAttribute, meta_rational32_t>(
                md, attrname, attrib );
        else if ( type == EXR::EnvmapAttribute::staticTypeName() )
            cast_and_set<EXR::EnvmapAttribute, meta_string_t>(
                md, attrname, attrib );
        else
            std::cout << "EXR: Unknown attribute type " << type << ": "
                      << attrname << std::endl;
    }
}
...
public:
        ...
        , _full_plane_names( std::move( pfullnames ) )
    {
        ...
        set_active_area( area_rect::from_points(
            data.min.x, data.min.y, data.max.x, data.max.y ) );
        set_full_area( area_rect::from_points(
            disp.min.x, disp.min.y, disp.max.x, disp.max.y ) );
...
protected:
    bool storage_interleaved( void ) const override { return false; }

    void fill_plane( size_t plane, image_buffer &buffer ) override
    {
        if ( plane >= _plane_names.size() )
            throw_runtime(
                "Attempt to access plane {0}, beyond the end of the EXR image ({1} planes)",
                plane,
                _plane_names.size() );

        const EXR::Channel &imfchan =
            _header.channels()[_full_plane_names[plane]];
        ...
        switch ( imfchan.type )
        {
            case EXR::UINT: bytes = sizeof( uint32_t ); break;
            ...
            default:
                throw_logic(
                    "Unknown OpenEXR pixel type {0}",
                    static_cast<int>( imfchan.type ) );
        }

        if ( buffer.is_floating() != flt ||
             static_cast<size_t>( buffer.bits() ) != ( bytes * 8 ) )
            throw_runtime(
                "Attempt to access EXR image with wrong buffer type" );
        ...
        size_t scanlinebytes =
            static_cast<size_t>( dataWin.max.x - dataWin.min.x + 1 ) * bytes;
    ...
    void fill_image( std::vector<image_buffer> &planes ) override
    {
        ...
        for ( auto i = _header.channels().begin();
              i != _header.channels().end();
              ++i, ++pIdx )
        {
            // TODO: handle subsampling
            ...
            if ( pIdx == 0 )
                ...
                throw_runtime(
                    "Image reads must request the same region for all planes" );

            if ( buffer.is_floating() != flt )
                throw_runtime(
                    "Buffer and file differ in floating point interpretation of data" );

            if ( static_cast<size_t>( buffer.bits() ) != bytes * 8 )
                throw_runtime(
                    "Mismatch in bits for provided memory buffer and on-disk plane definition" );

            precondition(
                buffer.raw(),
                "Expect valid memory buffer to be provided to read" );
            ...
            fbuf.insert(
                _full_plane_names[pIdx],
                EXR::Slice(
                    type,
                    data,
                    buffer.xstride_bytes(),
                    buffer.ystride_bytes() ) );
        }
...
public:
        ...
        , _full_chan_names( std::move( chanfullnames ) )
    {
        ...
        const auto &disp = header.displayWindow();
        const auto &data = header.displayWindow();
        set_active_area( area_rect(
            data.min.x,
            data.min.y,
            data.max.x - data.min.x + 1,
            data.max.y - data.min.y + 1 ) );
        set_full_area( area_rect(
            disp.min.x,
            disp.min.y,
...
protected:
    ...
    void fill_plane_line(
        size_t plane, int64_t y, std::vector<float> &buffer ) override
    {
        throw_not_yet();
    }
...
public:
    exr_frame(
        const std::shared_ptr<base::file_system> &fs,
        const base::uri &                         frm,
        int64_t                                   num )
        : _uri( frm )
        , _stream( std::make_shared<exr_istream>( fs->open_read( frm ) ) )
    {
        ...
        for ( int p = 0; p < numP; ++p )
        {
            ...
            layer &l = register_layer(
                header.hasName() ? header.name() : std::string() );
            for ( auto c = chans.begin(); c != chans.end(); ++c )
            {
                ...
                {
                    std::string curview =
                        EXR::viewFromChannelName( fullname, *multiviews );
                    ...
                    if ( curview != viewname )
                    {
                        if ( !channames.empty() )
                        {
                            ...
                            {
                                v.store( std::make_shared<exr_image>(
                                    _file,
                                    _stream,
                                    0,
                                    header,
                            ...
                            {
                                v.store( std::make_shared<exr_deep>(
                                    _file,
                                    _stream,
                                    0,
                                    header,
                ...
                channames.emplace_back(
                    name.empty() ? std::string( fullname )
                                 : std::move( name ) );
...
public:
    ...
    {
        std::unique_ptr<exr_frame> ret{ new exr_frame(
...
{
public:
    OpenEXRReader( void ) : file_per_sample_reader( "OpenEXR" )
    {
    ...
    container create( const base::uri &u, const parameter_set &params ) override
    {
        ...
        result.add_track( std::make_shared<exr_read_track>(
            std::move( fseq ), start, last ) );
