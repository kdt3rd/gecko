...
public:
    ...
    metadata_type(
        const char *            name,
        metadata_to_utf8_func   toFunc,
        metadata_from_utf8_func fromFunc )
...
public:
        ...
        metadata_from_utf8_func deserialize )
    {
        ...
        if ( i != _mtype_name_to_id.end() )
        {
            ...
            if ( mtype._serializer == serialize &&
                 mtype._deserializer == deserialize )
                return i->second;
            throw_runtime(
                "Attempt to register duplicate metadata type name {0} with different serialization functions",
                name );
        }
        uint32_t id = _mtype_list.size();
        _mtype_list.emplace_back(
            std::make_shared<metadata_type>( name, serialize, deserialize ) );
...
private:
        ...
        metadata_from_utf8_func fromFunc = nullptr )
    {
        uint32_t id = _mtype_list.size();
        _mtype_list.emplace_back(
            std::make_shared<metadata_type>( name, toFunc, fromFunc ) );
        _mtype_name_to_id[base::cstring( _mtype_list.back()->name() )] = id;
    }
