...
using metadata_from_utf8_func =
    void *(*)( uint32_t meta_type, const char *meta_string );
...
struct metadata_value
{
    ...
    template <typename U> inline void assign( uint32_t t, U &&v )
    {
        _type    = t;
        _storage = std::forward<U>( v );
    }

    template <typename U> inline U as( void ) const
    {
    ...
    template <typename U> inline const U &as_ref( void ) const
    {
        // ADL does not work because of the template specialization. rather than
        // put a blank any_cast in the global namespace, just use the particular
        // namespace...
...
template <typename T> class meta_size_and_value
{
public:
    using value_type = T;
...
template <uint32_t tagval, typename T> struct meta_value
{
    ...
    static inline metadata_value make( value_type v )
    {
        return metadata_value(
            tag, base::make_any<value_type>( std::move( v ) ) );
    }
};

template <uint32_t tagval> struct meta_value<tagval, void>
{
    static const uint32_t tag      = tagval;
...
using meta_signed_rational32_t =
    meta_value<meta_signed_rational32_tag, std::pair<int32_t, int32_t>>;
using meta_unsigned_rational32_t =
    meta_value<meta_unsigned_rational32_tag, std::pair<uint32_t, uint32_t>>;
using meta_rational32_t =
    meta_value<meta_rational32_tag, std::pair<int32_t, uint32_t>>;
...
using meta_stringlist_t =
    meta_value<meta_ascii_stringlist_tag, std::vector<std::string>>;
using meta_utf8_string_t =
    meta_value<meta_utf8_string_tag, std::vector<uint8_t>>;
using meta_utf16_string_t =
    meta_value<meta_utf16_string_tag, std::vector<uint16_t>>;
using meta_utf32_string_t =
    meta_value<meta_utf32_string_tag, std::vector<uint32_t>>;
...
using meta_m44f_t =
    meta_value<meta_matrix44_float32_tag, std::array<float, 16>>;
using meta_m33d_t =
    meta_value<meta_matrix33_float64_tag, std::array<double, 9>>;
using meta_m44d_t =
    meta_value<meta_matrix44_float64_tag, std::array<double, 16>>;

using meta_float_list_t = meta_value<meta_float32_list_tag, std::vector<float>>;

using meta_chromaticities_t =
    meta_value<meta_chromaticities_tag, std::array<float, 8>>;

using meta_keycode_t = meta_value<meta_keycode_tag, std::array<int32_t, 7>>;
using meta_smpte_timecode_t =
    meta_value<meta_timecode_tag, std::pair<uint32_t, uint32_t>>;
