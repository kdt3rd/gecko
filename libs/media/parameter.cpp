...
namespace
{
static const char *     ptype_names[] = { "int", "string", "number", "bool" };
static constexpr size_t k_ptype_count =
    sizeof( ptype_names ) / sizeof( const char * );
...
parameter_value::parameter_value( void ) : _ptype( PARAM_UNKNOWN ) {}
...
parameter_value::parameter_value( int val ) : _ptype( PARAM_INT ), _ival( val )
{}
...
parameter_value::parameter_value( base::cstring val ) : _ptype( PARAM_STRING )
{
...
parameter_value::~parameter_value( void ) { destroy(); }
...
parameter_value &parameter_value::operator=( const parameter_value &o )
{
    if ( this != &o )
    {
        destroy();
...
parameter_value::parameter_value( parameter_value &&o ) : _ptype( o._ptype )
{
    switch ( _ptype )
    {
        case PARAM_BOOL: _bval = o._bval; break;
        ...
        case PARAM_FLOAT: _dval = o._dval; break;
        case PARAM_STRING:
            new ( &_sval ) std::string( std::move( o._sval ) );
            break;
        case PARAM_UNKNOWN: break;
    }
}
...
parameter_value &parameter_value::operator=( parameter_value &&o )
{
    if ( this != &o )
    {
        destroy();
...
bool parameter_value::as_bool( void ) const
{
    ...
    throw_logic(
        "Request for bool from non-bool / convertible parameter value" );
}
...
int64_t parameter_value::as_int( void ) const
{
    ...
    if ( _ptype != PARAM_INT )
        throw_logic(
            "Request for int from non-int / convertible parameter value" );
    return _ival;
}
...
void parameter_value::destroy( void )
{
    if ( _ptype == PARAM_STRING )
        _sval.~basic_string();
}
...
std::ostream &operator<<( std::ostream &os, const parameter_value &pv )
{
    ...
    {
        case PARAM_BOOL:
            os << std::boolalpha << pv.as_bool() << std::noboolalpha;
            break;
        case PARAM_INT:
            os << pv.as_int();
            break;
...
parameter_definition::parameter_definition( base::cstring name, bool defVal )
    : _name( name ), _type( PARAM_BOOL ), _def_val( defVal )
{}

////////////////////////////////////////

parameter_definition::parameter_definition(
    base::cstring name, int64_t minVal, int64_t maxVal, int64_t defVal )
    : _name( name )
    , _type( PARAM_INT )
...
parameter_definition::parameter_definition(
    base::cstring name, double minVal, double maxVal, double defVal )
    : _name( name )
    , _type( PARAM_FLOAT )
    , _allowed_vals( std::make_pair( minVal, maxVal ) )
    , _def_val( defVal )
...
parameter_definition::parameter_definition(
    base::cstring name, std::vector<std::string> vals, base::cstring defVal )
    : _name( name )
    , _type( PARAM_STRING )
...
{
    precondition(
        _type == PARAM_INT && _allowed_vals.valid() &&
            _allowed_vals.is<int_range>(),
        "querying range for integers, but not an integer type or doesn't have a range" );
...
{
    precondition(
        _type == PARAM_FLOAT && _allowed_vals.valid() &&
            _allowed_vals.is<float_range>(),
        "querying range for floats, but not an float type or doesn't have a range" );
...
{
    precondition(
        _type == PARAM_STRING && _allowed_vals.valid() &&
            _allowed_vals.is<string_list>(),
        "querying allowed string values, but not a string parameter type or doesn't have a set of values" );
...
parameter_value parameter_definition::default_value( void ) const
{
    return _def_val;
}
...
parameter_set initialize_parameters(
    const parameter_definitions &params, const std::string &option )
{
    parameter_set r;
    for ( auto &p: params )
        ...
        for ( auto &o: opts )
        {
            ...
            if ( nv.size() == 2 )
            {
                ...
                if ( ri == r.end() )
                {
                    ...
                    throw_runtime(
                        "Invalid option '{0}' requested, valid options are:\n{1}",
                        nv[0],
                        errstr.str() );
                }
            }
                ...
                throw_runtime(
                    "Invalid media parameter option '{0}' in '{1}', unable to parse name=value",
                    o,
                    option );
        }
    }
