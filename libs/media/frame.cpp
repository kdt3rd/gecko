...
{
	return ( layerIdx < _layers.size() ) ? _layers[layerIdx].has_view( viewIdx ) : false;
}
...
{
	return ( layerIdx < _layers.size() ) ? _layers[layerIdx].has_view( v ) : false;
}
...
std::shared_ptr<image> frame::find_image( size_t layerIdx, size_t viewIdx ) const
{
	if ( layerIdx < _layers.size() )
		return static_cast< std::shared_ptr<image> >( _layers[layerIdx][viewIdx] );
	return std::shared_ptr<image>();
}

////////////////////////////////////////

std::shared_ptr<image> frame::find_image( size_t layerIdx, base::cstring v ) const
{
	if ( layerIdx < _layers.size() )
	{
		if ( v.empty() )
			return static_cast< std::shared_ptr<image> >( _layers[layerIdx].default_view() );
...
std::shared_ptr<image> frame::find_image( base::cstring layer, size_t viewIdx ) const
{
	if ( layer.empty() )
...
std::shared_ptr<image> frame::find_image( base::cstring layer, base::cstring v ) const
{
	if ( layer.empty() )
...
std::shared_ptr<data> frame::find_data( size_t layerIdx, base::cstring v ) const
{
	...
	{
		if ( v.empty() )
			return static_cast< std::shared_ptr<data> >( _layers[layerIdx].default_view() );
...
std::shared_ptr<data> frame::find_data( base::cstring layer, size_t viewIdx ) const
{
	if ( layer.empty() )
...
std::shared_ptr<data> frame::find_data( base::cstring layer, base::cstring v ) const
{
	if ( layer.empty() )
...
frame::named_image_list
frame::images_for_view( base::cstring v ) const
{
