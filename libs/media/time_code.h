...
public:
    ...
    inline void
    set( int hr, int mins, int sec, int frm, const sample_rate &sr );
    ...
    std::string
         to_string( const sample_rate &sr, bool edlNotation = false ) const;
    void from_string(
        const base::cstring &cs,
...
inline bool time_code::valid( void ) const
{
    return hours() >= 0 && minutes() >= 0 && seconds() >= 0 && frames() >= 0;
}

inline void time_code::set( int hr, int mins, int sec, int frm )
{
    _hours   = hr;
    _minutes = mins;
    _seconds = sec;
...
inline void               time_code::hours( int hr ) { _hours = hr; }
inline int                time_code::hours( void ) const { return _hours; }
inline void               time_code::minutes( int mins ) { _minutes = mins; }
inline int                time_code::minutes( void ) const { return _minutes; }
inline void               time_code::seconds( int sec ) { _seconds = sec; }
inline int                time_code::seconds( void ) const { return _seconds; }
inline void               time_code::frames( int fr ) { _frames = fr; }
inline int                time_code::frames( void ) const { return _frames; }
inline const sample_rate &time_code::parse_rate( void ) const { return _rate; }

////////////////////////////////////////

inline sample time_code::to_sample( const sample_rate &sr ) const
{
    return sample( to_frame( sr ), sr );
}
inline void time_code::from_sample( const sample &s )
{
