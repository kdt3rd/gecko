...
public:
    ...
    std::shared_ptr<frame> operator[]( int64_t f ) { return at( f ); }
    ...
    inline void
    store( int64_t f, const std::shared_ptr<frame> &frm, base::allocator &a )
    {
        if ( frm )
            doWrite( f, *frm, a );
    }
...
protected:
    ...
    virtual void
    doWrite( int64_t offset, const frame &sd, base::allocator &a ) = 0;
