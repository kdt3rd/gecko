...
public:
    ...
    template <size_t N>
    fourcc( const char ( &c )[N] ) : _char{ c[0], c[1], c[2], c[3] }
    {
    ...
    /// @brief Set FourCC.
    template <size_t N> void set( const char ( &c )[N] )
    {
        static_assert( N == 5, "fourcc must be 4 characters" );
        _char[0] = c[0];
        _char[1] = c[1];
    ...
    void set( uint32_t v ) { _code = v; }

    /// @brief Compare FourCC with literal string.
    template <size_t N> bool operator==( const char ( &c )[N] ) const
    {
        static_assert( N == 5, "fourcc must be 4 characters" );
        return c[0] == _char[0] && c[1] == _char[1] && c[2] == _char[2] &&
               c[3] == _char[3];
    }

    /// @brief Compare FourCC for equality.
    bool operator==( const fourcc &c ) const { return _code == c._code; }
    ...
    char operator[]( size_t i ) const { return _char[i]; }

    /// @brief Get the ith character.
    char &operator[]( size_t i ) { return _char[i]; }
