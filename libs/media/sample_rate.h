...
public:
    ...
    inline constexpr sample_rate(
        int64_t n, int64_t d = 1, bool realtime = false )
...
inline constexpr bool operator<( const sample_rate &a, const sample_rate &b )
{
    return (
        a.ratio() < b.ratio()
            ? true
            : ( a.ratio() == b.ratio() ? a.realtime() < b.realtime()
                                       : false ) );
}
...
inline constexpr bool sample_rate::valid( void ) const
{
    return _ratio.valid();
}

inline constexpr bool sample_rate::realtime( void ) const { return _realtime; }
...
inline constexpr int64_t sample_rate::numerator( void ) const
{
    return _ratio.numerator();
}

inline constexpr int64_t sample_rate::denominator( void ) const
{
