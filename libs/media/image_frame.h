...
public:
    ...
    size_t add_channel( std::string n, image_buffer i )
    {
        precondition(
            i.width() == width(),
            "invalid channel width {0} vs {1}",
            i.width(),
            width() );
        precondition(
            i.height() == height(),
            "invalid channel height {0} vs {1}",
            i.height(),
            height() );
    ...
    int64_t width( void ) const { return _area.width(); }

    int64_t height( void ) const { return _area.height(); }

    size_t size( void ) const { return _channels.size(); }
    ...
    image_buffer &      operator[]( size_t i ) { return _channels.at( i ); }
    const image_buffer &operator[]( size_t i ) const
    {
        return _channels.at( i );
    }

    image_buffer &      at( size_t i ) { return _channels.at( i ); }
    const image_buffer &at( size_t i ) const { return _channels.at( i ); }
    ...
    size_t channel( const std::string &n ) const { return _names.at( n ); }
    ...
    inline void set_render_data( base::cstring name, metadata_value v )
    {
        _renderdata[name] = std::move( v );
    }
    inline const metadata &render_data( void ) const { return _renderdata; }

private:
    bool has_channels( void ) { return true; }
