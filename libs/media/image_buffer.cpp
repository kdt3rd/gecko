...
inline void unconvert_pel( uint8_t &d, float x )
{
    d = static_cast<uint8_t>(
        std::max( 0.F, std::min( 255.F, x * 255.0F + 0.5F ) ) );
}
...
inline void unconvert_pel( uint16_t &d, float x )
{
    d = static_cast<uint16_t>(
        std::max( 0.F, std::min( 65535.F, x * 65535.0F + 0.5F ) ) );
}

inline float convert_pel( base::half x ) { return float( x ); }

inline void unconvert_pel( base::half &d, float x ) { d = x; }

inline float convert_pel( float x ) { return x; }

inline void unconvert_pel( float &d, float x ) { d = x; }

inline float convert_pel( double x ) { return static_cast<float>( x ); }
...
void image_buffer::get_scanline( int64_t y, float *line, int64_t stride ) const
{
    ...
    throw_logic(
        "image_buffer::get_scanline not yet implemented: {0} {1} {2} bits",
        uns,
        flt,
        _bits );
}
...
void image_buffer::set_scanline( int64_t y, const float *line, int64_t stride )
{
    ...
    const char *flt = ( _floating ? "float" : "integer" );
    throw_logic(
        "image_buffer::set_scanline not yet implemented: {0} {1} {2} bits",
        uns,
        flt,
        _bits );
}
...
void image_buffer::get_scanline_u8(
    int64_t y, float *line, int64_t stride ) const
{
    ...
    for ( int64_t x = 0, W = width(); x < W; ++x )
    {
        const uint8_t *curData =
            data + ( ( x >> _xsubsample_shift ) * _xstride ) / 8;
...
void image_buffer::get_scanline_u16(
    int64_t y, float *line, int64_t stride ) const
{
    ...
    if ( _endian == base::endianness::NATIVE )
    {
        for ( int64_t x = 0, W = width(); x < W; ++x )
        {
            const uint16_t *curData =
                data + ( ( x >> _xsubsample_shift ) * _xstride ) / 16;
            line[x * stride] = convert_pel( *curData );
        }
    }
    else
...
void image_buffer::get_scanline_f16(
    int64_t y, float *line, int64_t stride ) const
{
    ...
    else
    {
        ...
        {
            const uint16_t *curData =
                udata + ( ( x >> _xsubsample_shift ) * _xstride ) / 16;
            line[x * stride] = convert_pel(
                base::half( base::half::binary, bswap_16( *curData ) ) );
        }
    }
}
...
void image_buffer::get_scanline_f32(
    int64_t y, float *line, int64_t stride ) const
{
    ...
    if ( _endian == base::endianness::NATIVE )
    {
        ...
        {
            const float *curData =
                data + ( ( x >> _xsubsample_shift ) * _xstride ) / 32;
            line[x * stride] = convert_pel( *curData );
        }
    }
    else
    {
        ...
        for ( int64_t x = 0, W = width(); x < W; ++x )
        {
            const uint32_t *curData =
                udata + ( ( x >> _xsubsample_shift ) * _xstride ) / 32;
            union
            {
                uint32_t u;
                float    f;
...
void image_buffer::get_scanline_f64(
    int64_t y, float *line, int64_t stride ) const
{
    ...
    if ( _endian == base::endianness::NATIVE )
    {
        for ( int64_t x = 0, W = width(); x < W; ++x )
        {
            const double *curData =
                data + ( ( x >> _xsubsample_shift ) * _xstride ) / 64;
            line[x * stride] = convert_pel( *curData );
        }
    }
    else
    {
        ...
        for ( int64_t x = 0, W = width(); x < W; ++x )
        {
            const uint64_t *curData =
                udata + ( ( x >> _xsubsample_shift ) * _xstride ) / 64;
            union
            {
                uint64_t u;
                double   f;
...
void image_buffer::set_scanline_u8(
    int64_t y, const float *line, int64_t stride )
{
...
void image_buffer::set_scanline_u16(
    int64_t y, const float *line, int64_t stride )
{
...
void image_buffer::set_scanline_f16(
    int64_t y, const float *line, int64_t stride )
{
...
void image_buffer::set_scanline_f32(
    int64_t y, const float *line, int64_t stride )
{
...
void image_buffer::set_scanline_f64(
    int64_t y, const float *line, int64_t stride )
{
