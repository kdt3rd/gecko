...
int64_t time_code::to_frame( const sample_rate &sr ) const
{
    if ( !valid() )
        throw std::logic_error(
            "Attempt to access uninitialized time_code object" );

    int64_t secs = _hours * 3600 + _minutes * 60 + _seconds;
    if ( sr.is_drop_frame() )
    {
        if ( sr.ratio().numerator() == 30000 )
            return _frames + secs * 30 -
                   ( _hours * 60 + _minutes - _hours * 6 - _minutes / 10 ) * 2;
        else
            return _frames + secs * 60 -
                   ( _hours * 60 + _minutes - _hours * 6 - _minutes / 10 ) * 4;
    }

    // sample_rate is in n / second
    // so use seconds and add frames, then round
    return base::ratio(
               secs * sr.ratio().numerator() +
                   _frames * sr.ratio().denominator(),
               sr.ratio().denominator(),
               false )
        .round();
}
...
void time_code::from_frame( int64_t f, const sample_rate &sr )
{
    if ( sr.is_drop_frame() )
    {
        // for 30:
...
void time_code::from_smpte( uint32_t tc )
{
...
uint32_t time_code::to_smpte( void ) const
{
    uint32_t hrs =
        static_cast<uint32_t>( ( ( _hours / 10 ) << 4 ) | ( _hours % 10 ) );
    uint32_t min =
        static_cast<uint32_t>( ( ( _minutes / 10 ) << 4 ) | ( _minutes % 10 ) );
    uint32_t sec =
        static_cast<uint32_t>( ( ( _seconds / 10 ) << 4 ) | ( _seconds % 10 ) );
    uint32_t frm =
        static_cast<uint32_t>( ( ( _frames / 10 ) << 4 ) | ( _frames % 10 ) );
    return ( hrs << 24 ) | ( min << 16 ) | ( sec << 8 ) | frm;
}

////////////////////////////////////////

void time_code::from_smpte_rp188(
    uint32_t low, uint32_t high, const sample_rate &sr )
{
...
time_code::to_smpte_rp188( const sample_rate &sr ) const
{
    ...
    if ( sr.to_number() > 39.0 )
        low = static_cast<uint32_t>(
            ( ( _frames % 2 ) << 27 ) | ( ( _frames / 20 ) << 8 ) |
            ( ( _frames / 2 ) % 10 ) );
    else
        low = static_cast<uint32_t>(
            ( ( _frames / 10 ) << 8 ) | ( _frames % 10 ) );
    low |= static_cast<uint32_t>(
        ( ( _seconds / 10 ) << 24 ) | ( ( _seconds % 10 ) << 16 ) );
    high =
        static_cast<uint32_t>( ( ( _minutes / 10 ) << 8 ) | ( _minutes % 10 ) );
    high |= static_cast<uint32_t>(
        ( ( _hours / 10 ) << 24 ) | ( ( _hours % 10 ) << 16 ) );
    ...
    if ( edlNotation )
    {
        ...
        if ( frate > 60.0 )
            throw std::runtime_error(
                "EDL notation not supported from frame rates in excess of 60FPS" );
...
void time_code::from_string(
    const base::cstring &cs, const sample_rate &sr, bool edlNotation )
{
    _hours     = 0;
    _minutes   = 0;
    _seconds   = 0;
    ...
    if ( cs[pos] == ':' || cs[pos] == ';' || cs[pos] == '.' )
    {
        ...
        if ( ( pos + 2 ) >= cs.size() )
            throw std::runtime_error(
                "Found hours:minutes separator, but expecting 2 digits for minutes" );
        _minutes =
            base::from_digit( cs[pos] ) * 10 + base::from_digit( cs[pos + 1] );
        pos += 2;
    }
    else
        throw std::runtime_error(
            "Expecting time_code separator (':', ';', '.')" );
    if ( pos == cs.size() )
        return;
    if ( cs[pos] == ':' || cs[pos] == ';' || cs[pos] == '.' )
    {
        ...
        if ( ( pos + 2 ) >= cs.size() )
            throw std::runtime_error(
                "Found minutes:seconds separator, but expecting 2 digits for seconds" );
        _seconds =
            base::from_digit( cs[pos] ) * 10 + base::from_digit( cs[pos + 1] );
        pos += 2;
    }
    ...
    if ( cs[pos] == ':' || cs[pos] == ';' || cs[pos] == '.' )
    {
        ...
        if ( ( pos + 2 ) >= cs.size() )
            throw std::runtime_error(
                "Found seconds:frames separator, but expecting 2 digits for frames" );
        _seconds =
            base::from_digit( cs[pos] ) * 10 + base::from_digit( cs[pos + 1] );
        pos += 2;
    }
        ...
        if ( frate > 30.0 )
        {
            ...
            else if ( cs[pos] == '.' )
            {
                if ( pos + 1 < cs.size() )
                {
                    ...
                    throw std::runtime_error(
                        "missing data after field marker" );
            }
        }
    }
}
...
std::istream &operator>>( std::istream &in, time_code &tc )
{
    ...
    if ( x == ':' || x == ';' || x == '.' )
    {
        ...
        if ( in && std::isdigit( m1 ) && std::isdigit( m2 ) )
            ...
            throw std::runtime_error(
                "Unable to extract 2 digits for minutes field of time_code" );
        in >> x;
    }
    else
        throw std::runtime_error(
    ...
    if ( x == ':' || x == ';' || x == '.' )
    {
        ...
        if ( in && std::isdigit( s1 ) && std::isdigit( s2 ) )
            ...
            throw std::runtime_error(
                "Unable to extract 2 digits for seconds field of time_code" );
        in >> x;
    }
    else
        throw std::runtime_error(
    ...
    if ( x == ':' || x == ';' || x == '.' )
    {
        ...
        if ( in && std::isdigit( f1 ) && std::isdigit( f2 ) )
            ...
            throw std::runtime_error(
                "Unable to extract 2 digits for frames field of time_code" );
    }
    else
        throw std::runtime_error(
            "Expecting time_code separator (':', ';', '.')" );
