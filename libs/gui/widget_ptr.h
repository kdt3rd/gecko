...
///
template <typename T> class widget_ptr
{
public:
    using widget_type  = T;
    ...
    widget_ptr( Args &&... args )
        : _instance(
              std::make_shared<widget_type>( std::forward<Args>( args )... ) )
    {}
...
inline bool
operator==( const widget_ptr<X> &x, const widget_ptr<Y> &y ) noexcept
{
    return x.get() == y.get();
}
...
template <typename X>
inline void swap( widget_ptr<X> &a, widget_ptr<X> &b ) noexcept
{
    a.swap( b );
}
...
{
    inline bool
    operator()( const gui::widget_ptr<X> &a, const gui::widget_ptr<X> &b ) const
        noexcept
    {
...
template <typename X>
inline void swap( gui::widget_ptr<X> &a, gui::widget_ptr<X> &b ) noexcept
{
    a.swap( b );
}
...
{
    shared_ptr<X> t =
        static_pointer_cast<X>( static_cast<std::shared_ptr<Y>>( r ) );
...
{
    shared_ptr<X> t =
        const_pointer_cast<X>( static_cast<std::shared_ptr<Y>>( r ) );
    return gui::widget_ptr<X>( std::move( t ) );
}
...
{
    shared_ptr<X> t =
        dynamic_pointer_cast<X>( static_cast<std::shared_ptr<Y>>( r ) );
    return gui::widget_ptr<X>( std::move( t ) );
}
...
{
    shared_ptr<X> t =
        reinterpret_pointer_cast<X>( static_cast<std::shared_ptr<Y>>( r ) );
    return gui::widget_ptr<X>( std::move( t ) );
}
#endif
