...
button_w::button_w( void ) : _rect( draw::color{ 0.26F, 0.26F, 0.26F } ) {}

////////////////////////////////////////

button_w::button_w( std::string l, base::alignment a ) : _align( a )
{
...
void button_w::build( context &ctxt )
{
    ...
    const coord          ht  = std::max(
        ...
        coord( fex.height ) + ctxt.from_native_vert( 4 ) );
...
bool button_w::mouse_move( const event &e )
{
    ...
    {
        set_pressed(
            contains( e.from_native( e.raw_mouse().x, e.raw_mouse().y ) ) );
        return true;
    }
    return false;
}
