...
template <typename TheLayout> class container_w : public base_container
{
public:
    template <typename... Args>
    container_w( Args &&... args )
        : base_container( std::unique_ptr<TheLayout>(
              new TheLayout( std::forward<Args>( args )... ) ) )
    {
    ...
    TheLayout &layout( void ) { return *_layout; }
    ...
    void set_spacing( coord h, coord v ) { _layout->set_spacing( h, v ); }
