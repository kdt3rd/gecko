...
public:
    ...
    const platform::event::window_info &raw_window( void ) const
    {
        return _pe.window();
    }
    const platform::event::mouse_info &raw_mouse( void ) const
    {
        return _pe.mouse();
    }
    const platform::event::key_info &raw_key( void ) const { return _pe.key(); }
    const platform::event::text_info &raw_text( void ) const
    {
        return _pe.text();
    }
    const platform::event::tablet_info &raw_tablet( void ) const
    {
        return _pe.tablet();
    }
    const platform::event::hid_info &raw_hid( void ) const { return _pe.hid(); }
    const platform::event::user_info &raw_user( void ) const
    {
        return _pe.user();
    }
