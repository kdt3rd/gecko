...
public:
    ...
    virtual point
                 from_native( platform::coord_type x, platform::coord_type y ) const = 0;
    ...
    push_clip( coord x, coord y, coord w, coord h )
    {
    ...
    const style &get_style( void ) const { return _style; }

    style &get_style( void ) { return _style; }

    template <typename func> void in_context( func &&f )
    {
        push_context();
