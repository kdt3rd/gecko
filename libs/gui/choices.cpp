...
void choices_w::add_choice( const std::string &l )
{
    ...
    _radios.back()->when_selected.connect(
        [=]( void ) { self->choose( choice ); } );
...
{
    _layout->set_spacing(
        ctxt.from_native_horiz( 8 ), ctxt.from_native_vert( 8 ) );
    composite::build( ctxt );
}

////////////////////////////////////////

void choices_w::for_subwidgets(
    const std::function<void( const std::shared_ptr<widget> & )> &f )
{
    for ( auto &l: _labels )
        f( l );
    for ( auto &r: _radios )
