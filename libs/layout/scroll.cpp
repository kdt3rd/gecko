...
void scroll::compute_layout( void )
{
    ...
    coord mw = std::max(
        min_coord(),
        width() - _pad[0] - _pad[1] -
            ( vw > min_coord() ? vw + _spacing[0] : min_coord() ) );
    coord mh = std::max(
        min_coord(),
        height() - _pad[2] - _pad[3] -
            ( vh > min_coord() ? vh + _spacing[1] : min_coord() ) );
    ...
    if ( c )
    {
        c->set_position( { x() + _pad[0] + mw + _spacing[0],
                           y() + _pad[2] + mh + _spacing[1] } );
        c->set_size( vw, vh );
        c->compute_layout();
    }
