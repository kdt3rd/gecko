...
form::form( const std::shared_ptr<area> &layout ) : _layout( layout )
{
...
void form::add( const std::shared_ptr<field> &f ) { _fields.push_back( f ); }
...
void form::compute_layout( void )
{
    _layout->set(
        { x1() + _pad[0], y1() + _pad[2] },
        { width() - _pad[0] - _pad[1], height() - _pad[2] - _pad[3] } );
