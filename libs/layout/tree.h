...
public:
    ...
    tree( coord i, const std::shared_ptr<area> &t ) : _title( t ), _indent( i )
    {}

    void set_groove( const std::shared_ptr<area> &g ) { _groove = g; }

    void set_title( const std::shared_ptr<area> &t ) { _title = t; }

    void set_indent( coord i ) { _indent = i; }

    void add( const std::shared_ptr<area> &c ) { _areas.push_back( c ); }
