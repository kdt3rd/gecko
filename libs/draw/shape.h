...
public:
    shape( void );
    shape( dim x, dim y ) { set_position( x, y ); }
    ...
    void set_position( dim x, dim y ) { _top_left.set( x, y ); }

    void set_size( dim w, dim h ) { _resize.set( w, h ); }

    void shape_size( dim w, dim h ) { _shape.set( w, h ); }
