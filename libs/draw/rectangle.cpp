...
rectangle::rectangle( const color &c ) : _color( c )
{
...
rectangle::rectangle( dim x, dim y, dim w, dim h, const color &c ) : _color( c )
{
...
void rectangle::rebuild( platform::context & ) { _stash.reset(); }
...
void rectangle::draw( platform::context &ctxt )
{
    ...
    bound.set_uniform(
        _stash->_matrix_loc, _rect * ctxt.api().current_matrix() );
...
void rectangle::initialize( platform::context &ctxt )
{
    if ( !_stash )
    {
        if ( ctxt.retrieve_common( this, _stash ) )
        {
            ...
            _stash->_mesh.set_program(
                new_program( ctxt.api(), "simple.vert", "single_color.frag" ) );
            ...
            _stash->_matrix_loc =
                _stash->_mesh.get_uniform_location( "matrix" );
