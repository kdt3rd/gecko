...
public:
    ...
    void set_position( dim x, dim y ) { set_position( point( x, y ) ); }
    template <typename T> void set_position( const base::point<T, 2> &p )
    {
        set_position(
            point( static_cast<dim>( p[0] ), static_cast<dim>( p[1] ) ) );
    }
    ...
    const std::string &get_text( void ) const { return _utf8; }

    std::string &get_text( void ) { return _utf8; }
