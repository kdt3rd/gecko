...
drawable::new_gradient( gl::api &ogl, const gradient &g, size_t n )
{
    ...
    for ( size_t i = 0; i < n; ++i )
    {
        ...
        bytes[i * 4 + 0] = static_cast<uint8_t>(
            std::max( 0.F, std::min( 255.F, std::floor( c.red() * 256.F ) ) ) );
        bytes[i * 4 + 1] = static_cast<uint8_t>( std::max(
            0.F, std::min( 255.F, std::floor( c.green() * 256.F ) ) ) );
        bytes[i * 4 + 2] = static_cast<uint8_t>( std::max(
            0.F, std::min( 255.F, std::floor( c.blue() * 256.F ) ) ) );
        bytes[i * 4 + 3] = static_cast<uint8_t>( std::max(
            0.F, std::min( 255.F, std::floor( c.alpha() * 256.F ) ) ) );
    }
...
std::shared_ptr<gl::program> drawable::new_program(
    gl::api &          ogl,
    ...
    bool               cached )
{
    ...
    if ( !result )
    {
        result = std::make_shared<gl::program>(
            ...
            ogl.new_shader(
                gl::shader::type::FRAGMENT, draw::shaders( frag ) ) );
        if ( cached )
...
std::shared_ptr<gl::texture>
drawable::get_fill_texture( gl::api &ogl, const paint &p )
{
...
gl::program::uniform drawable::fill_mesh(
    gl::api &ogl, gl::mesh &m, const paint &p, const std::string &vert )
{
    if ( p.has_fill_color() )
    {
    ...
    {
        m.set_program(
            new_program( ogl, vert, "linear_gradient.frag", false ) );
        m.get_program().use();
        m.set_uniform( "txt", 0 );
    ...
    {
        m.set_program(
            new_program( ogl, vert, "radial_gradient.frag", false ) );
        m.get_program().use();
        m.set_uniform( "txt", 0 );
    ...
    {
        m.set_program(
            new_program( ogl, vert, "conical_gradient.frag", false ) );
        m.get_program().use();
        m.set_uniform( "txt", 0 );
...
gl::program::uniform drawable::stroke_mesh(
    gl::api &ogl, gl::mesh &m, const paint &p, const std::string &vert )
{
    m.set_program( new_program( ogl, vert, "single_color.frag", false ) );
    m.get_program().use();
