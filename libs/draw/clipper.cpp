...
struct TEdge
{
    ...
    EdgeSide Side = esLeft; //side only refers to current side of solution poly
    int      WindDelta = 1; //1 or -1 depending on winding direction
    int      WindCnt   = 0;
...
{
    inline bool
    operator()( const LocalMinimum &locMin1, const LocalMinimum &locMin2 )
    {
...
{
    int result = static_cast<int>( AllNodes.size() );
...
int PolyNode::ChildCount() const { return static_cast<int>( Childs.size() ); }
//------------------------------------------------------------------------------

void PolyNode::AddChild( PolyNode &child )
{
    unsigned cnt = static_cast<unsigned>( Childs.size() );
...
public:
    ...
    Int128( Int128 &&val )              = default;
    Int128 &operator=( Int128 && ) = default;

    Int128( const long64 &_hi, const ulong64 &_lo ) : lo( _lo ), hi( _hi ) {}

    Int128 &operator=( const long64 &val )
    {
        lo = static_cast<ulong64>( val );
        if ( val < 0 )
            hi = -1;
        else
            hi = 0;
    ...
    operator double() const
    {
        ...
        if ( hi < 0 )
        {
            if ( lo == 0 )
                ...
                return -static_cast<double>( ~lo + ~hi * shift64 );
        }
        else
            return static_cast<double>( lo + hi * shift64 );
    }
...
Int128 Int128Mul( long64 lhs, long64 rhs )
{
    ...
    Int128 tmp;
    tmp.hi = static_cast<long64>( a + ( c >> 32 ) );
    tmp.lo = static_cast<ulong64>( c << 32 );
    tmp.lo += static_cast<ulong64>( b );
    if ( tmp.lo < b )
        tmp.hi++;
    if ( negate )
        tmp = -tmp;
...
static double Area( const OutPt *op )
{
    ...
    do
    {
        a += static_cast<double>( op->Prev->Pt.X + op->Pt.X ) *
             static_cast<double>( op->Prev->Pt.Y - op->Pt.Y );
        op = op->Next;
    } while ( op != startOp );
    return a * 0.5;
}
...
static inline double Area( const OutRec &outRec ) { return Area( outRec.Pts ); }
//------------------------------------------------------------------------------
    ...
    for ( size_t i = 1; i <= cnt; ++i )
    {
        ...
        if ( ( ip.Y < pt.Y ) != ( ipNext.Y < pt.Y ) )
        {
            if ( ip.X >= pt.X )
            {
                ...
                {
                    int64_t d = static_cast<int64_t>( ip.X - pt.X ) *
                                    ( ipNext.Y - pt.Y ) -
                                static_cast<int64_t>( ipNext.X - pt.X ) *
                                    ( ip.Y - pt.Y );
                    if ( !d )
                        return -1;
    ...
    for ( ;; )
    {
        ...
        if ( ( op->Pt.Y < pt.Y ) != ( op->Next->Pt.Y < pt.Y ) )
        {
            if ( op->Pt.X >= pt.X )
            {
                ...
                {
                    int64_t d = static_cast<int64_t>( op->Pt.X - pt.X ) *
                                    ( op->Next->Pt.Y - pt.Y ) -
                                static_cast<int64_t>( op->Next->Pt.X - pt.X ) *
                                    ( op->Pt.Y - pt.Y );
                    if ( !d )
                        return -1;
...
inline void SetDx( TEdge &e )
{
    ...
    if ( dy == 0 )
        ...
        e.Dx = static_cast<double>( e.Top.X - e.Bot.X ) /
...
inline bool Pt2IsBetweenPt1AndPt3(
...
static void RangeTest( const IntPoint &Pt, bool &useFullRange )
{
    if ( useFullRange )
    {
        if ( Pt.X > hiRange || Pt.Y > hiRange || -Pt.X > hiRange ||
             -Pt.Y > hiRange )
...
bool ClipperBase::AddPath( const Path &pg, PolyType PolyTyp, bool Closed )
{
    ...
    for ( ;; )
    {
        ...
        if ( E->Prev == E->Next )
            ...
            Closed &&
            SlopesEqual(
                E->Prev->Curr, E->Curr, E->Next->Curr, m_UseFullRange ) &&
            ( !m_PreserveCollinear ||
              !Pt2IsBetweenPt1AndPt3(
                  E->Prev->Curr, E->Curr, E->Next->Curr ) ) )
        {
...
void ClipperBase::Reset()
{
    ...
    //reset all edges ...
    for ( MinimaList::iterator lm = m_MinimaList.begin();
          lm != m_MinimaList.end();
          ++lm )
    {
        InsertScanbeam( lm->Y );
...
bool Clipper::Execute(
    ClipType clipType, Paths &solution, PolyFillType fillType )
{
...
bool Clipper::Execute(
    ClipType clipType, PolyTree &polytree, PolyFillType fillType )
{
...
void Clipper::FixHoleLinkage( OutRec &outrec )
{
    ...
    if ( !outrec.FirstLeft || ( outrec.IsHole != outrec.FirstLeft->IsHole &&
...
void Clipper::SetWindingCount( TEdge &edge )
{
    ...
    else
    {
        ...
        else
        {
            ...
            if ( edge.WindDelta == 0 )
                edge.WindCnt =
                    ( e->WindCnt < 0 ? e->WindCnt - 1 : e->WindCnt + 1 );
    ...
    switch ( m_ClipType )
    {
        case ctIntersection:
            switch ( pft2 )
            {
                ...
                case pftNegative: return ( edge.WindCnt2 < 0 );
            }
            // fallthrough
        ...
        if ( e1->PolyTyp != e2->PolyTyp )
            ...
            switch ( m_ClipType )
            {
                ...
                case ctUnion:
                    ...
                    break;
                case ctDifference:
                    if ( ( ( e1->PolyTyp == ptClip ) && ( e1Wc2 > 0 ) &&
                           ( e2Wc2 > 0 ) ) ||
                         ( ( e1->PolyTyp == ptSubject ) && ( e1Wc2 <= 0 ) &&
                           ( e2Wc2 <= 0 ) ) )
                        AddLocalMinPoly( e1, e2, Pt );
                    break;
...
TEdge *GetMaximaPairEx( TEdge *e )
{
    ...
    if ( result &&
         ( result->OutIdx == Skip || ( result->NextInAEL == result->PrevInAEL &&
                                       !IsHorizontal( *result ) ) ) )
        return 0;
    return result;
}
...
void GetHorzDirection(
    TEdge &HorzEdge, Direction &Dir, cInt &Left, cInt &Right )
{
                ...
                if ( dir == dLeftToRight )
                {
                    while ( maxIt != m_Maxima.end() && *maxIt < e->Curr.X )
                    {
                        if ( horzEdge->OutIdx >= 0 && !IsOpen )
                            AddOutPt(
                                horzEdge, IntPoint( *maxIt, horzEdge->Bot.Y ) );
                        maxIt++;
                    }
                }
                else
                {
                    while ( maxRit != m_Maxima.rend() && *maxRit > e->Curr.X )
                    {
                        if ( horzEdge->OutIdx >= 0 && !IsOpen )
                            AddOutPt(
                                horzEdge,
                                IntPoint( *maxRit, horzEdge->Bot.Y ) );
                        maxRit++;
                    }
                }
            };
...
bool Clipper::FixupIntersectionOrder()
{
    ...
    CopyAELToSEL();
    std::sort(
        m_IntersectList.begin(), m_IntersectList.end(), IntersectListSort );
    ...
    for ( size_t i = 0; i < cnt; ++i )
    {
        ...
        SwapPositionsInSEL(
            m_IntersectList[i]->Edge1, m_IntersectList[i]->Edge2 );
    }
    return true;
}
//------------------------------------------------------------------------------
    ...
    while ( e )
    {
        ...
        else
        {
            ...
            else
            {
                ...
                e->Curr.Y = topY;
#ifdef use_xyz
                e->Curr.Z = topY == e->Top.Y
                                ? e->Top.Z
                                : ( topY == e->Bot.Y ? e->Bot.Z : 0 );
#endif
            }
    ...
    while ( e )
    {
        if ( IsIntermediate( e, topY ) )
        {
            ...
            if ( ePrev && ePrev->Curr.X == e->Bot.X &&
                 ePrev->Curr.Y == e->Bot.Y && op && ePrev->OutIdx >= 0 &&
...
void Clipper::FixupOutPolygon( OutRec &outrec )
{
    ...
    for ( ;; )
    {
        ...
        if ( ( pp->Pt == pp->Next->Pt ) || ( pp->Pt == pp->Prev->Pt ) ||
             ( SlopesEqual(
                   pp->Prev->Pt, pp->Pt, pp->Next->Pt, m_UseFullRange ) &&
               ( !preserveCol || !Pt2IsBetweenPt1AndPt3(
                                     pp->Prev->Pt, pp->Pt, pp->Next->Pt ) ) ) )
        {
            lastOK         = 0;
            OutPt *tmp     = pp;
...
bool Clipper::JoinPoints( Join *j, OutRec *outRec1, OutRec *outRec2 )
{
    ...
    else if ( isHorizontal )
    {
        ...
        if ( !GetOverlap(
                 op1->Pt.X, op1b->Pt.X, op2->Pt.X, op2b->Pt.X, Left, Right ) )
            return false;
    ...
    const Path &path, JoinType joinType, EndType endType )
{
    ...
    {
        IntPoint ip =
            m_polyNodes.Childs[(int)m_lowest.X]->Contour[(int)m_lowest.Y];
...
void ClipperOffset::DoOffset( double delta )
{
    ...
    for ( int i = 0; i < m_polyNodes.ChildCount(); i++ )
    {
        ...
        if ( len == 0 || ( delta <= 0 &&
                           ( len < 3 || node.m_endtype != etClosedPolygon ) ) )
            continue;
        ...
        for ( int j = 0; j < len - 1; ++j )
            m_normals.push_back(
                GetUnitNormal( m_srcPoly[j], m_srcPoly[j + 1] ) );
        if ( node.m_endtype == etClosedLine ||
             node.m_endtype == etClosedPolygon )
            m_normals.push_back(
                GetUnitNormal( m_srcPoly[len - 1], m_srcPoly[0] ) );
        else
        ...
        else if ( node.m_endtype == etClosedLine )
        {
            ...
            for ( int j = len - 1; j > 0; j-- )
                m_normals[j] =
                    DoublePoint( -m_normals[j - 1].X, -m_normals[j - 1].Y );
...
void ClipperOffset::OffsetPoint( int j, int &k, JoinType jointype )
{
    //cross product ...
    m_sinA =
        ( m_normals[k].X * m_normals[j].Y - m_normals[j].X * m_normals[k].Y );
    if ( std::fabs( m_sinA * m_delta ) < 1.0 )
    {
        //dot product ...
        double cosA =
            ( m_normals[k].X * m_normals[j].X +
...
void ClipperOffset::DoSquare( int j, int k )
{
    ...
    m_destPoly.push_back( IntPoint(
        Round(
            m_srcPoly[j].X +
            m_delta * ( m_normals[k].X - m_normals[k].Y * dx ) ),
        Round(
            m_srcPoly[j].Y +
    ...
    m_destPoly.push_back( IntPoint(
        Round(
            m_srcPoly[j].X +
            m_delta * ( m_normals[j].X + m_normals[j].Y * dx ) ),
        Round(
            m_srcPoly[j].Y +
...
void SimplifyPolygon(
    const Path &in_poly, Paths &out_polys, PolyFillType fillType )
{
    Clipper c;
...
void SimplifyPolygons(
    const Paths &in_polys, Paths &out_polys, PolyFillType fillType )
{
    Clipper c;
    c.StrictlySimple( true );
    ...
    if ( isSum )
        for ( size_t i = 0; i < pathCnt; ++i )
        {
            ...
            for ( size_t j = 0; j < poly.size(); ++j )
                p.push_back(
                    IntPoint( path[i].X + poly[j].X, path[i].Y + poly[j].Y ) );
            pp.push_back( p );
        }
    else
        for ( size_t i = 0; i < pathCnt; ++i )
...
void AddPolyNodeToPaths(
    const PolyNode &polynode, NodeType nodetype, Paths &paths )
{
    bool match = true;
