...
void polylines::quadratic_to(
    const polyline::point &p1, const polyline::point &p2 )
{
    precondition( !_lines.empty(), "no point to start from" );
    precondition( !_lines.back().empty(), "no point to start from" );
...
polylines polylines::stroked( dim width, dim dx, dim dy, dim dz ) const
{
    ...
    for ( const auto &line: _lines )
    {
        ...
        for ( const auto &p: line )
            subj << IntPoint(
                static_cast<int>( to_api( p[0] ) * 100.F + 0.5F ),
        ...
        for ( auto path: solution )
        {
            ...
            for ( const auto &p: path )
                result.add_point( { p.X / 100.F + adx,
                                    p.Y / 100.F + ady,
                                    p.Z / 100.F + adz } );
            result.close();
        }
...
polylines polylines::offset( dim width, dim dx, dim dy, dim dz ) const
{
    ...
    for ( const auto &line: _lines )
    {
        ...
        ClipperOffset co;
        co.AddPath(
            subj, jtRound, line.closed() ? etClosedPolygon : etOpenRound );
...
void polylines::filled(
    const std::function<void( float, float )> &                add_point,
    const std::function<void( uint32_t, uint32_t, uint32_t )> &add_tri ) const
{
    ...
    for ( auto &poly: solution )
    {
        ...
        for ( auto p: poly )
            tess.contour_point(
                contour, p.X / 100.F, p.Y / 100.F, p.Z / 100.F );
