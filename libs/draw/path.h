...
public:
    ...
    static_assert(
        ...
        "expect floating point dimension type" );
    ...
    void horizontal_to( dim x ) { line_to( { x, _last[1] } ); }

    void vertical_to( dim y ) { line_to( { _last[0], y } ); }
    ...
    void horizontal_by( dim x ) { line_by( { x, 0 } ); }

    void vertical_by( dim y ) { line_by( { 0, y } ); }

    /// @brief Quadratic curve relative to current position.
    void quadratic_by( const point &p1, const point &p2 )
    {
        quadratic_to( next_point( p1 ), next_point( p2 ) );
    }

    /// @brief Cubic curve relative to current position.
    void cubic_by( const point &p1, const point &p2, const point &p3 )
    {
        cubic_to( next_point( p1 ), next_point( p2 ), next_point( p3 ) );
    }
    void cubic_by( const point &p2, const point &p3 )
    {
        cubic_to( next_point( p2 ), next_point( p3 ) );
    }
    ...
    void arc_by( const point &center, dim radius, float angle1, float angle2 )
    {
    ...
    /// @brief Draw a rectangle.
    void rectangle( const point &p1, dim w, dim h )
    {
        rectangle( p1, p1 + point( w, h ) );
    }
    ...
    template <typename P> void replay( P &player ) const
    {
        ...
        for ( auto act: _actions )
        {
            switch ( act )
            {
                case action::MOVE: player.move_to( pt( data ) ); break;

                case action::LINE: player.line_to( pt( data ) ); break;
                ...
                case action::CLOSE: player.close(); break;
            }
        }
    }
    ...
    bool empty( void ) const { return _actions.empty(); }

private:
    template <typename Iterator> point pt( Iterator &i ) const
    {
        dim x = *i++;
        dim y = *i++;
        return point( x, y );
    ...
    template <typename Iterator> value_type dbl( Iterator &i ) const
    {
        return *i++;
    }

    inline point next_point( const point &d ) { return d + _last; }
    ...
    void addit( dim v ) { _data.push_back( v.count() ); }
