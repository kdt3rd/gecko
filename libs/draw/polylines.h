...
public:
    ...
    void arc_to(
        const polyline::point &center, dim radius, float angle1, float angle2 );

    void add_point( const polyline::point &p );

    void close( void );

    void push_back( polyline &&p ) { _lines.emplace_back( std::move( p ) ); }

    size_t size( void ) { return _lines.size(); }

    const polyline &operator[]( size_t i ) const { return _lines[i]; }

    polyline &operator[]( size_t i ) { return _lines[i]; }
    ...
    polylines stroked(
        ...
        dim dz = dim( 0 ) ) const;
    polylines
    offset( dim width, dim dx = dim( 0 ), dim dy = dim( 0 ), dim dz = dim( 0 ) )
        const;

    void filled(
        const std::function<void( float, float )> &                points,
        const std::function<void( uint32_t, uint32_t, uint32_t )> &tris ) const;
    void filled( gl::mesh &mesh, const std::string &pos ) const;

    std::vector<polyline>::iterator begin( void ) { return _lines.begin(); }
    ...
    std::vector<polyline>::iterator end( void ) { return _lines.end(); }
    ...
    bool empty( void ) const { return _lines.empty(); }
...
{
    std::copy(
        p.begin(), p.end(), std::ostream_iterator<polyline>( out, "\n" ) );
    return out;
}
