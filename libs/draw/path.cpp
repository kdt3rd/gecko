...
path::path( const point &p ) { move_to( p ); }
...
std::ostream &operator<<( std::ostream &out, const path &p )
{
    ...
    public:
        ...
        void move_to( const point &x ) { out << "Move( " << x << " )\n"; }

        void line_to( const point &x ) { out << "Line( " << x << " )\n"; }
        ...
        void
        arc_to( const point &center, dim radius, float angle1, float angle2 )
        {
            out << "Arc( " << center << ", " << radius << ", " << angle1 << ", "
                << angle2 << " )\n";
        }
