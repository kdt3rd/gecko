...
public:
	...
	template <typename T>
	const T *as( void ) const { return dynamic_cast<const T *>( _node->implementation() ); }

	inline writable_node_ref make_writable( void ) { return writable_node_ref( *this ); }

protected:
	explicit node_reference( scene &s, node_reference &&parent, node *n ) noexcept
...
inline writable_node_ref &
writable_node_ref::operator=( writable_node_ref &&r )
{
...
template <typename T>
inline T *writable_node_ref::as( void ) const
