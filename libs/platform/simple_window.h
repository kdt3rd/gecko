...
public:
	...
	const std::shared_ptr<screen> &query_screen( void ) const { return _win->query_screen(); }
	...
	///
	void set_default_cursor( const std::shared_ptr<cursor> &c ) { _win->set_default_cursor( c ); }
	/// @brief Set a new cursor for the window, storing them as a stack
	void push_cursor( const std::shared_ptr<cursor> &c ) { _win->push_cursor( c ); }
...
//	virtual bool include_menu_in_window_area( void ) const = 0;
	...
	void set_minimum_size( coord_type w, coord_type h ) { _win->set_minimum_size( w, h ); }
