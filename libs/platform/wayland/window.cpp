...
{
static void
shell_ping( void *data, struct wl_shell_surface *surf, uint32_t serial )
{
    wl_shell_surface_pong( surf, serial );
}

static void shell_configure(
    void *                   data,
    ...
    int32_t                  height )
{
    platform::wayland::window *win =
        reinterpret_cast<platform::wayland::window *>( data );
    win->shell_resize_event(
        platform::coord_type( width ), platform::coord_type( height ) );
}

static void shell_popup_done( void *data, struct wl_shell_surface *surf ) {}

static struct wl_shell_surface_listener shell_listener = { &shell_ping,
                                                           &shell_configure,
                                                           &shell_popup_done };
...
void window::make_current( const std::shared_ptr<::platform::cursor> & )
{
    throw_not_yet();
}
