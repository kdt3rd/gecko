...
static void global_registry_handler(
    ...
    uint32_t            version )
{
    std::cout << "wayland global_registry interface '" << interface << "' ID "
              << id << " (version " << version << ")" << std::endl;

    platform::wayland::system *sysPtr =
        reinterpret_cast<platform::wayland::system *>( data );
    if ( !sysPtr )
        return;
    if ( 0 == strcmp( interface, "wl_compositor" ) )
    {
        sysPtr->set_compositor( (struct wl_compositor *)wl_registry_bind(
            registry, id, &wl_compositor_interface, version ) );
    }
    else if ( 0 == strcmp( interface, "wl_shell" ) )
    {
        sysPtr->set_shell( (struct wl_shell *)wl_registry_bind(
            registry, id, &wl_shell_interface, version ) );
    }
...
static void
global_registry_remover( void *data, struct wl_registry *reg, uint32_t id )
{
    ...
    : ::platform::system( "wayland", "Wayland/EGL" )
{
    ...
    _display.reset( wl_display_connect( dname ), []( struct wl_display *ptr ) {
        if ( ptr )
    ...
    if ( _display )
    {
        ...
        reg.reset(
            wl_display_get_registry( _display.get() ),
        ...
        if ( !reg )
            throw std::runtime_error(
                "unable to create global wayland registry" );
        ...
        if ( maj < 1 || ( maj == 1 && min < 4 ) )
            throw std::runtime_error(
                "EGL version too old, reuire 1.4 to support OpenGL API" );
...
{
    auto ret =
        std::make_shared<window>( wintype, _egl_disp, _compositor, _shell, s );
