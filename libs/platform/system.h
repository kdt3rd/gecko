...
public:
    ...
    ///
    virtual std::pair<std::vector<uint8_t>, std::string> query_selection(
        selection_type                  sel,
    ...
    virtual std::pair<std::vector<uint8_t>, std::string> query_drop(
        const selection_type_function &chooseMimeType =
            selection_type_function() ) = 0;
    ...
    ///
    /// @return A new window
    virtual std::shared_ptr<window> new_window(
        window_type                    wintype = window_type::normal,
        const std::shared_ptr<screen> &s = std::shared_ptr<screen>() ) = 0;
    ...
    void create_one_off_timer(
        double in_seconds, const std::function<void( void )> &e );
    void create_one_off_timer(
        timer::duration in_seconds, const std::function<void( void )> &e );
    void create_one_off_timer(
        timer::time_point when, const std::function<void( void )> &e );

private:
    void dispatch_oneoff_timer(
        const std::shared_ptr<timer> &t, const std::function<void( void )> &e );
    std::string _name;
    std::string _desc;
};
