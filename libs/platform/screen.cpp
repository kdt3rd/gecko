...
phys_rect screen::to_physical( const rect &p )
{
    dots_per_unit d_mm = dpmm();
    using val_type = dots_per_unit::coord_type;
    return phys_rect( phys_unit( static_cast<val_type>( p.x() ) / d_mm.w() ),
...
size screen::to_native( const phys_size &u )
{
    ...
    return size( static_cast<coord_type>( std::ceil( ( u.w() * mm( d_mm.w() ) ).count() ) ),
                 static_cast<coord_type>( std::ceil( ( u.h() * mm( d_mm.h() ) ).count() ) ) );
}
...
rect screen::to_native( const phys_rect &u )
{
    ...
    return rect( static_cast<coord_type>( ( u.x() * mm( d_mm.w() ) ).count() ),
                 static_cast<coord_type>( ( u.y() * mm( d_mm.h() ) ).count() ),
                 static_cast<coord_type>( std::ceil( ( u.width() * mm( d_mm.w() ) ).count() ) ),
                 static_cast<coord_type>( std::ceil( ( u.height() * mm( d_mm.h() ) ).count() ) ) );
}
