...
keyboard::keyboard( xcb_connection_t *c ) : _connection( c )
{
    update_mapping();
}
...
void keyboard::update_mapping( void )
{
    ...
    auto numlockcodes =
        core::wrap_cptr( xcb_key_symbols_get_keycode( _keysyms, XK_Num_Lock ) );
    auto shiftlockcodes = core::wrap_cptr(
        xcb_key_symbols_get_keycode( _keysyms, XK_Shift_Lock ) );
    auto capslockcodes = core::wrap_cptr(
        xcb_key_symbols_get_keycode( _keysyms, XK_Caps_Lock ) );
    auto modeswitchcodes = core::wrap_cptr(
        xcb_key_symbols_get_keycode( _keysyms, XK_Mode_switch ) );

    auto modmap_r = core::wrap_cptr(
...
xcb_keysym_t keyboard::get_keysym( xcb_keycode_t code, uint16_t state )
{
    ...
    if ( ( state & _numlock ) && xcb_is_keypad_key( k1 ) )
    {
        ...
        !( state & XCB_MOD_MASK_SHIFT ) && !( state & XCB_MOD_MASK_LOCK ) )
        return k0;
    else if (
