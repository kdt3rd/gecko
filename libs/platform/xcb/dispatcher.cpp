...
int dispatcher::execute( void )
{
    ...
    while ( !done )
    {
        ...
        switch ( event->response_type & ~0x80 )
        {
            ...
            {
                auto *ev =
                    reinterpret_cast<xcb_expose_event_t *>( event.get() );
                if ( ev->count == 0 )
            ...
            {
                auto *ev = reinterpret_cast<xcb_configure_notify_event_t *>(
                    event.get() );
            ...
            {
                auto *ev = reinterpret_cast<xcb_destroy_notify_event_t *>(
                    event.get() );
                auto w = _windows[ev->window];
                w->closed();
                break;
            ...
            {
                auto *ev =
                    reinterpret_cast<xcb_map_notify_event_t *>( event.get() );
                auto w = _windows[ev->window];
                w->shown();
                w->restored();
                break;
            ...
            {
                auto *ev =
                    reinterpret_cast<xcb_unmap_notify_event_t *>( event.get() );
                auto w = _windows[ev->window];
                w->hidden();
                w->minimized();
                break;
            ...
            {
                auto *ev =
                    reinterpret_cast<xcb_enter_notify_event_t *>( event.get() );
            ...
            {
                auto *ev =
                    reinterpret_cast<xcb_leave_notify_event_t *>( event.get() );
                auto w = _windows[ev->event];
                w->exited();
                break;
            }
            ...
            {
                auto *ev =
                    reinterpret_cast<xcb_key_press_event_t *>( event.get() );
                auto               w  = _windows[ev->event];
            ...
            {
                auto *ev = reinterpret_cast<xcb_mapping_notify_event_t *>(
                    event.get() );
            ...
            case XCB_BUTTON_PRESS:
            {
                ...
                switch ( ev->detail )
                {
                    ...
                    case 3:
                        w->mouse_pressed(
                            _mouse,
                            { double( ev->event_x ), double( ev->event_y ) },
                            ev->detail );
                        break;
            ...
            {
                auto *ev = reinterpret_cast<xcb_motion_notify_event_t *>(
                    event.get() );
                auto w = _windows[ev->event];
                w->mouse_moved(
                    _mouse, { double( ev->event_x ), double( ev->event_y ) } );
                break;
            }

            case XCB_VISIBILITY_NOTIFY:
            case XCB_REPARENT_NOTIFY: break;

            case XCB_CLIENT_MESSAGE:
            {
                auto *ev = reinterpret_cast<xcb_client_message_event_t *>(
                    event.get() );
            ...
            default:
                std::cout << "Unknown event: "
                          << uint32_t( event->response_type & ~0x80 ) << ' '
                          << uint32_t( event->response_type ) << std::endl;
        }
    }
