...
public:
    ...
    Reply *operator->( void ) { return get(); }

    Reply &operator*( void ) { return *get(); }

    operator bool() { return fetch() != nullptr; }
...
xcbpp_intern_atom_reply_t xcbpp_intern_atom(
    xcb_connection_t *c, uint8_t only_if_exist, uint16_t len, const char *name )
{
