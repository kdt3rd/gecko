    ...
    : _connection( c ), _screen( screen )
{
    ...
    xcb_create_window(
        ...
        mask,
        cwvals ); // masks, not used yet

    const uint32_t values[] = { screen->black_pixel };
    xcb_change_window_attributes(
        _connection, _win, XCB_CW_BACK_PIXEL, values );
...
void window::raise( void )
{
    ...
    xcb_configure_window(
        _connection, _win, XCB_CONFIG_WINDOW_STACK_MODE, values );
}
...
void window::show( void ) { xcb_map_window( _connection, _win ); }

////////////////////////////////////////

void window::hide( void ) { xcb_unmap_window( _connection, _win ); }
...
{
    const static uint32_t values[] = { uint32_t( w + 0.5 ),
                                       uint32_t( h + 0.5 ) };
    xcb_configure_window(
        _connection,
        _win,
...
std::shared_ptr<draw::canvas> window::canvas( void )
{
    if ( !_canvas )
    {
        ...
        auto geom   = core::wrap_cptr(
            xcb_get_geometry_reply( _connection, cookie, nullptr ) );
...
xcb_window_t window::id( void ) const { return _win; }
...
void window::update_canvas( double ww, double hh )
{
    ...
    if ( w > 0 && h > 0 )
    {
        if ( _canvas->has_surface() )
        {
            ...
            _canvas->set_surface(
                cairo_xcb_surface_create( _connection, _win, _visual, w, h ) );
    }
    else
