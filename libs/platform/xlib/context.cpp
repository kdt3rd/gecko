...
const int visual_attribs[] = {
    ...
    8,
    GLX_ALPHA_SIZE,
    2, // only use 2 in case we're on a 10-bit display
    GLX_DEPTH_SIZE,
    24,
    GLX_STENCIL_SIZE,
    8,
...
platform::context::render_func_ptr queryGL( const char *fname )
{
...
void init_opengl( void )
{
    ...
    if ( opengl_dso )
    {
        glx_query = (platform::context::render_query)dlsym(
            opengl_dso, "glXGetProcAddressARB" );
...
void check_functions( Display *disp )
{
    ...
    _glx_queryversion = reinterpret_cast<Bool ( * )( Display *, int *, int * )>(
        queryGL( "glXQueryVersion" ) );
    ...
    _glx_choosefbconfig = reinterpret_cast<PFNGLXCHOOSEFBCONFIGPROC>(
        queryGL( "glXChooseFBConfig" ) );
    if ( !_glx_choosefbconfig )
        throw_runtime( "Unable to find glXChooseFBConfig" );
    _glx_getvisual = reinterpret_cast<PFNGLXGETVISUALFROMFBCONFIGPROC>(
        queryGL( "glXGetVisualFromFBConfig" ) );
    if ( !_glx_getvisual )
        throw_runtime( "Unable to find glXGetVisualFromFBConfig" );

    _glx_getfbattr = reinterpret_cast<PFNGLXGETFBCONFIGATTRIBPROC>(
        queryGL( "glXGetFBConfigAttrib" ) );
    if ( !_glx_getfbattr )
        throw_runtime( "Unable to find glXGetFBConfigAttrib" );

    _glx_createctxt = reinterpret_cast<PFNGLXCREATECONTEXTATTRIBSARBPROC>(
        queryGL( "glXCreateContextAttribsARB" ) );
    if ( !_glx_createctxt )
        throw_runtime( "unable to find glXCreateContextAttribsARB" );

    _glx_destroyctxt = reinterpret_cast<void ( * )( Display *, GLXContext )>(
        queryGL( "glXDestroyContext" ) );
    if ( !_glx_destroyctxt )
        throw_runtime( "unable to find glXDestroyContext" );

    _glx_createwindow = reinterpret_cast<PFNGLXCREATEWINDOWPROC>(
        queryGL( "glXCreateWindow" ) );
    if ( !_glx_createwindow )
        throw_runtime( "GLX 1.3 or newer, but no glXCreateWindow" );

    _glx_destroywindow = reinterpret_cast<PFNGLXDESTROYWINDOWPROC>(
        queryGL( "glXDestroyWindow" ) );
    if ( !_glx_destroywindow )
        throw_runtime( "GLX 1.3 or newer, but no glXDestroyWindow" );

    _glc_makecurrent =
        reinterpret_cast<void ( * )( Display *, GLXDrawable, GLXContext )>(
            queryGL( "glXMakeCurrent" ) );
    if ( !_glc_makecurrent )
        throw_runtime( "unable to retrieve glXMakeCurrent" );

    _glc_swapbuffers = reinterpret_cast<void ( * )( Display *, GLXDrawable )>(
        queryGL( "glXSwapBuffers" ) );
    if ( !_glc_swapbuffers )
        throw_runtime( "unable to retrieve glXSwapBuffers" );
    _glc_scissor =
        reinterpret_cast<void ( * )( GLint, GLint, GLsizei, GLsizei )>(
            queryGL( "glScissor" ) );
    if ( !_glc_scissor )
    ...
    _glc_enable =
        reinterpret_cast<void ( * )( GLenum )>( queryGL( "glEnable" ) );
    if ( !_glc_enable )
        throw_runtime( "unable to retrieve glEnable" );
    _glc_disable =
        reinterpret_cast<void ( * )( GLenum )>( queryGL( "glDisable" ) );
    if ( !_glc_disable )
...
context::context( const std::shared_ptr<Display> &dpy ) : _display( dpy )
{
...
XVisualInfo *context::choose_best_config( void )
{
    ...
    int          fbcount;
    GLXFBConfig *fbc = _glx_choosefbconfig(
        disp, DefaultScreen( disp ), visual_attribs, &fbcount );
    if ( fbc == nullptr )
...
void context::create( Window win, context *sharectxt )
{
    ...
    {
        share( *sharectxt );
        _ctxt = _glx_createctxt(
            disp, _bestFBC, sharectxt->os_context(), True, attributes );
    }
    else
...
void context::swap_buffers( void ) { _glc_swapbuffers( _display.get(), _win ); }
