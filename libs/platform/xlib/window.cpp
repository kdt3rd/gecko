...
::platform::context &window::hw_context( void ) { return *( _ctxt ); }

////////////////////////////////////////

void window::raise( void ) { XRaiseWindow( _display.get(), _win ); }

////////////////////////////////////////

void window::lower( void ) { XLowerWindow( _display.get(), _win ); }
...
void window::hide( void ) { XUnmapWindow( _display.get(), _win ); }
...
Window window::id( void ) const { return _win; }

////////////////////////////////////////

void window::submit_delayed_expose( const rect &r )
{
    XExposeEvent exp = { Expose, 0,     1,         _display.get(), _win,
...
rect window::query_geometry( void )
{
    ...
    {
        ret.set(
            static_cast<coord_type>( x ),
            static_cast<coord_type>( y ),
            static_cast<coord_type>( w ),
            static_cast<coord_type>( h ) );
    }
    return ret;
}
...
void window::set_win_manager_type( const char *name )
{
    ...
    XChangeProperty(
        ...
        32,
        PropModeReplace,
        reinterpret_cast<unsigned char *>( &defType ),
        1 );
}
