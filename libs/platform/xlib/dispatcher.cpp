...
platform::scancode get_scancode( XKeyEvent &ev, KeySym symbol )
{
    ...
    if ( symbol >= XK_a && symbol <= XK_z )
        return static_cast<platform::scancode>(
            static_cast<uint32_t>( platform::scancode::KEY_A ) +
            ( symbol - XK_a ) );
    ...
    if ( symbol >= XK_0 && symbol <= XK_9 )
        return static_cast<platform::scancode>(
            static_cast<uint32_t>( platform::scancode::KEY_0 ) +
            ( symbol - XK_0 ) );
    ...
    std::cout << "WARNING, unknown keysym: 0x" << std::hex << std::setw( 4 )
              << std::setfill( '0' ) << symbol << std::dec << std::endl;
...
dispatcher::dispatcher(
    ::platform::system *sys, const std::shared_ptr<Display> &dpy )
    : ::platform::dispatcher( sys )
    , _system( sys )
    , _wait_pipe( true, false )
    , _display( dpy )
    ...
    , _mouse( std::make_shared<mouse>( sys ) )
{
    ...
    if ( XkbQueryExtension( disp, &xkbOp, &xkbEv, &xkbErr, &xkbMaj, &xkbMin ) ==
         True )
    ...
    _clipboard_win = XCreateSimpleWindow(
        disp,
        root,
        0,
        0,
...
void dispatcher::set_selection( selection sel )
{
    initSelectionAtoms();
...
std::pair<std::vector<uint8_t>, std::string> dispatcher::query_selection(
    selection_type                  sel,
...
context *dispatcher::get_share_context( void ) const
{
    if ( _windows.empty() )
        return nullptr;
...
void dispatcher::wake_up_executor( void ) { _wait_pipe.write( "x", 1 ); }

////////////////////////////////////////

void dispatcher::run_event_loop_until( std::atomic<bool> *end )
{
...
bool dispatcher::drain_xlib_events( void )
{
    XEvent                  event;
...
dispatcher::Property dispatcher::read_property( Atom sel )
{
    Property       r;
    Atom           actual_type;
    int            actual_format;
    ...
    r.nitems = nitems;
    r.type   = actual_type;
    std::cout << "read property: fmt " << r.format << " nitems " << r.nitems
              << " type " << XGetAtomName( _display.get(), r.type )
              << std::endl;
    return r;
}
...
void dispatcher::initSelectionAtoms( void )
{
...
Atom dispatcher::getSelectionAtom(
    selection_type sel, const std::string &clipboardName )
{
    Atom selA = None;
    switch ( sel )
    {
        ...
        case selection_type::CUSTOM:
        {
            precondition(
                !clipboardName.empty(),
                "valid clipboard name required for custom selection type" );
            auto i = _sel_custom_clips.find( clipboardName );
            if ( i == _sel_custom_clips.end() )
            {
                selA =
                    XInternAtom( _display.get(), clipboardName.c_str(), False );
...
void dispatcher::dispatchKeyPress(
    const std::shared_ptr<window> &w, XEvent &event )
{
    if ( !w )
        return;
    ...
    do
    {
        ...
        switch ( status )
        {
            ...
            case XLookupNone:
            case XLookupChars: break;

            case XLookupBoth: gotSym = true; break;
    ...
    uint8_t mods =
        ( ( ( ( event.xkey.state & ControlMask ) != 0 ) ? 0x01 : 0 ) |
          ( ( ( event.xkey.state & ShiftMask ) != 0 ) ? 0x02 : 0 ) );

    w->process_event( event::key(
        _keyboard.get(),
...
void dispatcher::dispatchKeyRelease(
    const std::shared_ptr<window> &w, XEvent &event )
{
    if ( w )
    {
...
void dispatcher::dispatchButtonPress(
    const std::shared_ptr<window> &w, XEvent &event )
{
    if ( w )
    {
        uint8_t mods =
            ( ( ( ( event.xbutton.state & ControlMask ) != 0 ) ? 0x01 : 0 ) |
              ( ( ( event.xbutton.state & ShiftMask ) != 0 ) ? 0x02 : 0 ) );

        if ( event.xbutton.button == 4 )
            w->process_event( event::hid(
                _mouse.get(),
...
void dispatcher::dispatchButtonRelease(
    const std::shared_ptr<window> &w, XEvent &event )
{
    if ( w )
    {
        ...
        {
            uint8_t mods =
                ( ( ( ( event.xbutton.state & ControlMask ) != 0 ) ? 0x01
                                                                   : 0 ) |
                  ( ( ( event.xbutton.state & ShiftMask ) != 0 ) ? 0x02 : 0 ) );
            w->process_event( event::mouse(
                _mouse.get(),
...
void dispatcher::dispatchMotionNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{
    if ( w )
    {
...
void dispatcher::dispatchEnterNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{
    ...
    {
        uint8_t mods =
            ( ( ( ( event.xcrossing.state & ControlMask ) != 0 ) ? 0x01 : 0 ) |
...
void dispatcher::dispatchLeaveNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{
    // skip virtual crossings
    if ( event.xcrossing.detail == NotifyVirtual ||
...
void dispatcher::dispatchFocusIn(
    const std::shared_ptr<window> &w, XEvent &event )
{
    if ( w && _xim )
        XSetICFocus( w->input_context() );
...
void dispatcher::dispatchFocusOut(
    const std::shared_ptr<window> &w, XEvent &event )
{
    if ( w && _xim )
        XUnsetICFocus( w->input_context() );
...
void dispatcher::dispatchKeymapNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{
    int min_keycode = 8;
    int max_keycode = 255;
...
void dispatcher::dispatchExpose(
    const std::shared_ptr<window> &w, XEvent &event )
{
    if ( w )
    {
...
void dispatcher::dispatchGraphicsExpose(
    const std::shared_ptr<window> &w, XEvent &event )
{
...
void dispatcher::dispatchNoExpose(
    const std::shared_ptr<window> &w, XEvent &event )
{
    // TODO: this is for XCopyArea, etc, what should we do?
}
...
void dispatcher::dispatchVisibilityNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{}

////////////////////////////////////////

void dispatcher::dispatchCreateNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{}

////////////////////////////////////////

void dispatcher::dispatchDestroyNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{
    if ( w )
    {
        if ( _xim )
            XUnsetICFocus( w->input_context() );
        w->process_event( event::window(
            _ext_events.get(), event_type::WINDOW_DESTROYED, 0, 0, 0, 0 ) );
        _windows.erase( w->id() );
    }
}
...
void dispatcher::dispatchUnmapNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{
    if ( w )
    {
        if ( _xim )
            XUnsetICFocus( w->input_context() );
        w->process_event( event::window(
            _ext_events.get(), event_type::WINDOW_HIDDEN, 0, 0, 0, 0 ) );
        w->process_event( event::window(
            _ext_events.get(), event_type::WINDOW_MINIMIZED, 0, 0, 0, 0 ) );
...
void dispatcher::dispatchMapNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{
    if ( w )
    {
        w->process_event( event::window(
            _ext_events.get(), event_type::WINDOW_SHOWN, 0, 0, 0, 0 ) );
        w->process_event( event::window(
            _ext_events.get(), event_type::WINDOW_RESTORED, 0, 0, 0, 0 ) );
    }
}

////////////////////////////////////////

void dispatcher::dispatchMapRequest(
    const std::shared_ptr<window> &w, XEvent &event )
{}

////////////////////////////////////////

void dispatcher::dispatchReparentNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{}

////////////////////////////////////////

void dispatcher::dispatchConfigureNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{
    if ( w )
        w->process_event( event::window(
...
void dispatcher::dispatchConfigureRequest(
    const std::shared_ptr<window> &w, XEvent &event )
{}

////////////////////////////////////////

void dispatcher::dispatchGravityNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{}

////////////////////////////////////////

void dispatcher::dispatchResizeRequest(
    const std::shared_ptr<window> &w, XEvent &event )
{}

////////////////////////////////////////

void dispatcher::dispatchCirculateNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{}

////////////////////////////////////////

void dispatcher::dispatchCirculateRequest(
    const std::shared_ptr<window> &w, XEvent &event )
{}

////////////////////////////////////////

void dispatcher::dispatchPropertyNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{
    if ( _sel_stack.empty() )
        return;
    ...
    if ( req.incr_request && event.xproperty.state == PropertyNewValue &&
         event.xproperty.atom == req.sel )
    {
...
void dispatcher::dispatchSelectionClear(
    const std::shared_ptr<window> &w, XEvent &event )
{
...
void dispatcher::dispatchSelectionRequest(
    const std::shared_ptr<window> &w, XEvent &event )
{
    initSelectionAtoms();
...
void dispatcher::dispatchSelectionNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{
    Atom targ = event.xselection.target;
    std::cout << "selection notify:\n  requestor 0x" << std::hex
              << event.xselection.requestor << std::dec << "\n  sel atom '"
              << XGetAtomName( _display.get(), event.xselection.selection )
              << "'\n  targ atom '" << XGetAtomName( _display.get(), targ )
              << "'\n  prop atom '"
              << XGetAtomName( _display.get(), event.xselection.property )
              << "'" << std::endl;
    ...
    {
        std::cout << "Handling large bytes " << prop.format << ", items "
                  << prop.nitems << " expected bytes: "
                  << reinterpret_cast<int *>( prop.data )[0] << std::endl;
    ...
    if ( targ == _sel_targets && !req.sent_request )
    {
        ...
        {
            std::cout << "requesting type: " << *( req.resulttype )
                      << std::endl;
            req.result->clear();
            XConvertSelection(
                _display.get(),
...
void dispatcher::dispatchColormapNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{}

////////////////////////////////////////

void dispatcher::dispatchClientMessage(
    const std::shared_ptr<window> &w, XEvent &event )
{
    ...
    if ( w && ( evatom == _atom_delete_window || evatom == _atom_quit_app ) )
    {
        ...
        if ( evatom == _atom_quit_app )
            doWinClose = w->process_event( event::window(
                _ext_events.get(),
                event_type::WINDOW_CLOSE_REQUEST,
                0,
                0,
                0,
                0 ) );
...
void dispatcher::dispatchMappingNotify(
    const std::shared_ptr<window> &w, XEvent &event )
{
...
void dispatcher::dispatchGenericEvent(
    const std::shared_ptr<window> &w, XEvent &event )
{}

////////////////////////////////////////

void dispatcher::dispatchUNKNOWN(
    const std::shared_ptr<window> &w, XEvent &event )
{}
...
bool dispatcher::fill_requested(
    SelectionRequestInfo &req, const Property &prop )
{
    ...
    if ( ( prop.type != XA_ATOM && prop.type != _sel_targets ) ||
         prop.format != 32 )
    {
        if ( req.reqTypes )
        {
            ...
            if ( reqTypes.empty() )
                ...
                std::find(
                    reqTypes.begin(), reqTypes.end(), *( req.resulttype ) ) !=
                reqTypes.end() )
                req.requested = XA_STRING;
        }
