...
template <typename CacheObject> class cache_guard
{
public:
    inline cache_guard( void ) noexcept : _co( nullptr ) {}
    explicit inline cache_guard( CacheObject &c ) : _co( std::addressof( c ) )
    {
        bind();
    }
    inline ~cache_guard( void ) { unbind(); }
    ...
    inline bool owns( void ) const noexcept { return _owns; }

    inline CacheObject *object( void ) const noexcept { return _co; }
