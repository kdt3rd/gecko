...
static plane extract_u( const vector_field &v ) { return v.u(); }

////////////////////////////////////////

static plane extract_v( const vector_field &v ) { return v.v(); }
...
static void cvtToAbsU( scanline &dest, const scanline &u )
{
...
static void cvtToAbsV( scanline &dest, int y, const plane &v )
{
...
static void cvtToRelU( scanline &dest, const scanline &u )
{
    float offX = static_cast<float>( dest.offset() );
    for ( int x = 0; x < dest.width(); ++x )
        dest[x] = u[x] - static_cast<float>( x ) - offX;
...
static void cvtToRelV( scanline &dest, int y, const plane &v )
{
    const float *vLine = v.line( y );
    float        yOff  = static_cast<float>( y );
    for ( int x = 0; x < dest.width(); ++x )
...
static void applyWarpBilinearP(
    scanline &dest, int y, const plane &src, const vector_field &v )
{
    if ( v.is_absolute() )
    {
    ...
    else
    {
        ...
        plane::value_type        xOff =
            static_cast<plane::value_type>( dest.offset() );
        for ( int x = 0; x < dest.width(); ++x )
        {
            plane::value_type uV =
                uLine[x] + static_cast<plane::value_type>( x ) + xOff;
    ...
    scanline &dest, int y, const plane &src, const vector_field &v )
{
    if ( v.is_absolute() )
    {
        ...
        for ( int x = 0; x < dest.width(); ++x )
        {
            ...
            dest[x] =
                get_hold( src, static_cast<int>( uV ), static_cast<int>( vV ) );
        }
    }
    else
    {
        ...
        for ( int x = 0; x < dest.width(); ++x )
        {
            dest[x] = get_hold(
                src,
                static_cast<int>( uLine[x] ) + x + offX,
...
inline void
colorize_hsv2rgb( float &r, float &g, float &b, float h, float s, float v )
{
    float c  = v * s;
    float h2 = h * 6.F;
...
static void colorize_thread_final(
    size_t,
    int        s,
    int        e,
    image_buf &ret,
...
static void colorize_thread_abs(
    size_t               tIdx,
    int                  s,
    int                  e,
    image_buf &          ret,
...
static void colorize_thread_rel(
    size_t               tIdx,
    int                  s,
    int                  e,
    image_buf &          ret,
...
static image_buf colorize_vector( const vector_field &v, float scale )
{
    ...
    std::cout << "color scaling by " << maxMag << " ave " << aveMag
              << std::endl;

    threading::get().dispatch(
        std::bind(
            colorize_thread_final,
            std::placeholders::_1,
            std::placeholders::_2,
            std::placeholders::_3,
            std::ref( ret ),
...
static void colorize_thread_rel_alpha(
    size_t               tIdx,
    int                  s,
    int                  e,
    image_buf &          ret,
...
static image_buf
colorize_vector_alpha( const vector_field &v, const plane &alpha, float scale )
{
    std::vector<float>  maxmags;
    std::vector<double> avemags;
    std::vector<double> avesums;
...
static void project_splat_bilinear(
    size_t,
    int                       s,
    int                       e,
    plane_buffer &            outU,
    plane_buffer &            outV,
    plane_buffer &            splatCount,
    const const_plane_buffer &inU,
    const const_plane_buffer &inV,
    float                     scale )
{
    int   w           = outU.width();
    int   offX        = outU.x1();
...
static void project_final(
    size_t,
    int                 s,
    int                 e,
    plane_buffer &      outU,
    plane_buffer &      outV,
    const plane_buffer &splatCount )
{
    int w = outU.width();
    for ( int y = s; y < e; ++y )
    {
...
static vector_field project_vector( const vector_field &v, float scale )
{
    ...
    project_splat_bilinear(
        0, inU.y1(), inU.y2() + 1, accumUb, accumVb, splatb, inU, inV, scale );
...
{
    return vector_field(
        false, "v.project", a.dims(), convert_to_relative( a ), scale );
}

////////////////////////////////////////

plane confidence(
    const vector_field &a, const vector_field &b, int conservativeness )
{
    precondition(
        a.dims() == b.dims(),
        "Vector field not same size as other requested for divergence" );
    if ( conservativeness <= 0 )
        return log1p( magnitude(
            a.u() + warp_bilinear( b.u(), a ),
            a.v() + warp_bilinear( b.v(), a ) ) );
    ...
    return log1p(
        magnitude(
            ...
            a.v() + warp_bilinear( b.v(), a ) ) *
        magnitude(
            erode( a.u(), conservativeness ) -
                dilate( a.u(), conservativeness ),
            erode( a.v(), conservativeness ) -
                dilate( a.v(), conservativeness ) ) );
}
...
{
    precondition(
        v.x1() == src.x1() && v.y1() == src.y1() && v.x2() == src.x2() &&
            v.y2() == src.y2(),
        "Vector field not same size as plane requested for warp" );
...
image_buf colorize( const vector_field &v, const plane &alpha, float scale )
{
    ...
    if ( alpha.valid() )
    {
        precondition(
            v.u().dims() == alpha.dims(),
            "colorize must have an alpha channel of same size as vectors, received a {0} alpha {1}",
            v.dims(),
            alpha.dims() );
...
vector_field modify_vectors( const plane &newu, const plane &newv, bool a )
{
    precondition(
        newu.dims() == newv.dims(),
        "modify planes must have vector u,v of same size, received u {0} v {1}",
        newu.dims(),
        newv.dims() );
...
void add_vector_ops( engine::registry &r )
{
    ...
    r.add(
        op( "v.p.warp_dirac",
            base::choose_runtime( applyWarpDiracP ),
            n_scanline_plane_adapter<false, decltype( applyWarpDiracP )>(),
            dispatch_scan_processing,
            op::n_to_one ) );
    r.add(
        op( "v.p.warp_bilinear",
            base::choose_runtime( applyWarpBilinearP ),
            n_scanline_plane_adapter<false, decltype( applyWarpBilinearP )>(),
            dispatch_scan_processing,
            op::n_to_one ) );

    r.add(
        op( "v.cvt_to_abs_u",
            base::choose_runtime( cvtToAbsU ),
            scanline_plane_adapter<true, decltype( cvtToAbsU )>(),
            dispatch_scan_processing,
            op::one_to_one ) );
    r.add(
        op( "v.cvt_to_abs_v",
            base::choose_runtime( cvtToAbsV ),
            n_scanline_plane_adapter<false, decltype( cvtToAbsV )>(),
            dispatch_scan_processing,
            op::n_to_one ) );
    r.add(
        op( "v.cvt_to_rel_u",
            base::choose_runtime( cvtToRelU ),
            scanline_plane_adapter<true, decltype( cvtToRelU )>(),
            dispatch_scan_processing,
            op::one_to_one ) );
    r.add(
        op( "v.cvt_to_rel_v",
            base::choose_runtime( cvtToRelV ),
            n_scanline_plane_adapter<false, decltype( cvtToRelV )>(),
            dispatch_scan_processing,
            op::n_to_one ) );
