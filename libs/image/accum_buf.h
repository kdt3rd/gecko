...
public:
    ...
    {
        return static_cast<size_t>( stride() * height() ) * sizeof( double );
    }
    ...
    inline const double *cdata( void ) const
    {
        check_compute();
        return _mem.get();
    }
    inline double *line( int y ) { return data() + ( y - _y1 ) * stride(); }
    inline const double *line( int y ) const
    {
    ...
    template <typename T> inline void set( int x, int y, T v )
    {
        get( x, y ) = static_cast<double>( v );
    }
