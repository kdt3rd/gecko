...
plane pad( const plane &a, int l, int t, int r, int b, float val )
{
    precondition(
        l >= 0 && t >= 0 && r >= 0 && b >= 0,
        "expecting to only grow the image in pad, got pad request of {0},{1},{2},{3}",
        l,
        t,
        r,
        b );
...
plane union_dim( const plane &a, const plane &b, float outside )
{
    engine::dimensions adim = a.dims();
...
plane union_dim_hold( const plane &a, const plane &b )
{
    ...
    return pad_hold(
        a, adim.x1 - r.x1, adim.y1 - r.y1, r.x2 - adim.x2, r.y2 - adim.y2 );
}

////////////////////////////////////////

plane dirichlet( const plane &p, int border )
{
