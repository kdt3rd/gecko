...
void image_buf::add_plane( const plane &p )
{
    if ( !_planes.empty() )
        precondition(
            _planes.front().width() == p.width() &&
                _planes.front().height() == p.height(),
            "Attempt to create image with mis-matched plane dimensions" );
    _planes.push_back( p );
}

////////////////////////////////////////

void image_buf::add_plane( plane &&p )
{
    if ( !_planes.empty() )
        precondition(
            _planes.front().width() == p.width() &&
...
engine::hash &operator<<( engine::hash &h, const image_buf &i )
{
