...
{
	return p.get( std::max( p.x1(), std::min( p.x2(), x ) ),
...
template <typename P>
GK_FORCE_INLINE P bilinCoord( int &p1, int &p2, P v )
{
	P perc;
	if ( v < 0.F )
	{
...
template <typename P>
GK_FORCE_INLINE typename P::value_type bilinear_zero( const P &p, typename P::value_type x, typename P::value_type y )
{
...
template <typename P>
GK_FORCE_INLINE typename P::value_type bilinear_hold( const P &p, typename P::value_type x, typename P::value_type y )
{
	typedef typename P::value_type value_type;
...
/// bilinear sample, treating requests outside as zero
template <typename P>
GK_FORCE_INLINE typename P::value_type bilinear_bignum( const P &p, typename P::value_type x, typename P::value_type y, typename P::value_type bignum )
{
	typedef typename P::value_type value_type;
