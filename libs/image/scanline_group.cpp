...
void scanline_group::output_scan( size_t i, scanline &&s )
{
...
scanline scanline_group::output_scan_and_clear( size_t i )
{
    scanline r;
    swap( r, _outputs[i] );
    return r;
...
scanline scanline_group::find_or_checkout(
    const std::vector<scanline> &inputs, bool in_place )
{
    if ( in_place )
    {
        for ( auto &s: inputs )
