...
struct PatchMatchSSD
{
    ...
    static inline float compute(
        const std::vector<const_plane_buffer> &a,
        const std::vector<const_plane_buffer> &b,
        const std::vector<const_plane_buffer> &,
        const std::vector<const_plane_buffer> &,
        const std::vector<const_plane_buffer> &,
        const std::vector<const_plane_buffer> &,
        int ax,
        ...
        for ( int y = -radius; y <= radius; ++y )
        {
            ...
            if ( sby < y1 || sby > y2 )
            {
                ret += static_cast<float>(
                    static_cast<int>( numC ) * ( 2 * radius + 1 ) );
                continue;
            }
...
struct PatchMatchSSDGrad
{
    static inline float compute(
        const const_plane_buffer &a,
        const const_plane_buffer &b,
        const const_plane_buffer &adx,
        const const_plane_buffer &ady,
        const const_plane_buffer &bdx,
        const const_plane_buffer &bdy,
        int                       ax,
        int                       ay,
        int                       px,
        int                       py,
    ...
    static inline float compute(
        const std::vector<const_plane_buffer> &a,
        const std::vector<const_plane_buffer> &b,
        const std::vector<const_plane_buffer> &adx,
        const std::vector<const_plane_buffer> &ady,
        const std::vector<const_plane_buffer> &bdx,
        const std::vector<const_plane_buffer> &bdy,
        int                                    ax,
        int                                    ay,
        int                                    px,
        int                                    py,
            ...
            for ( int x = -radius; x <= radius; ++x )
            {
                ...
                else
                {
                    for ( size_t c = 0; c != numC; ++c )
                    {
                        ...
                        float gx =
                            adx[c].get( sax, say ) - bdx[c].get( sbx, sby );
                        float gy =
                            ady[c].get( sax, say ) - bdy[c].get( sbx, sby );
                        ret += d * d + gx * gx + gy * gy;
                    }
                }
            }
...
struct PatchMatchSSDGradDist
{
        ...
        int                       radius )
    {
        ...
        float distX =
            static_cast<float>( ax - px ) / static_cast<float>( a.width() );
        float distY =
            static_cast<float>( ay - py ) / static_cast<float>( a.height() );
        ...
        int                                    radius )
    {
        TODO( "validate that this is correct math in a datawindow box world" );
        float distX =
            static_cast<float>( ax - px ) / static_cast<float>( a[0].width() );
        float distY =
            static_cast<float>( ay - py ) / static_cast<float>( a[0].height() );
        float  ret  = distX * distX + distY * distY;
        size_t numC = a.size();
        int    x1 = a[0].x1(), y1 = a[0].y1(), x2 = a[0].x2(), y2 = a[0].y2();
        for ( int y = -radius; y <= radius; ++y )
...
struct PatchMatchGrad
{
        ...
        int                                    radius )
    {
        ...
        int    x1 = adx[0].x1(), y1 = adx[0].y1(), x2 = adx[0].x2(),
            y2 = adx[0].y2();
        for ( int y = -radius; y <= radius; ++y )
        {
            int say = ay + y;
...
static void matchPassInitThread(
    size_t,
    int                 s,
    int                 e,
    plane_buffer &      d,
    const plane_buffer &u,
    const plane_buffer &v,
    const BufType &     a,
    const BufType &     b,
    const BufType &     adx,
    const BufType &     ady,
    ...
    int                 radius )
{
    ...
    for ( int y = s; y < e; ++y )
    {
        ...
        for ( int x = 0; x < w; ++x )
        {
            ...
            dLine[x]  = DistFunc::compute(
                a, b, adx, ady, bdx, bdy, x + offX, y, curDX, curDY, radius );
        }
    }
}

template <typename BufType, typename DistFunc>
inline void matchPassInit(
    plane_buffer & u,
    plane_buffer & v,
    plane_buffer & d,
    const BufType &a,
    ...
    int            radius )
{
    ...
    threading::get().dispatch(
        std::bind(
            matchPassInitThread<BufType, DistFunc>,
            std::placeholders::_1,
            std::placeholders::_2,
            std::placeholders::_3,
            std::ref( d ),
...
template <typename BufType, typename DistFunc>
static void matchPassInitThreadAlpha(
    size_t,
    int                       s,
    int                       e,
    plane_buffer &            d,
    ...
    int                       radius )
{
    ...
    threading::get().dispatch(
        std::bind(
            matchPassInitThreadAlpha<BufType, DistFunc>,
            std::placeholders::_1,
            std::placeholders::_2,
            std::placeholders::_3,
            std::ref( d ),
...
template <typename BufType, typename DistFunc>
static void matchPassHorizThreadForward1(
    size_t               tIdx,
    int                  s,
    int                  e,
    plane_buffer &       u,
    ...
    float                eps )
{
    ...
    for ( int y = s; y < e; ++y )
    {
        ...
        for ( int x = 1; x < w; ++x )
        {
            ...
            float hDist = DistFunc::compute(
                a, b, adx, ady, bdx, bdy, x + offX, y, altDX, altDY, radius );
            if ( hDist < curDist )
            {
...
template <typename BufType, typename DistFunc>
static void matchPassHorizThreadForwardN(
    size_t               tIdx,
    int                  s,
    int                  e,
    plane_buffer &       u,
...
template <typename BufType, typename DistFunc>
static void matchPassHorizThreadBackward1(
    size_t               tIdx,
    int                  s,
    int                  e,
    plane_buffer &       u,
    ...
    float                eps )
{
    ...
    for ( int y = s; y < e; ++y )
    {
        ...
        for ( int x = w - 2; x >= 0; --x )
        {
            ...
            float hDist = DistFunc::compute(
                a, b, adx, ady, bdx, bdy, offX + x, y, altDX, altDY, radius );
            if ( hDist < curDist )
            {
                uLine[x] = static_cast<float>( altDX );
                vLine[x] = static_cast<float>( altDY );
...
template <typename BufType, typename DistFunc>
static void matchPassHorizThreadBackwardN(
    size_t               tIdx,
    int                  s,
    int                  e,
    plane_buffer &       u,
...
template <typename BufType, typename DistFunc>
static void matchPassVertThreadDown1(
    size_t               tIdx,
    int                  s,
    int                  e,
    plane_buffer &       u,
    ...
    float                eps )
{
    ...
    for ( int x = s; x < e; ++x )
    {
        for ( int y = u.y1() + 1; y <= u.y2(); ++y )
        {
            ...
            float hDist = DistFunc::compute(
                a, b, adx, ady, bdx, bdy, x, y, altDX, altDY, radius );
            if ( hDist < curDist )
            {
...
template <typename BufType, typename DistFunc>
static void matchPassVertThreadDownN(
    size_t               tIdx,
    int                  s,
    int                  e,
    plane_buffer &       u,
...
template <typename BufType, typename DistFunc>
static void matchPassVertThreadUp1(
    size_t               tIdx,
    int                  s,
    int                  e,
    plane_buffer &       u,
...
template <typename BufType, typename DistFunc>
static void matchPassVertThreadUpN(
    size_t               tIdx,
    int                  s,
    int                  e,
    plane_buffer &       u,
...
template <typename BufType, typename DistFunc>
static void matchPassRandomThread(
    size_t                                 tIdx,
    int                                    s,
    int                                    e,
    plane_buffer &                         u,
    ...
    const BufType &                        bdy,
    int                                    radius,
    const std::vector<std::uint_fast32_t> &seeds,
    std::vector<size_t> &                  changeCounts,
    float                                  eps )
{
    int    w           = d.width();
        ...
        for ( int x = 1; x < ( w - 1 ); ++x )
        {
            ...
            if ( count > 0 )
            {
                ...
                float hDist = DistFunc::compute(
                    ...
                    altDY,
                    radius );
                float distRatio = static_cast<float>( curMagSq ) /
                                  static_cast<float>( tmpMagSq );
...
static inline size_t getCount( std::vector<size_t> &counts )
{
    size_t ret = 0;
    for ( size_t &a: counts )
    {
    ...
    float          eps )
{
    std::vector<size_t> counts(
        static_cast<size_t>( threading::get().size() ), size_t( 0 ) );
    size_t nChange;
    size_t totChange = 0;
    // do one round of jump flood on second iteration
    int maxSkipX = ( iter == 1 )
                       ? std::max( int( 1 ), u.width() / ( iter * iter + 1 ) )
                       : 1;
    int maxSkipY = ( iter == 1 )
                       ? std::max( int( 1 ), u.height() / ( iter * iter + 1 ) )
                       : 1;

    std::cout << "   iter " << iter << " skip: " << maxSkipX << ' ' << maxSkipY
              << " l->r: " << std::flush;
    if ( maxSkipX == 1 )
        threading::get().dispatch(
            std::bind(
                matchPassHorizThreadForward1<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
        ...
        threading::get().dispatch(
            std::bind(
                matchPassHorizThreadForwardN<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
    ...
    if ( maxSkipY == 1 )
        threading::get().dispatch(
            std::bind(
                matchPassVertThreadDown1<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
        ...
        threading::get().dispatch(
            std::bind(
                matchPassVertThreadDownN<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
    ...
    threading::get().dispatch(
        std::bind(
            matchPassRandomThread<BufType, DistFunc>,
            std::placeholders::_1,
            std::placeholders::_2,
            std::placeholders::_3,
            std::ref( u ),
    ...
    if ( maxSkipX == 1 )
        threading::get().dispatch(
            std::bind(
                matchPassHorizThreadBackward1<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
        ...
        threading::get().dispatch(
            std::bind(
                matchPassHorizThreadBackwardN<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
    ...
    if ( maxSkipY == 1 )
        threading::get().dispatch(
            std::bind(
                matchPassVertThreadUp1<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
        ...
        threading::get().dispatch(
            std::bind(
                matchPassVertThreadUpN<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
    ...
    threading::get().dispatch(
        std::bind(
            matchPassDiagonal<BufType, DistFunc>,
            std::placeholders::_1,
            std::placeholders::_2,
            std::placeholders::_3,
            std::ref( u ),
    ...
    threading::get().dispatch(
        std::bind(
            matchPassRegAve<BufType, DistFunc>,
            std::placeholders::_1,
            std::placeholders::_2,
            std::placeholders::_3,
            std::ref( u ),
...
template <typename BufType, typename DistFunc>
static void matchPassHorizThreadForwardAlpha1(
    size_t                    tIdx,
    int                       s,
    int                       e,
    plane_buffer &            u,
...
template <typename BufType, typename DistFunc>
static void matchPassHorizThreadForwardAlphaN(
    size_t                    tIdx,
    int                       s,
    int                       e,
    plane_buffer &            u,
...
template <typename BufType, typename DistFunc>
static void matchPassVertThreadDownAlpha1(
    size_t                    tIdx,
    int                       s,
    int                       e,
    plane_buffer &            u,
...
template <typename BufType, typename DistFunc>
static void matchPassHorizThreadBackwardAlpha1(
    size_t                    tIdx,
    int                       s,
    int                       e,
    plane_buffer &            u,
...
template <typename BufType, typename DistFunc>
static void matchPassHorizThreadBackwardAlphaN(
    size_t                    tIdx,
    int                       s,
    int                       e,
    plane_buffer &            u,
...
template <typename BufType, typename DistFunc>
static void matchPassVertThreadDownAlphaN(
    size_t                    tIdx,
    int                       s,
    int                       e,
    plane_buffer &            u,
...
template <typename BufType, typename DistFunc>
static void matchPassVertThreadUpAlpha1(
    size_t                    tIdx,
    int                       s,
    int                       e,
    plane_buffer &            u,
...
template <typename BufType, typename DistFunc>
static void matchPassVertThreadUpAlphaN(
    size_t                    tIdx,
    int                       s,
    int                       e,
    plane_buffer &            u,
...
template <typename BufType, typename DistFunc>
static void matchPassDiagonalAlpha(
    size_t                    tIdx,
    int                       s,
    int                       e,
    plane_buffer &            u,
...
template <typename BufType, typename DistFunc>
static void matchPassRandomThreadAlpha(
    size_t                                 tIdx,
    int                                    s,
    int                                    e,
    plane_buffer &                         u,
...
template <typename BufType, typename DistFunc>
static void matchPassRegAveAlpha(
    size_t                    tIdx,
    int                       s,
    int                       e,
    plane_buffer &            u,
        ...
        for ( int x = 1; x < ( w - 1 ); ++x )
        {
            ...
            for ( int oy = -1; oy <= 1; ++oy )
            {
                ...
                for ( int ox = -1; ox <= 1; ++ox )
                {
                    ...
                    int curX = x + offX + ox;
                    // valid and dist measurement is better
                    if ( alpha.get( curX, curY ) >
                         0.F ) //&& d.get( curX, curY ) <= curDist )
                    {
                        int tmpDX = static_cast<int>( u.get( curX, curY ) );
                        int tmpDY = static_cast<int>( v.get( curX, curY ) );
                        tmpDX -= ox;
            ...
            if ( count > 0 )
            {
                ...
                if ( get_zero( alpha, altDX, altDY ) > 0.F )
                {
                    ...
                    // the closer the potential match, the more slop we allow
                    float slop = powf( 1.15F, distRatio );
                    if ( hDist < curDist || ( tmpMagSq < curMagSq &&
                                              hDist <= ( curDist * slop ) ) )
                    {
                        uLine[x] = static_cast<float>( altDX );
                        vLine[x] = static_cast<float>( altDY );
    ...
    float               eps )
{
    ...
    if ( maxSkipX == 1 )
        threading::get().dispatch(
            std::bind(
                matchPassHorizThreadForwardAlpha1<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
        ...
        threading::get().dispatch(
            std::bind(
                matchPassHorizThreadForwardAlphaN<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
    ...
    if ( maxSkipY == 1 )
        threading::get().dispatch(
            std::bind(
                matchPassVertThreadDownAlpha1<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
        ...
        threading::get().dispatch(
            std::bind(
                matchPassVertThreadDownAlphaN<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
    ...
    threading::get().dispatch(
        std::bind(
            matchPassRandomThreadAlpha<BufType, DistFunc>,
            std::placeholders::_1,
            std::placeholders::_2,
            std::placeholders::_3,
            std::ref( u ),
    ...
    if ( maxSkipX == 1 )
        threading::get().dispatch(
            std::bind(
                matchPassHorizThreadBackwardAlpha1<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
        ...
        threading::get().dispatch(
            std::bind(
                matchPassHorizThreadBackwardAlphaN<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
    ...
    if ( maxSkipY == 1 )
        threading::get().dispatch(
            std::bind(
                matchPassVertThreadUpAlpha1<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
        ...
        threading::get().dispatch(
            std::bind(
                matchPassVertThreadUpAlphaN<BufType, DistFunc>,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( u ),
    ...
    threading::get().dispatch(
        std::bind(
            matchPassDiagonalAlpha<BufType, DistFunc>,
            std::placeholders::_1,
            std::placeholders::_2,
            std::placeholders::_3,
            std::ref( u ),
    ...
    threading::get().dispatch(
        std::bind(
            matchPassRegAveAlpha<BufType, DistFunc>,
            std::placeholders::_1,
            std::placeholders::_2,
            std::placeholders::_3,
            std::ref( u ),
    ...
    int            iters )
{
    ...
    switch ( static_cast<patch_style>( style ) )
    {
        case patch_style::SSD:
            matchPassInit<BufType, PatchMatchSSD>(
                u, v, d, a, b, adx, ady, bdx, bdy, radius );
            std::cout << "\npatch_match SSD eps " << eps << " radius " << radius
                      << ": " << std::endl;
            for ( int p = 0; p < iters; ++p )
            {
                // if changes don't change, finished
                if ( matchPass2<BufType, PatchMatchSSD>(
                         u,
                         v,
                         d,
                         a,
                         ...
                         eps ) )
                {
                    std::cout << "patch_match converged after " << ( p + 1 )
                              << " iterations" << std::endl;
                    break;
                }
            }
        ...
        case patch_style::SSD_GRAD:
            matchPassInit<BufType, PatchMatchSSDGrad>(
                u, v, d, a, b, adx, ady, bdx, bdy, radius );
            std::cout << "\npatch_match SSD_GRAD eps " << eps << " radius "
                      << radius << ": " << std::endl;
            for ( int p = 0; p < iters; ++p )
            {
                //				if ( p > 0 && p % 5 == 0 )
                ...
                if ( matchPass2<BufType, PatchMatchSSDGrad>(
                         u,
                         v,
                         d,
                         a,
        ...
        case patch_style::SSD_GRAD_DIST:
            matchPassInit<BufType, PatchMatchSSDGradDist>(
                u, v, d, a, b, adx, ady, bdx, bdy, radius );
            std::cout << "\npatch_match SSD_GRAD_DIST eps " << eps << " radius "
                      << radius << ": " << std::endl;
            for ( int p = 0; p < iters; ++p )
            {
                if ( matchPass2<BufType, PatchMatchSSDGradDist>(
                         u,
                         v,
                         d,
                         a,
        ...
        case patch_style::GRAD:
            matchPassInit<BufType, PatchMatchGrad>(
                u, v, d, a, b, adx, ady, bdx, bdy, radius );
            std::cout << "\npatch_match GRAD eps " << eps << " radius "
                      << radius << ": " << std::endl;
            for ( int p = 0; p < iters; ++p )
            {
                if ( matchPass2<BufType, PatchMatchGrad>(
                         u,
                         v,
                         d,
                         a,
    ...
    int                 iters )
{
    ...
    switch ( static_cast<patch_style>( style ) )
    {
        case patch_style::SSD:
            matchPassInit<BufType, PatchMatchSSD>(
                u, v, d, a, b, adx, ady, bdx, bdy, alpha, radius );
            std::cout << "\npatch_match SSD eps " << eps << " radius " << radius
                      << ": " << std::endl;
            for ( int p = 0; p < iters; ++p )
            {
...
static vector_field pmPlane(
    ...
    int          iters )
{
    plane dist = create_plane(
        a.x1(),
        a.y1(),
        a.x2(),
        a.y2(),
        std::numeric_limits<plane::value_type>::max() );
    uint32_t seedU = static_cast<uint32_t>( frameA + ( frameA - frameB ) + 1 );
    uint32_t seedV = static_cast<uint32_t>( frameA + ( frameA - frameB ) - 1 );
    plane    u     = create_random_plane(
        a.x1(),
        a.y1(),
        a.x2(),
        a.y2(),
        seedU,
        static_cast<float>( a.x1() ),
        static_cast<float>( a.x2() ) );
    plane v = create_random_plane(
        a.x1(),
        a.y1(),
        a.x2(),
        a.y2(),
        seedV,
        static_cast<float>( a.y1() ),
        static_cast<float>( a.y2() ) );
...
static bool matchRefineIter(
    ...
    int                       iter,
    float                     eps,
    const const_plane_buffer &alpha = const_plane_buffer() )
{
    std::vector<size_t> counts(
        static_cast<size_t>( threading::get().size() ), size_t( 0 ) );
    size_t nChange;
    ...
    int            iters )
{
    ...
    switch ( static_cast<patch_style>( style ) )
    {
        case patch_style::SSD:
            matchPassInit<BufType, PatchMatchSSD>(
                u, v, d, a, b, adx, ady, bdx, bdy, radius );
            std::cout << "\npatch_match_refine SSD eps " << eps << " radius "
                      << radius << " iters " << iters << std::endl;
            for ( int p = 0; p < iters; ++p )
            {
                // if changes don't change, finished
                if ( matchRefineIter<BufType, PatchMatchSSD>(
                         u,
                         v,
                         d,
                         a,
        ...
        case patch_style::SSD_GRAD:
            ...
            for ( int p = 0; p < iters; ++p )
            {
                if ( matchRefineIter<BufType, PatchMatchSSDGrad>(
                         u,
                         v,
                         d,
                         a,
        ...
        case patch_style::SSD_GRAD_DIST:
            ...
            for ( int p = 0; p < iters; ++p )
            {
                if ( matchRefineIter<BufType, PatchMatchSSDGradDist>(
                         u,
                         v,
                         d,
                         a,
        ...
        case patch_style::GRAD:
            ...
            for ( int p = 0; p < iters; ++p )
            {
                if ( matchRefineIter<BufType, PatchMatchGrad>(
                         u,
                         v,
                         d,
                         a,
...
static vector_field pmHierPlane(
    const plane &a,
    const plane &b,
    const plane &alpha,
    int64_t      frameA,
    ...
    int          iters )
{
    ...
    int levelRadius =
        std::max( int( 2 ), radius - static_cast<int>( hierA.size() ) );
    vector_field prevUV;
    ...
    while ( !hierA.empty() )
    {
        ...
        float scaleX = static_cast<float>( curW ) /
                       static_cast<float>( prevUV.u().width() );
        float scaleY = static_cast<float>( curH ) /
                       static_cast<float>( prevUV.u().height() );

        vector_field newUV;
        if ( curAlpha.valid() )
        {
            plane mask = threshold( curAlpha, 0.F );
            newUV      = vector_field::create(
                resize_bicubic( prevUV.u(), curW, curH ) * mask * scaleX,
        ...
        {
            newUV = vector_field::create(
                resize_bicubic( prevUV.u(), curW, curH ) * scaleX,
...
static vector_field pmHierImage(
    const image_buf &a,
    const image_buf &b,
    const plane &    alpha,
    int64_t          frameA,
    ...
    int              iters )
{
    ...
    vector_field prevUV;
    int          refineIters =
        std::max( int( 4 ), iters *static_cast<int>( hierA.size() ) );
    while ( !hierA.empty() )
    {
    ...
    int          iters )
{
    ...
    precondition(
        a.dims() == b.dims(),
        "patch_match must have a & b of same size, received a {0} and b {1}",
        a.dims(),
        b.dims() );
    d        = a.dims();
    d.planes = 2;
    ...
    int              iters )
{
    engine::dimensions d;
    precondition(
        !a.empty() && a.dims() == b.dims(),
        "patch_match must have a & b of same size, received a {0} and b {1}",
        a.dims(),
        b.dims() );
    precondition(
        !alpha.valid() || a[0].dims() == alpha.dims(),
        "patch_match must have a & alpha of same size, received a {0} and b {1}",
        a.dims(),
        alpha.dims() );
    d        = a.dims();
    d.planes = 2;
    ...
    int          iters )
{
    ...
    precondition(
        a.dims() == b.dims(),
        "hier_patch_match must have a & b of same size, received a {0} and b {1}",
        a.dims(),
        b.dims() );
    d        = a.dims();
    d.planes = 2;
