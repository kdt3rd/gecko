...
inline plane doGradFalloff( const plane &ret, float lambda )
{
    return ret * ( exp( square( ret ) * ( -lambda * 0.5F ) ) + 0.01F );
    //return ret;
}
////////////////////////////////////////

inline plane doGradX( const plane &a, const plane &alpha, float lambda = 10.F )
{
    //	if ( alpha.valid() )
    ...
    return doGradFalloff( ret, lambda );
}

////////////////////////////////////////

inline plane doGradY( const plane &a, const plane &alpha, float lambda = 10.F )
{
    //	if ( alpha.valid() )
...
static void killAlpha( plane &u, plane &v, const plane &alpha )
{
    if ( alpha.valid() )
    {
        plane_buffer       uB   = u;
...
static void zilchAlpha( plane &u, plane &v, const plane &alpha )
{
    if ( alpha.valid() )
    {
        plane_buffer       uB   = u;
    ...
    int                       edgeBorder )
{
    ...
    if ( alpha.valid() )
    {
        ...
        threading::get().dispatch(
            std::bind(
                ahtvl1_edgeWeight_alpha_thread,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
                std::ref( eb ),
    ...
    float                     theta )
{
    ...
    for ( int y = s; y < e; ++y )
    {
        ...
        for ( int x = 0; x < sseChunks; x += 4 )
        {
            ...
            rho = _mm_add_ps( rho, _mm_mul_ps( gYAve, _mm_sub_ps( v_, v0v ) ) );

            __m128 magSq = _mm_add_ps(
                _mm_mul_ps( gXAve, gXAve ), _mm_mul_ps( gYAve, gYAve ) );
            ...
            __m128 puu_v_m1 =
                _mm_castsi128_ps( _mm_alignr_epi8( tmppuu, puuPrev, 12 ) );
            puuPrev = tmppuu;
            ...
            __m128 pvu_v_m1 =
                _mm_castsi128_ps( _mm_alignr_epi8( tmppvu, pvuPrev, 12 ) );
            pvuPrev = tmppvu;
    ...
    float                     epsilon )
{
    ...
    for ( int y = s; y < e; ++y )
    {
        ...
        if ( y < pu.y2() )
        {
            ...
            __m128i curUx =
                _mm_load_si128( reinterpret_cast<const __m128i *>( uLine ) );
            for ( int x = 0; x < sseChunks; x += 4 )
            {
                __m128i nextUx = _mm_load_si128(
                    reinterpret_cast<const __m128i *>( uLine + x + 4 ) );
                ...
                outU = _mm_add_ps(
                    outU,
                    _mm_mul_ps(
                        tauV, _mm_sub_ps( uX, _mm_mul_ps( epsV, outU ) ) ) );
                outV = _mm_add_ps(
                    outV,
                    _mm_mul_ps(
                        tauV, _mm_sub_ps( uY, _mm_mul_ps( epsV, outV ) ) ) );
    ...
    float               eta )
{
    ...
    if ( initUV.valid() )
        std::cout << "add constraint / init for initial UV passed in"
                  << std::endl;
    ...
    while ( !hierA.empty() )
    {
        ...
        else
        {
            ...
            float scaleX =
                static_cast<float>( curW ) / static_cast<float>( u.width() );
            float scaleY =
                static_cast<float>( curH ) / static_cast<float>( u.height() );
        ...
        plane puu =
            create_plane( curA.x1(), curA.y1(), curA.x2(), curA.y2(), 0.F );
        plane puv = puu.copy();
        plane pvu = puu.copy();
        plane pvv = puu.copy();
        ...
        plane edgeW;
        if ( edgeAlpha > 0.F )
            edgeW = ahtvl1_edgeWeight(
                curA, curAlpha, edgeKern, edgePower, edgeAlpha, edgeBorder );
        ...
        if ( adaptiveIters )
        {
            wI = std::max(
                std::min( 2, warpIters ),
                warpIters - static_cast<int>( hierA.size() ) );
        ...
        std::cout << "pyramid " << curA.width() << 'x' << curA.height() << '('
                  << warpIters << "->" << wI << ',' << tvl1Iters << "->" << iI
                  << ") l " << lambda << " e " << epsilon << ": ";
    ...
    for ( int y = s; y < e; ++y )
    {
        ...
        for ( int x = 0; x < w; ++x )
        {
            ...
            float reproj =
                ...
                mymaxf(
                    1.F,
                    sqrtf( p1v * p1v + p2v * p2v + p3v * p3v + p4v * p4v ) );
            p1L[x] = p1v * reproj;
            p2L[x] = p2v * reproj;
            p3L[x] = p3v * reproj;
            p4L[x] = p4v * reproj;
    ...
    float               eta )
{
    ...
    if ( initUV.valid() )
    {
        std::cout << "Implement input UV constraint / mapping options"
                  << std::endl;
    }
...
static vector_field runZilch( const vector_field &a, const plane &alpha )
{
    plane newU = a.u().copy();
    plane newV = a.v().copy();
    zilchAlpha( newU, newV, alpha );
    return vector_field::create(
        std::move( newU ), std::move( newV ), a.is_absolute() );
}
    ...
    float               eta )
{
    precondition(
        a.dims() == b.dims(),
        "oflow_ahtvl1 must have a & b of same size, received a {0} b {1}",
        a.dims(),
        b.dims() );
    ...
    float               eta )
{
    precondition(
        a.dims() == b.dims(),
        "oflow_primaldual must have a & b of same size, received a {0} b {1}",
        a.dims(),
        b.dims() );
    engine::dimensions d = a.dims();
    d.planes             = 2;
...
vector_field oflow_zilch_alpha( const vector_field &a, const plane &alpha )
{
    ...
    precondition(
        a.u().dims() == alpha.dims(),
        "zilch_alpha must have an alpha channel of same size as vectors, received a {0} alpha {1}",
        a.dims(),
        alpha.dims() );
