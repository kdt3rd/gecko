...
static void horiz_igrad( scanline &dest, const scanline &src )
{
...
static void vert_igrad( scanline &dest, int y, const plane &src )
{
...
static void horiz_cgrad( scanline &dest, const scanline &src )
{
    if ( dest.width() < 2 )
    {
        dest[0] = 0.F;
...
static void vert_cgrad( scanline &dest, int y, const plane &src )
{
    precondition( dest.get() != src.line( y ), "Need not-in-place flag to op" );
...
static void horiz_ngrad5( scanline &dest, const scanline &src )
{
    if ( dest.width() < 5 )
    {
    ...
    for ( int x = 2; x < wm2; ++x )
        dest[x] = ( ( src[x + 1] - src[x - 1] ) * 8.F +
                    ( src[x - 2] - src[x + 2] ) ) /
                  12.F;

    dest[wm2] =
        0.F; //( ( src[wm1] - src[wm2-1] ) * 8.F + ( src[wm2-2] - src[wm1] ) ) / 12.F;
    dest[wm1] =
        0.F; //( ( src[wm1] - src[wm2] ) * 8.F + ( src[wm2-1] - src[wm1] ) ) / 12.F;
}

////////////////////////////////////////

static void vert_ngrad5( scanline &dest, int y, const plane &src )
{
    precondition( dest.get() != src.line( y ), "Need not-in-place flag to op" );
...
{
    precondition(
        dest.get() != src.line( y ) && dest.get() != alpha.line( y ),
        "Need not-in-place flag to op" );
...
vert_ngrad5_alpha( scanline &dest, int y, const plane &src, const plane &alpha )
{
    ...
    //	if ( y >= (src.y1() + 2) && y <= (src.y2() - 2) )
    {
        ...
        for ( int x = 0; x < w; ++x )
        {
            float ppA = am2[x], pA = am1[x], cA = acur[x], nA = ap1[x],
                  nnA = ap2[x];
...
static void horiz_convolve3_mirror(
    scanline &dest, const scanline &src, float outer, float center )
{
    if ( dest.width() < 2 )
    {
        dest[0] = src[0];
...
static void horiz_convolve3(
    scanline &dest, const scanline &src, float left, float center, float right )
{
    if ( dest.width() < 2 )
    {
        dest[0] = src[0];
...
static void horiz_convolve(
    scanline &dest, const scanline &src, const std::vector<float> &k )
{
    if ( dest.width() < 2 )
    {
        dest[0] = src[0];
...
static void vert_convolve3_mirror(
    scanline &dest, int y, const plane &src, float outer, float center )
{
    precondition( dest.get() != src.line( y ), "Need not-in-place flag to op" );
...
static void vert_convolve(
    scanline &dest, int y, const plane &src, const std::vector<float> &k )
{
    precondition( dest.get() != src.line( y ), "Need not-in-place flag to op" );
...
plane instant_gradient_horiz( const plane &p, const plane &alpha )
{
    precondition(
        p.dims() == alpha.dims(),
        "alpha based gradient requested with different sized planes {0} vs alpha {1}",
        p.dims(),
        alpha.dims() );
...
void add_convolve( engine::registry &r )
{
    ...
    r.add(
        op( "p.igrad_h",
            base::choose_runtime( horiz_igrad ),
            scanline_plane_adapter<false, decltype( horiz_igrad )>(),
            dispatch_scan_processing,
            op::one_to_one ) );
    r.add(
        op( "p.igrad_v",
            base::choose_runtime( vert_igrad ),
            n_scanline_plane_adapter<false, decltype( vert_igrad )>(),
            dispatch_scan_processing,
            op::n_to_one ) );
    r.add(
        op( "p.cgrad_h",
            base::choose_runtime( horiz_cgrad ),
            scanline_plane_adapter<false, decltype( horiz_cgrad )>(),
            dispatch_scan_processing,
            op::one_to_one ) );
    r.add(
        op( "p.cgrad_v",
            base::choose_runtime( vert_cgrad ),
            n_scanline_plane_adapter<false, decltype( vert_cgrad )>(),
            dispatch_scan_processing,
            op::n_to_one ) );
    r.add(
        op( "p.ngrad_h5",
            base::choose_runtime( horiz_ngrad5 ),
            scanline_plane_adapter<false, decltype( horiz_ngrad5 )>(),
            dispatch_scan_processing,
            op::one_to_one ) );
    r.add(
        op( "p.ngrad_v5",
            base::choose_runtime( vert_ngrad5 ),
            n_scanline_plane_adapter<false, decltype( vert_ngrad5 )>(),
            dispatch_scan_processing,
            op::n_to_one ) );

    r.add(
        op( "p.igrad_h_alpha",
            base::choose_runtime( horiz_igrad_alpha ),
            scanline_plane_adapter<false, decltype( horiz_igrad_alpha )>(),
            dispatch_scan_processing,
            op::one_to_one ) );
    r.add(
        op( "p.igrad_v_alpha",
            base::choose_runtime( vert_igrad_alpha ),
            n_scanline_plane_adapter<false, decltype( vert_igrad_alpha )>(),
            dispatch_scan_processing,
            op::n_to_one ) );
    r.add(
        op( "p.cgrad_h_alpha",
            base::choose_runtime( horiz_cgrad_alpha ),
            scanline_plane_adapter<false, decltype( horiz_cgrad_alpha )>(),
            dispatch_scan_processing,
            op::one_to_one ) );
    r.add(
        op( "p.cgrad_v_alpha",
            base::choose_runtime( vert_cgrad_alpha ),
            n_scanline_plane_adapter<false, decltype( vert_cgrad_alpha )>(),
            dispatch_scan_processing,
            op::n_to_one ) );
    r.add(
        op( "p.ngrad_h5_alpha",
            base::choose_runtime( horiz_ngrad5_alpha ),
            scanline_plane_adapter<false, decltype( horiz_ngrad5_alpha )>(),
            dispatch_scan_processing,
            op::one_to_one ) );
    r.add(
        op( "p.ngrad_v5_alpha",
            base::choose_runtime( vert_ngrad5_alpha ),
            n_scanline_plane_adapter<false, decltype( vert_ngrad5_alpha )>(),
            dispatch_scan_processing,
            op::n_to_one ) );

    r.add(
        op( "p.sep_conv3_mirror_h",
            base::choose_runtime( horiz_convolve3_mirror ),
            scanline_plane_adapter<false, decltype( horiz_convolve3_mirror )>(),
            dispatch_scan_processing,
            op::one_to_one ) );
    r.add(
        op( "p.sep_conv3_h",
            base::choose_runtime( horiz_convolve3 ),
            scanline_plane_adapter<false, decltype( horiz_convolve3 )>(),
            dispatch_scan_processing,
            op::one_to_one ) );
    r.add(
        op( "p.sep_conv_h",
            base::choose_runtime( horiz_convolve ),
            scanline_plane_adapter<false, decltype( horiz_convolve )>(),
            dispatch_scan_processing,
            op::one_to_one ) );

    r.add( op(
        "p.sep_conv3_mirror_v",
        base::choose_runtime( vert_convolve3_mirror ),
        n_scanline_plane_adapter<false, decltype( vert_convolve3_mirror )>(),
        dispatch_scan_processing,
        op::n_to_one ) );
    r.add(
        op( "p.sep_conv3_v",
            base::choose_runtime( vert_convolve3 ),
            n_scanline_plane_adapter<false, decltype( vert_convolve3 )>(),
            dispatch_scan_processing,
            op::n_to_one ) );
    r.add(
        op( "p.sep_conv_v",
            base::choose_runtime( vert_convolve ),
            n_scanline_plane_adapter<false, decltype( vert_convolve )>(),
            dispatch_scan_processing,
            op::n_to_one ) );
}
