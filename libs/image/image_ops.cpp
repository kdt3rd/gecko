...
image_buf combine( const plane &p, const plane &p2 )
{
    ...
    precondition(
        p.dims() == p2.dims(),
        "plane dimensions should match to combine to an image" );
    engine::dimensions id = p.dims();
    id.planes             = 2;
    id.images             = 1;
...
combine( const plane &p, const plane &p2, const plane &p3, const plane &p4 )
{
    ...
    // but what about 4:2:0?
    precondition(
        p.dims() == p2.dims() && p.dims() == p3.dims() && p.dims() == p4.dims(),
        "plane dimensions should match to combine to an image" );
    engine::dimensions id = p.dims();
    id.planes             = 4;
    id.images             = 1;
...
image_buf union_dim( const image_buf &a, const image_buf &b, float outside )
{
    image_buf ret = a;
