    ...
    const media::area_rect &   area )
{
    if ( area.x1() == buf.x1() && area.x2() == buf.x2() &&
         area.y1() == buf.y1() && area.y2() == buf.y2() && buf.offset() == 0 )
    {
        int64_t bytes = static_cast<int64_t>( ( int64_t( pl._bits ) + 7 ) / 8 );
        int64_t ystride =
            ( area.width() * bytes ) >> int64_t( pl._xsubsample_shift );
    ...
    buf = media::image_buffer::full_plane(
        area.x1(),
        area.y1(),
        area.x2(),
        area.y2(),
...
protected:
    ...
    void fill_plane( size_t pIdx, media_buf &buffer ) override
    {
        precondition(
            pIdx < _image.size(),
            "Invalid plane {0} requested ({1} available)",
            pIdx,
            _image.size() );
    ...
    {
        precondition(
            planes.size() == size(), "expect all planes for output image" );
    ...
    const std::vector<std::string> &planes )
{
    ...
    if ( img )
    {
        ...
        else
        {
            ...
            for ( auto &plane: planes )
            {
                ...
                if ( !found )
                    throw_runtime(
                        "Request for channel '{0}' in layer '{0}', view '{1}' does not exist in frame {2}",
                        plane,
                        layer,
                        view,
                        f.number() );
    ...
    else
    {
        ...
        throw_runtime(
            "Request for layer '{0}', view '{1}' does not exist in frame {2}",
            layer,
            view,
            f.number() );
    }
...
std::shared_ptr<media::frame> to_frame(
    const image_buf &               i,
    const std::vector<std::string> &chans,
    const std::string &             type,
    const media::metadata &         meta )
{
    if ( i.size() < chans.size() )
        throw_runtime(
            "image does not have enough channels ({0}) for requested channel list size ({1})",
            i.size(),
            chans.size() );

    auto r = std::make_shared<media::frame>();
    // TODO: add layer / view names?
    r->register_layer( base::cstring() )
        .add_view( base::cstring() )
        .store( std::make_shared<frame_image_adapter>( i, chans, type, meta ) );
    return r;
#if 0
...
void debug_save_image(
    const image_buf &               i,
    const std::string &             fn,
    int64_t                         sampNum,
    const std::vector<std::string> &chans,
    const std::string &             type,
    const media::parameter_set &    params,
    const media::metadata &         meta )
{
    ...
    oc.video_tracks()[0]->store(
        sampNum, to_frame( i, chans, type, meta ), allocator::get() );
    std::cout << "Saved debug image to '" << fn << "', frame " << sampNum
              << std::endl;
}
