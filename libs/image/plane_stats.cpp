...
compute_variance( scanline &dest, int y, const plane &p, int radius )
{
    ...
    // passes :(
    const double n =
        static_cast<double>( ( radius * 2 + 1 ) * ( radius * 2 + 1 ) );
    int wm1 = dest.width() - 1;
    for ( int x = 0; x <= wm1; ++x )
    {
        double K   = 0.0;
...
static accum_buf compute_SAT( const plane &p, int power )
{
...
compute_histogram( const plane &p, int bins, float lowV, float highV )
{
    ...
    if ( n > 1 )
    {
        ...
        vals.resize(
            nThread, std::vector<uint64_t>( static_cast<size_t>( bins ), 0 ) );
    ...
    const float  sigma )
{
    ...
    const double c2 =
        static_cast<double>( k2 * L ) * static_cast<double>( k2 * L );
    ...
    gweight.resize( static_cast<size_t>( radius * 2 + 1 ), 0.f );
    int i = -radius;
    for ( auto &g: gweight )
    {
        ...
        for ( int cy = yRangeB; cy <= yRangeE; ++cy )
        {
            ...
            if ( cy == yRangeB )
            {
                K = 0.5 * static_cast<double>( srcL[xrb] ) +
                    static_cast<double>( srcL[xre] );
                K2 = 0.5 * static_cast<double>( srcL2[xrb] ) +
                     static_cast<double>( srcL2[xre] );
            }
            size_t widxx = 0;
            for ( int cx = xrb; cx <= xre; ++cx )
            {
                double weight =
                    weightY * static_cast<double>( gweight[widxx++] );
                double lv1 = ( static_cast<double>( srcL[cx] ) - K );
                double lv2 = ( static_cast<double>( srcL2[cx] ) - K2 );
                Ex += lv1 * weight;
                Ey += lv2 * weight;
                Exy += lv1 * lv2 * weight;
                Exx += lv1 * lv1 * weight;
...
TODO(
    "check breakover point where local ops are worth the summed area table optimization" )
...
plane local_variance( const plane &p, int radius )
{
    ...
    return local_variance(
        sum_area_table( p, 1 ), sum_area_table( p, 2 ), radius );
}
...
plane local_mean( const accum_buf &sat, int radius )
{
    ...
    d.bytes_per_item =
        static_cast<engine::dimensions::value_type>( sizeof( float ) );
...
plane mse( const plane &p, const plane &p2, int radius )
{
    precondition(
        p.dims() == p2.dims(),
        "unable to compute MSE for planes of different sizes" );
    ...
    float        sigma )
{
    precondition(
        p.dims() == p2.dims(),
        "unable to compute SSIM for planes of different sizes" );
    return plane( "p.ssim", p.dims(), p, p2, radius, L, k1, k2, sigma );
}
...
histogram( const plane &p, int bins, float lowV, float highV )
{
    ...
    d.x1 = 0;
    d.y1 = 0;
    d.x2 = static_cast<engine::dimensions::value_type>( bins - 1 );
    d.y2 = 0;
    d.bytes_per_item =
        static_cast<engine::dimensions::value_type>( sizeof( uint64_t ) );
    return engine::computed_value<std::vector<uint64_t>>(
        op_registry(), "p.histogram", d, p, bins, lowV, highV );
}
...
void add_plane_stats( engine::registry &r )
{
    ...
    r.add(
        op( "p.sum_area_table",
            base::choose_runtime(
                compute_SAT,
                { { base::cpu::simd_feature::SSE3, sse3::compute_SAT } } ),
            op::threaded ) );
    ...
    r.add(
        op( "p.local_mean_sat",
            base::choose_runtime( compute_mean_sat ),
            n_scanline_plane_adapter<false, decltype( compute_mean_sat )>(),
            dispatch_scan_processing,
            op::n_to_one ) );
    r.add(
        op( "p.local_variance_sat",
            base::choose_runtime( compute_variance_sat ),
            n_scanline_plane_adapter<false, decltype( compute_variance_sat )>(),
            dispatch_scan_processing,
            op::n_to_one ) );

    r.add(
        op( "p.local_mean",
            base::choose_runtime( compute_mean ),
            n_scanline_plane_adapter<false, decltype( compute_mean )>(),
            dispatch_scan_processing,
            op::n_to_one ) );
    r.add(
        op( "p.local_variance",
            base::choose_runtime( compute_variance ),
            n_scanline_plane_adapter<false, decltype( compute_variance )>(),
            dispatch_scan_processing,
            op::n_to_one ) );

    r.add(
        op( "p.mean_square_error",
            base::choose_runtime( compute_mse ),
            n_scanline_plane_adapter<false, decltype( compute_mse )>(),
            dispatch_scan_processing,
            op::n_to_one ) );
    r.add(
        op( "p.ssim",
            base::choose_runtime( compute_ssim ),
            n_scanline_plane_adapter<false, decltype( compute_ssim )>(),
            dispatch_scan_processing,
            op::n_to_one ) );
}
