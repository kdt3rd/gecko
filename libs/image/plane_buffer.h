...
public:
    ...
    inline size_t buffer_size( void ) const
    {
    ...
    inline const value_type *line( int y ) const
    {
        return cdata() + ( y - _y1 ) * stride();
    }
    ...
    inline value_type &get( int x, int y )
    {
        return *( line( y ) + ( x - _x1 ) );
    }
    /// Get a value from the plane
    /// @sa plane_util.h for other access methods
    inline value_type get( int x, int y ) const
    {
        return *( line( y ) + ( x - _x1 ) );
    }
...
public:
    ...
    inline constexpr const_plane_buffer(
        const float *p, int x1, int y1, int x2, int y2, int s )
        : _ptr( p ), _x1( x1 ), _y1( y1 ), _x2( x2 ), _y2( y2 ), _stride( s )
    {}
    ...
    inline constexpr size_t buffer_size( void ) const
    {
        return static_cast<size_t>( stride() * height() ) *
               sizeof( value_type );
    }
    ...
    inline constexpr const value_type *line( int y ) const
    {
        return cdata() + ( y - _y1 ) * stride();
    }

    inline constexpr bool in_bounds_x( int x ) const
    {
        return x >= _x1 && x <= _x2;
    }
    inline constexpr bool in_bounds_y( int y ) const
    {
        return y >= _y1 && y <= _y2;
    }
    inline constexpr bool in_bounds( int x, int y ) const
    {
        return in_bounds_x( x ) && in_bounds_y( y );
    }
    ...
    /// Get a value from the plane
    /// @sa plane_util.h for other access methods
    inline constexpr value_type get( int x, int y ) const
    {
        return *( line( y ) + ( x - _x1 ) );
    }
