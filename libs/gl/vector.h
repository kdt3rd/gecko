...
public:
    ...
    template <size_t M> vector( const vector<M> &v )
    {
    ...
    template <typename F1, typename F2> static vector polar( F1 r, F2 a )
    {
        static_assert(
        ...
        vector result;
        result[0] =
            static_cast<float>( r ) * static_cast<float>( std::cos( a ) );
        result[1] =
            static_cast<float>( r ) * static_cast<float>( std::sin( a ) );
        return result;
    }
