...
template <typename... Args> class vertex_buffer_data
{
public:
    ...
    void pop_back( void )
    {
        ...
        _data.erase(
            _data.end() -
                static_cast<std::vector<float>::difference_type>( stride() ),
            _data.end() );
    }

    /// @brief Returns the number of vertices.
    size_t size( void ) const { return _data.size() / stride(); }
    ...
    const float *data( void ) const { return _data.data(); }
    ...
    {
        static const constexpr std::array<size_t, sizeof...( Args )> sizes = {
            { detail::count_stride<Args>::value... }
        };
        size_t sum = 0;
    ...
    size_t attributes( void ) const { return sizeof...( Args ); }
    ...
    vbo( buffer_usage u = buffer_usage::STATIC_DRAW )
    {
        if ( !_vbo )
...
private:
    void push_back( const tuple &t ) { push_back( t.data ); }

    template <typename... More> void append( const vec2 &p, More... args )
    {
        _data.push_back( p[0] );
        _data.push_back( p[1] );
        append( args... );
    ...
    template <typename... More> void append( const vec3 &p, More... args )
    {
        _data.push_back( p[0] );
        _data.push_back( p[1] );
        _data.push_back( p[2] );
    ...
    template <typename... More> void append( const vec4 &p, More... args )
    {
        _data.push_back( p[0] );
        _data.push_back( p[1] );
        _data.push_back( p[2] );
    ...
    template <typename... More> void append( const color &c, More... args )
    {
    ...
    template <typename... More> void append( float f, More... args )
    {
        _data.push_back( f );
        append( args... );
    }
