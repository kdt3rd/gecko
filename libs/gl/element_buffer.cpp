...
void element_buffer::binding::data( const uint32_t *data, size_t n, buffer_usage u )
{
...
void element_buffer::binding::data( const std::vector<uint32_t> &data, buffer_usage u )
{
	precondition( _bound == this, "element buffer not bound" );
...
void element_buffer::binding::sub_data( const uint32_t *data, size_t offset, size_t n )
{
	precondition( _bound == this, "element buffer not bound" );
...
void element_buffer::binding::sub_data( const std::vector<uint32_t> &data, size_t offset )
{
	precondition( _bound == this, "element buffer not bound" );
...
element_buffer::element_buffer( void )
{
...
element_buffer::element_buffer( const std::vector<uint32_t> &data, buffer_usage u )
	: element_buffer()
{
	auto bb = bind();
	bb.data( data, u );
