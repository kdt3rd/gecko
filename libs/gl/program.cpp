...
program::program( void ) { _program = glCreateProgram(); }
...
void program::set_uniform( uniform uni, const vec4 &value )
{
    precondition( _using == this, "program not in use" );
    glUniform4f(
        static_cast<GLint>( uni ), value[0], value[1], value[2], value[3] );
}
...
std::pair<uniform_type, std::string> program::active_uniform( size_t i )
{
    ...
    GLenum      type;
    glGetActiveUniform(
        _program, static_cast<GLuint>( i ), max, &len, &sz, &type, &name[0] );
