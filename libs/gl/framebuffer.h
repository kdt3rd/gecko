...
public:
    ...
    public:
        ...
        binding( binding &&other ) : _fb( other._fb ) { other._fb = 0; }
        ...
        void attach( const texture &t )
        {
            glFramebufferTexture(
                GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, t.id(), 0 );
        }

    private:
        friend class framebuffer;

        binding( GLuint fb ) : _fb( fb )
        {
        ...
        void unbind( void )
        {
            ...
            precondition(
                _stack.back() == this,
                "framebuffer destroyed out of sequence" );
            _stack.pop_back();
            if ( _stack.empty() )
    ...
    inline bool is_valid( void ) const { return _fb > 0; }

    binding bind( void ) { return binding( _fb ); }
