...
public:
    ...
    void clear(
        buffer_bits bits =
            ( buffer_bit::COLOR_BUFFER_BIT | buffer_bit::DEPTH_BUFFER_BIT ) );
    ...
    const matrix4 &view_matrix( void ) const { return _view; }

    void set_view( const matrix4 &m ) { _view = m; }

    const matrix4 &projection_matrix( void ) const { return _projection; }

    void set_projection( const matrix4 &m ) { _projection = m; }
    ...
    std::shared_ptr<shader> new_vertex_shader( Args &&... args )
    {
        return std::make_shared<shader>(
            gl::shader::type::VERTEX, std::forward<Args>( args )... );
    }
    ...
    {
        return std::make_shared<shader>(
            gl::shader::type::FRAGMENT, std::forward<Args>( args )... );
    }
    ...
    /// @brief Construct a new vertex buffer
    std::shared_ptr<vertex_buffer> new_vertex_buffer(
        const float *data,
        size_t       n,
