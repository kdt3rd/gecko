...
public:
    ...
    template <typename... Args> void push_back( uint32_t i, Args &&... args )
    {
    ...
    {
        precondition(
            !_ebo, "cannot remove index after creating buffer object" );
        _data.pop_back();
    }
    ...
    const uint32_t *data( void ) const { return _data.data(); }

    /// @brief Return the ith index.
    /// @param i Index of index to return.
    /// @returns ith index in array.
    uint32_t operator[]( size_t i ) const { return _data[i]; }
    ...
    std::shared_ptr<element_buffer>
    ebo( buffer_usage u = buffer_usage::STATIC_DRAW )
    {
        if ( !_ebo )
