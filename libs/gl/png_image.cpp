...
std::vector<uint8_t> png_read_data( const char *file_name, size_t &w, size_t &h, size_t &c )
{
...
gl::texture png_read( const char *file_name )
{
	...
	glTexImage2D( GL_TEXTURE_RECTANGLE, 0, GL_RGBA, static_cast<GLsizei>( w ), static_cast<GLsizei>( h ), 0, fmt, GL_UNSIGNED_BYTE, data.data() );
