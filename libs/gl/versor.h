...
public:
    ...
    versor( const std::array<float, 4> &list ) : _q( list ) {}
    ...
    float operator[]( size_t i ) const { return _q[i]; }

    /// @brief Access element at i
    float &operator[]( size_t i ) { return _q[i]; }

    /// @brief Access element at i
    float at( size_t i ) const { return _q.at( i ); }

    /// @brief Access element at i
    float &at( size_t i ) { return _q.at( i ); }
