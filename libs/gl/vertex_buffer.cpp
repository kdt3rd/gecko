...
void vertex_buffer::binding::data(
    const std::vector<float> &data, buffer_usage u )
{
    precondition( _bound == this, "vertex buffer not bound" );
    glBufferData(
        GL_ARRAY_BUFFER,
...
void vertex_buffer::binding::sub_data(
    const float *data, size_t offset, size_t n )
{
    precondition( _bound == this, "vertex buffer not bound" );
    glBufferSubData(
        GL_ARRAY_BUFFER,
        static_cast<GLsizeiptr>( offset * sizeof( float ) ),
        static_cast<GLsizeiptr>( n * sizeof( float ) ),
        data );
}
...
vertex_buffer::vertex_buffer( void ) { glGenBuffers( 1, &_buffer ); }
...
vertex_buffer::~vertex_buffer( void ) { glDeleteBuffers( 1, &_buffer ); }
